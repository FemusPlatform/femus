...
class DenseMatrixM : public DenseMatrixBaseM {
 ...
 public:
  ...
  /// @returns the \p (i,j) element of the matrix as a writeable reference.
  virtual double el(const int i, const int j) const { return (*this)(i, j); }
  /// @returns the \p (i,j) element of the matrix as a writeable reference.
   ...
   */
  void condense(const int i, const int j, const double val, DenseVectorBaseM& rhs) {
 ...
 private:
   ...
   * [ Implementation in dense_matrix_blas_lapack.C ]
   */
  void _lu_back_substitute_lapack(const DenseVectorM& b, DenseVectorM& x);
  ...
  /// BLAS operations
  void _multiply_blas(const DenseMatrixBaseM& other, _BLAS_Multiply_Flag flag);
...
inline DenseMatrixM::DenseMatrixM(const int m, const int n)
...
// =========================================
inline void DenseMatrixM::swap(DenseMatrixM& other_matrix) {
...
// ===============================================
inline void DenseMatrixM::resize(const int m, const int n) {
  _val.resize(m * n);
  this->_m = m;
  this->_n = n;
  _decomposition_type = NONE;
...
// ==========================================
inline void DenseMatrixM::zero() {
  _decomposition_type = NONE;
  std::fill(_val.begin(), _val.end(), 0.);
}

// =============================================
inline DenseMatrixM& DenseMatrixM::operator=(const DenseMatrixM& other_matrix) {
...
// =========================================
inline double DenseMatrixM::operator()(const int i, const int j) const {
...
// ========================================================
inline double& DenseMatrixM::operator()(const int i, const int j) {
  assert(i * j < (int)_val.size());
  assert(i < this->_m);
  assert(j < this->_n);
  // return _val[(i) + (this->_m)*(j)]; // col-major
...
// =================================================
inline void DenseMatrixM::scale(const double factor) {
  for (int i = 0; i < (int)_val.size(); i++) _val[i] *= factor;
}

// ===============================================
inline DenseMatrixM& DenseMatrixM::operator*=(const double factor) {
  this->scale(factor);
  return *this;
}
...
inline bool DenseMatrixM::operator==(const DenseMatrixM& mat) const {
  for (int i = 0; i < (int)_val.size(); i++)
    if (_val[i] != mat._val[i]) return false;
  return true;
}
...
inline bool DenseMatrixM::operator!=(const DenseMatrixM& mat) const {
  for (int i = 0; i < (int)_val.size(); i++)
    if (_val[i] != mat._val[i]) return true;
  return false;
}
