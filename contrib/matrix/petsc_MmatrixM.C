...
void PetscMMatrixM::_get_submatrix(
  ...
  int ierr = 0;
  IS isrow, iscol;
  ierr = ISCreateGeneral(this->comm().get(), rows.size(), (int*)&rows[0], PETSC_USE_POINTER, &isrow);
  CHKERRABORT(this->comm().get(), ierr);

  ierr = ISCreateGeneral(this->comm().get(), cols.size(), (int*)&cols[0], PETSC_USE_POINTER, &iscol);
  CHKERRABORT(this->comm().get(), ierr);
  ...
  ierr = MatGetSubMatrix(
      ...
      (reuse_submatrix ? MAT_REUSE_MATRIX : MAT_INITIAL_MATRIX), &(petsc_submatrix->_mat));
  CHKERRABORT(this->comm().get(), ierr);
#else
  ierr = MatGetSubMatrix(
