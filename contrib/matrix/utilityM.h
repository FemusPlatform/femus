...
// ------------------------------------------------------------
// The UtilityM namespace is for functions
...
//-------------------------------------------------------------------
/**
 * \p UtilityM::iota is a duplication of the SGI STL extension
...
/**
 * UtilityM::is_sorted mimics the behavior of the SGI STL extension
 ...
 * on whether the searched-for value is found.  UtilityM::binary_find() uses a
...
inline unsigned int factorial(unsigned int n) {
  ...
  if (n == 0) return factorial_n;

  for (unsigned int i = 1; i < n; i++) factorial_n *= i + 1;
...
//-------------------------------------------------------------------
// UtilityM functions useful when dealing with complex numbers.
...
class ReverseBytes {
 public:
   ...
   * do byte reversing.
   */
  explicit ReverseBytes(const bool dr);
...
inline ReverseBytes::ReverseBytes(const bool rb) : _do_reverse(rb) {}

template <typename T>
inline T ReverseBytes::operator()(T& data) const {
