...
public:
   ...
   */
  void add_vector_transpose (const NumericVectorM &V,
		             const SparseMatrixM &A_trans);
...
inline PetscVectorM::PetscVectorM (
  const ParallelM::Communicator &comm,
  const int n,const ParallelTypeM type)
        : NumericVectorM(comm, type),_array_is_present(false),
        _local_form(NULL),
        _values(NULL),
        _global_to_local_map(),
        _destroy_vec_on_exit(true)
...
// #else
      PetscInt n;
      ierr = ISLocalToGlobalMappingGetSize(mapping, &n);CHKERRABORT(MPI_COMM_WORLD,ierr);
...
// #endif
      ...
      ierr = ISLocalToGlobalMappingGetIndices(mapping,&indices);CHKERRABORT(MPI_COMM_WORLD,ierr);
#endif
      for(unsigned int i=ghost_begin; i<ghost_end; i++)
        _global_to_local_map[indices[i]] = i-local_size;
      this->_type = GHOSTEDM;
#if !PETSC_VERSION_RELEASE || !PETSC_VERSION_LESS_THAN(3,1,1)
      ierr = ISLocalToGlobalMappingRestoreIndices(mapping, &indices);CHKERRABORT(MPI_COMM_WORLD,ierr);
#endif
    }
...
// =-=======================================================
inline void PetscVectorM::init (const int n,
                                const int n_local,
                                const bool fast,
                                const ParallelTypeM type){
...
// ================================================
inline
int PetscVectorM::local_size () const{
    assert (this->initialized());
    int ierr=0, petsc_size=0;
