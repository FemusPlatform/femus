...
#define semiparallel_onlyM()                            \
  do {                                                  \
    if (this->initialized()) {                          \
      ...
      if (!strcmp(mytype, MATSEQAIJ)) parallel_onlyM(); \
    }                                                   \
  } while (0)
#else
#define semiparallel_onlyM()
...
class PetscMMatrixM : public SparseMMatrixM {
 ...
 public:
  ...
  explicit PetscMMatrixM(const ParallelM::Communicator& comm CAN_DEFAULT_TO_COMMWORLD);
  //   PetscMMatrixM();
  /// Constructor II.
  explicit PetscMMatrixM(
  ...
  // init
  void init(
      const int m, const int n, const int m_l, const int n_l,  ///< Initialize a Petsc matrix
      const int nnz = 0, const int noz = 0);
  void init();  ///< Initialize using sparsity structure
  void init(const int m, const int n) {
    _m = m;
    _n = n;
  }  ///< Initialize using sparsity structure
  // Destructors ----------------------------
  /// Destructor
  ~PetscMMatrixM();
  ...
  void update_sparsity_pattern(
      int m, int n, int m_l, int n_l, const std::vector<int> n_oz, const std::vector<int> n_nz);
  // set values
 ...
 protected:
  ...
  virtual void _get_submatrix(
      SparseMMatrixM& submatrix, const std::vector<int>& rows, const std::vector<int>& cols,
      const bool reuse_submatrix) const;
};

//-----------------------------------------------------------------------
// PetscMMatrixM inline members
inline PetscMMatrixM::PetscMMatrixM(const ParallelM::Communicator& comm)
    : SparseMMatrixM(comm), _destroy_mat_on_exit(true) {}
// =================================================================
inline PetscMMatrixM::PetscMMatrixM(
    Mat m,  ///< matrix already exists
    const ParallelM::Communicator& comm)
    : SparseMMatrixM(comm), _destroy_mat_on_exit(false) {
  this->_mat = m;
...
inline void PetscMMatrixM::set(const int i, const int j, const double value) {
  assert(this->initialized());
  int ierr = 0, i_val = i, j_val = j;
  PetscScalar petsc_value = static_cast<PetscScalar>(value);
  ierr = MatSetValues(_mat, 1, &i_val, 1, &j_val, &petsc_value, INSERT_VALUES);
  CHKERRABORT(this->comm().get(), ierr);
}

// =================================================
inline void PetscMMatrixM::add(const int i, const int j, const double value) {
  assert(this->initialized());

  int ierr = 0, i_val = i, j_val = j;

  PetscScalar petsc_value = static_cast<PetscScalar>(value);
  ierr = MatSetValues(_mat, 1, &i_val, 1, &j_val, &petsc_value, ADD_VALUES);
  CHKERRABORT(this->comm().get(), ierr);
}

// =====================================================
inline void PetscMMatrixM::add_matrix(const DenseMatrixM& dm, const std::vector<int>& dof_indices) {
...
// ========================================================
inline double PetscMMatrixM::operator()(const int i, const int j) const {
  assert(this->initialized());
