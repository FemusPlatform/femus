...
// * =====================================
double DistributedVectorM::sum() const {  // This function must be run on all processors at once
...
// * ========================================
double DistributedVectorM::l1_norm() const {  // This function must be run on all processors at once
                                              //   parallel_onlyM();
  parallel_object_onlyM();
  assert(this->initialized());
  assert((int)(int)_values.size() == _local_size);
...
// * ==========================================
double DistributedVectorM::l2_norm() const {  // This function must be run on all processors at once
                                              //   parallel_onlyM();
  parallel_object_onlyM();
  assert(this->initialized());
  assert((int)(int)_values.size() == _local_size);
...
// * =============================================
double DistributedVectorM::linfty_norm() const {  // This function must be run on all processors at once
                                                  //   parallel_onlyM();
  parallel_object_onlyM();
  assert(this->initialized());
  assert((int)(int)_values.size() == _local_size);
  ...
  for (numeric_index_type i = 0; i < local_size(); i++)
    local_linfty = std::max(
        local_linfty,
        static_cast<double>(std::abs(_values[i])));  // Note we static_cast so that both
...
// ===============================================
NumericVectorM& DistributedVectorM::operator+=(const NumericVectorM& v) {
  assert(this->closed());
  assert(this->initialized());
...
void DistributedVectorM::add_vector(
    const std::vector<double>& v, const std::vector<numeric_index_type>& dof_indices) {
  assert(!v.empty());
...
// * ============================================
void DistributedVectorM::add_vector(
    const NumericVectorM& V, const std::vector<numeric_index_type>& dof_indices) {
...
// * ============================================
void DistributedVectorM::add_vector(
    const DenseVectorM& V, const std::vector<numeric_index_type>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
void DistributedVectorM::add(const double a, const NumericVectorM& v) {
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
void DistributedVectorM::insert(const NumericVectorM& V, const std::vector<numeric_index_type>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
void DistributedVectorM::insert(const DenseVectorM& V, const std::vector<int>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
void DistributedVectorM::insert(
    const DenseSubVectorM& V, const std::vector<numeric_index_type>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
void DistributedVectorM::scale(const double factor) {
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
double DistributedVectorM::dot(const NumericVectorM& V) const {
  ...
  for (int i = 0; i < this->local_size(); i++) local_dot += this->_values[i] * v->_values[i];
...
// ============================================
NumericVectorM& DistributedVectorM::operator=(const double s) {
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
NumericVectorM& DistributedVectorM::operator=(const NumericVectorM& v_in) {
  // Make sure the NumericVector passed in is really a DistributedVectorM
...
// ============================================
DistributedVectorM& DistributedVectorM::operator=(const DistributedVectorM& v) {
...
// ============================================
NumericVectorM& DistributedVectorM::operator=(const std::vector<double>& v) {
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);

  if ((int)v.size() == local_size())
    _values = v;

  else if ((int)v.size() == size())
    for (int i = first_local_index(); i < last_local_index(); i++) _values[i - first_local_index()] = v[i];
  else {
    abort();
  }
...
// ============================================
void DistributedVectorM::localize(NumericVectorM& v_local_in) const {
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
// ============================================
void DistributedVectorM::localize(NumericVectorM& v_local_in, const std::vector<numeric_index_type>&) const {
  assert(this->initialized());
  assert((int)_values.size() == _local_size);
  assert((_last_local_index - _first_local_index) == _local_size);
...
void DistributedVectorM::localize(
    const numeric_index_type first_local_idx, const numeric_index_type last_local_idx,
...
// ============================================
void DistributedVectorM::localize_to_one(std::vector<double>& v_local, const processor_id_type pid) const {
  // This function must be run on all processors at once
  parallel_object_onlyM();
  //    parallel_onlyM();
...
// ============================================
void DistributedVectorM::pointwise_mult(const NumericVectorM&, const NumericVectorM&)
