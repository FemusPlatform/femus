...
std::unique_ptr<NumericVectorM> NumericVectorM::build(
  ...
  switch (solver_package) {
    ...
    default:
      ...
      std::cout << "NumericVectorM::build: Library not defined";
      abort();
...
int NumericVectorM::compare(const NumericVectorM& other_vector, const double threshold) const {
  assert(this->initialized());
  assert(other_vector.initialized());
  assert(this->first_local_index() == other_vector.first_local_index());
  assert(this->last_local_index() == other_vector.last_local_index());
...
// =================================
int NumericVectorM::local_relative_compare(const NumericVectorM& other_vector, const double threshold) const {
  assert(this->initialized());
  assert(other_vector.initialized());
  assert(this->first_local_index() == other_vector.first_local_index());
  assert(this->last_local_index() == other_vector.last_local_index());
  ...
  do {
    if (std::abs((*this)(i)-other_vector(i)) >
        threshold * std::max(std::abs((*this)(i)), std::abs(other_vector(i))))
      first_different_i = i;
    else
      i++;
  } while (first_different_i == std::numeric_limits<int>::max() && i < last_local_index());
...
int NumericVectorM::global_relative_compare(
    const NumericVectorM& other_vector, const double threshold) const {
  assert(this->initialized());
  assert(other_vector.initialized());
  assert(this->first_local_index() == other_vector.first_local_index());
  assert(this->last_local_index() == other_vector.last_local_index());
