...
// ==================================================
std::pair<unsigned int, Real> LaspackLinearSolverM::solve(
  ...
  switch (this->_solver_type) {
    ...
    default: {
      std::cerr << "ERROR:  Unsupported LASPACK Solver: " << this->_solver_type << std::endl
...
void LaspackLinearSolverM::MGSolve(
    std::vector<SparseMatrixM*>& matrix_in, std::vector<NumericVectorM*>& solution_in,
    std::vector<NumericVectorM*>& rhs_in, Matrix* P, Matrix* R, const double /*tol*/,
    const unsigned int /* m_its*/) {
  uint MaxIter = 15;
  double Eps = 1.e-6;
  ...
  if (PrecondProc == ILUPrecond || PrecondProcC == ILUPrecond) {
    ...
    if (PrecondProc == ILUPrecond && SmoothProc != JacobiIter && SmoothProc != SORForwIter &&
        SmoothProc != SORBackwIter && SmoothProc != SSORIter) {
      for (Level = NoLevels - 1; Level > 0; Level--) ILUFactor(&LA[Level]);
    }
    if (PrecondProcC == ILUPrecond && SolvProc != JacobiIter && SolvProc != SORForwIter &&
        SolvProc != SORBackwIter && SolvProc != SSORIter) {
      ILUFactor(&LA[0]);
    }
  ...
  if (MLSolverId == MGIterId && NestedMG) {
    ...
    NestedMGIter(
        NoLevels, LA, uwx, frb, R, P, Gamma, SmoothProc, Nu1, Nu2, PrecondProc, Omega, SolvProc, NuC,
        PrecondProcC, OmegaC);
  ...
  switch (MLSolverId) {
    case MGIterId:
      ...
      MGIter(
          NoLevels, LA, uwx, frb, R, P, MaxIter, Gamma, SmoothProc, Nu1, Nu2, PrecondProc, Omega, SolvProc,
    ...
    case MGPCGIterId:
      ...
      MGPCGIter(
          NoLevels, LA, uwx, frb, R, P, MaxIter, NoMGIter, Gamma, SmoothProc, Nu1, Nu2, PrecondProc, Omega,
          SolvProc, NuC, PrecondProcC, OmegaC);
      break;
    case BPXPCGIterId:
      printf(" Doing BPX preconditioned CG iterations \n");
      BPXPCGIter(
          NoLevels, LA, uwx, frb, R, P, MaxIter, SmoothProc, Nu1, PrecondProc, Omega, SolvProc, NuC,
          PrecondProcC, OmegaC);
      break;
    default: break;
  }
  ...
  // #endif
  //   for(int Level=1;Level<V_GetDim(&uw[NoLevels - 1])-1;Level++) printf(" %g ",V_GetCmp(&uw[NoLevels -
  //   1],Level));
...
void LaspackLinearSolverM::set_laspack_preconditioner_type() {
  switch (this->_preconditioner_type) {
    ...
    default:
      std::cerr << "ERROR:  Unsupported LASPACK Preconditioner: " << this->_preconditioner_type << std::endl
