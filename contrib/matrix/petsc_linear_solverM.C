  ...
  if (this->initialized()) {
    ...
    if (!this->_preconditioner) {
      if (this->n_processors() == 1)
        ...
        this->_preconditioner_type = BLOCK_JACOBI_PRECONDM;
    }
  }
}
...
void PetscLinearSolverM::restrict_solve_to(
  ...
  if (dofs != NULL) {
    ...
    ierr = ISCreateGeneral(
        this->comm().get(), dofs->size(), petsc_dofs, PETSC_OWN_POINTER, &_restrict_solve_to_is);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
  }
}
...
std::pair<int, double> PetscLinearSolverM::solve(
    SparseMatrixM& matrix_in, SparseMatrixM& precond_in, NumericVectorM& solution_in, NumericVectorM& rhs_in,
    const double tol, const int m_its, const int clearing) {
  ...
  if (clearing == 1) {
    this->clear();
    std::cout << " Clearing the precond " << std::endl;
  }
  ...
  if (_restrict_solve_to_is != NULL) {
    ...
    ierr = MatGetSubMatrix(
        matrix->mat(), _restrict_solve_to_is, _restrict_solve_to_is, MAT_INITIAL_MATRIX, &submat);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
    ierr = MatGetSubMatrix(
        precond->mat(), _restrict_solve_to_is, _restrict_solve_to_is, MAT_INITIAL_MATRIX, &subprecond);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
    ...
    if (_subset_solve_mode != SUBSET_ZERO) {
      ...
      ierr = VecScatterBegin(
          scatter1, _subset_solve_mode == SUBSET_COPY_RHS ? rhs->vec() : solution->vec(), subvec1,
      ...
      ierr = MatGetSubMatrix(
          matrix->mat(), _restrict_solve_to_is, _restrict_solve_to_is_complement, MAT_INITIAL_MATRIX,
          &submat1);
      CHKERRABORT(MPI_COMM_WORLD, ierr);
  ...
  //       ierr = MatSetOption(matrix->mat(),MAT_SPD,PETSC_TRUE);       CHKERRABORT(MPI_COMM_WORLD,ierr); /*
  //       set MUMPS id%SYM=1 */
  //      }
  ...
  //       CHKERRABORT(MPI_COMM_WORLD,ierr);
  //
  //       ierr = PCFactorSetUpMatSolverPackage(_pc);           CHKERRABORT(MPI_COMM_WORLD,ierr);   /* call
  //       MatGetFactor() to create F */ ierr = PCFactorGetMatrix(_pc,&precond_);
  //       CHKERRABORT(MPI_COMM_WORLD,ierr); icntl=7; ival = 2; ierr = MatMumpsSetIcntl(precond_,icntl,ival);
  //       CHKERRABORT(MPI_COMM_WORLD,ierr);
  //   }
...
std::pair<int, double> PetscLinearSolverM::adjoint_solve(
    SparseMatrixM& matrix_in, NumericVectorM& solution_in, NumericVectorM& rhs_in, const double tol,
    const int m_its) {
...
// 2.1.x & earlier style
  ...
  // Based on
  // http://wolfgang.math.tamu.edu/svn/public/deal.II/branches/MATH676/2008/deal.II/lac/source/petsc_solver.cc,
  // http://tccc.iesl.forth.gr/AMS_EPEAEK/Elements/doc/in_html/petsc/SLES/index.html
...
// 2.2.1 & newer style
  ...
  if (_restrict_solve_to_is != NULL) {
    ...
    ierr = MatGetSubMatrix(
        matrix->mat(), _restrict_solve_to_is, _restrict_solve_to_is, PETSC_DECIDE, MAT_INITIAL_MATRIX,
        &submat);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
    ierr = MatGetSubMatrix(
        precond->mat(), _restrict_solve_to_is, _restrict_solve_to_is, PETSC_DECIDE, MAT_INITIAL_MATRIX,
        &subprecond);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
#else
    ierr = MatGetSubMatrix(
...
#if PETSC_VERSION_LESS_THAN(3, 1, 0)
      ierr = MatGetSubMatrix(
          matrix->mat(), _restrict_solve_to_is, _restrict_solve_to_is_complement, PETSC_DECIDE,
...
void PetscLinearSolverM::set_petsc_solver_type() {
  ...
  switch (this->_solver_type) {
    ...
    default:
      std::cerr << "ERROR:  Unsupported PETSC Solver: " << this->_solver_type << std::endl
