...
// ===========================================================
void DenseMatrixBaseM::multiply(
    DenseMatrixBaseM& M1, const DenseMatrixBaseM& M2, const DenseMatrixBaseM& M3) {
  ...
  for (int k = 0; k < p_s; k++)
    for (int j = 0; j < n_s; j++)
      if (M3.el(k, j) != 0.)
        for (int i = 0; i < m_s; i++) M1.el(i, j) += M2.el(i, k) * M3.el(k, j);
}

// ============================================================
void DenseMatrixBaseM::condense(const int iv, const int jv, const double val, DenseVectorBaseM& rhs) {
...
void DenseMatrixBaseM::print_scientific(std::ostream& os) const {
  ...
  for (int i = 0; i < this->m(); i++) {
    for (int j = 0; j < this->n(); j++)
      os << std::setw(15) << std::scientific << std::setprecision(8) << this->el(i, j) << " ";
    os << std::endl;
  }
...
void DenseMatrixBaseM::print(std::ostream& os) const {
  for (int i = 0; i < this->m(); i++) {
    for (int j = 0; j < this->n(); j++) os << std::setw(8) << this->el(i, j) << " ";
    os << std::endl;
  }
  return;
}
