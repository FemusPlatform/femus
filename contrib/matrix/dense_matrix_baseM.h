...
class DenseMatrixBaseM {
 ...
 public:
  ...
  friend std::ostream& operator<<(std::ostream& os, const DenseMatrixBaseM& m) {
    m.print(os);
    return os;
  }
...
//  INLINE FUNCTIONS
// ===========================================================
inline void DenseMatrixBaseM::add(const double factor, const DenseMatrixBaseM& mat) {
  assert(this->m() == mat.m());
  assert(this->n() == mat.n());
  for (int j = 0; j < this->n(); j++)
    for (int i = 0; i < this->m(); i++) this->el(i, j) += factor * mat.el(i, j);
}
