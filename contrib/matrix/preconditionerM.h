...
class PreconditionerM :
 ...
 public:
  ...
  ///  Constructor
  PreconditionerM(const ParallelM::Communicator& comm  ///< parallel communicator
  );
  /// Destructor
  ...
  void set_matrix(SparseMatrixM& mat  ///< sparse matrix <-
  ) {                                 // If the matrix is changing then we (probably) need to reinitialize.
    _is_initialized = false;
    _matrix = &mat;
  }
  /// Sets the type of preconditioner to use.
  void set_type(const PreconditionerTypeM pct  ///< preconditioner type <-
...
/*----------------------- inline functions ----------------------------------*/
// =============================================
inline PreconditionerM::PreconditionerM(const ParallelM::Communicator& comm_in  ///< communicator <-
                                        )
    : ParallelObjectM(comm_in), _matrix(NULL), _preconditioner_type(ILU_PRECONDM), _is_initialized(false) {}
