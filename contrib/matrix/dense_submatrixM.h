...
class DenseSubMatrixM : public DenseMatrixBaseM {
 ...
 public:
   ...
   */
  DenseSubMatrixM(
      DenseMatrixM& parent, const int ioff = 0, const int joff = 0, const int m = 0, const int n = 0)
  ...
  DenseSubMatrixM(const DenseSubMatrixM& other_matrix)
      : DenseMatrixBaseM(other_matrix._m, other_matrix._n), _parent_matrix(other_matrix._parent_matrix) {
  ...
  virtual void zero() {
    for (int i = 0; i < this->m(); i++)
      for (int j = 0; j < this->n(); j++) _parent_matrix(i + this->i_off(), j + this->j_off()) = 0.;
  }
  // =====================================
  // Return functions
  // =====================================
  ...
  int i_off() const { return _i_off; }
  /// @returns the column offset into the parent matrix.
  int j_off() const { return _j_off; }
  ...
  void reposition(const int ioff, const int joff, const int m, const int n);
   ...
   */
  void condense(const int i, const int j, const double val, DenseSubVectorM& rhs) {
    this->parent().condense(this->i_off() + i, this->j_off() + j, val, rhs.parent());
  }
};
...
inline double DenseSubMatrixM::operator()(const int i, const int j) const {
  assert(i < this->m());
  assert(j < this->n());
  assert(i + this->i_off() < _parent_matrix.m());
  assert(j + this->j_off() < _parent_matrix.n());
...
inline void DenseSubMatrixM::reposition(const int ioff, const int joff, const int m, const int n) {
  _i_off = ioff;
  _j_off = joff;
  this->_m = m;
  this->_n = n;
