...
// ===================================
// ===================================
void LaspackVectorM::add_vector(const std::vector<Real>& v, const std::vector<unsigned int>& dof_indices) {
  assert(!v.empty());
  assert(v.size() == dof_indices.size());
  for (unsigned int i = 0; i < v.size(); i++) this->add(dof_indices[i], v[i]);
}
// ===================================
void LaspackVectorM::add_vector(const NumericVectorM& V, const std::vector<unsigned int>& dof_indices) {
  assert(V.size() == dof_indices.size());
  for (unsigned int i = 0; i < V.size(); i++) this->add(dof_indices[i], V(i));
}
// ================================
void LaspackVectorM::add_vector(const DenseVectorM& V, const std::vector<unsigned int>& dof_indices) {
  assert(V.size() == dof_indices.size());
  for (unsigned int i = 0; i < V.size(); i++) this->add(dof_indices[i], V(i));
}
// ==================================
void LaspackVectorM::add_vector(const NumericVectorM& vec_in, const SparseMatrixM& mat_in) {
  ...
  assert(mat != NULL);
  // += mat*vec
  AddAsgn_VV(&_vec, Mul_QV(const_cast<QMatrix*>(&mat->_QMat), const_cast<QVector*>(&vec->_vec)));
  return;
}

// ==================================
void LaspackVectorM::add_vector(const NumericVectorM& vec_in, const SparseMMatrixM& mat_in) {
  // Make sure the data passed in are really in Laspack types
  ...
  //     double sum=0.;
  //     for (uint j=0;j<M__GetLen(mat,irow);j++) sum
  //     +=M__GetVal(mat,irow,j)*V__GetCmp(vec,M__GetPos(mat,irow,j));
...
// =====================================================
void LaspackVectorM::resid(
    const NumericVectorM& rhs_in, const NumericVectorM& vec_in, const SparseMatrixM& mat_in) {
  // Make sure the data passed in are really in Laspack types
  ...
  for (uint irow = 1; irow <= n_row; irow++) {
    double sum = 0.;
    for (uint j = 0; j < Q__GetLen(mat, irow); j++)
      sum += Q__GetVal(mat, irow, j) * V__GetCmp(vec, Q__GetPos(mat, irow, j));
    V__SetCmp(&this->_vec, irow, V__GetCmp(rhs, irow) - sum);
  }
  return;
}
// ============================================================
void LaspackVectorM::matrix_mult(const NumericVectorM& vec_in, const SparseMMatrixM& mat_in) {
  // Make sure the data passed in are really in Laspack types
  const LaspackVectorM* Lvec = static_cast<const LaspackVectorM*>(&vec_in);
  const LaspackMMatrixM* Lmat = static_cast<const LaspackMMatrixM*>(&mat_in);
  assert(Lvec != NULL);
  ...
  for (uint irow = 1; irow <= n_row; irow++) {
    double sum = 0.;
    for (uint j = 0; j < M__GetLen(mat, irow); j++)
      sum += M__GetVal(mat, irow, j) * V__GetCmp(vec, M__GetPos(mat, irow, j));
...
// =====================================================
// =====================================================
void LaspackVectorM::insert(const std::vector<Real>& v, const std::vector<unsigned int>& dof_indices) {
  assert(!v.empty());
  assert(v.size() == dof_indices.size());
...
// ====================================================
void LaspackVectorM::insert(const NumericVectorM& V, const std::vector<unsigned int>& dof_indices) {
  assert(V.size() == dof_indices.size());
  for (unsigned int i = 0; i < V.size(); i++) V__SetCmp(&this->_vec, dof_indices[i] + 1, V(i));
  // this->set(dof_indices[i], V(i));
}
// ===================================================
void LaspackVectorM::insert(const DenseVectorM& V, const std::vector<unsigned int>& dof_indices) {
  assert(V.size() == dof_indices.size());
...
// ===================================================
void LaspackVectorM::insert(const DenseSubVectorM& V, const std::vector<unsigned int>& dof_indices) {
  assert(V.size() == dof_indices.size());
  for (unsigned int i = 0; i < V.size(); i++) V__SetCmp(&_vec, dof_indices[i] + 1, V(i));
  // this->set(dof_indices[i], V(i));
}
...
// ==================================
void LaspackVectorM::pointwise_mult(const NumericVectorM& /*vec1*/, const NumericVectorM& /*vec2*/) {
...
Real LaspackVectorM::dot(const NumericVectorM& V) const {
  ...
  for (unsigned int i = 1; i <= V.size(); i++)
    sum += V__GetCmp(&_vec, i) * V__GetCmp(const_cast<QVector*>(&(v->_vec)), i);
  return sum;
...
LaspackVectorM& LaspackVectorM::operator=(const LaspackVectorM& v) {
  ...
  if (v.size() != 0)
    for (unsigned int i = 1; i <= V_GetDim(&_vec); ++i)
...
NumericVectorM& LaspackVectorM::operator=(const std::vector<Real>& v) {
  ...
  if (this->size() == v.size())
    for (uint i = 0; i < v.size(); i++) this->set(i, v[i]);
  else
    abort();
  return *this;
}
...
// =================================================
void LaspackVectorM::localize(NumericVectorM& v_local_in, const std::vector<unsigned int>& send_list) const {
  // Make sure the NumericVector passed in is really a LaspackVectorM
  LaspackVectorM* v_local = static_cast<LaspackVectorM*>(&v_local_in);
  assert(v_local != NULL);
...
// ==================================
void LaspackVectorM::localize_to_one(std::vector<Real>& v_local, const unsigned int pid) const {
  assert(pid == 0);
...
void LaspackVectorM::print_hdf5(std::string file) const {
  ...
  // Value storage
  double* val = new double[ndim];
  for (uint i = 0; i < ndim; i++) val[i] = V__GetCmp(&_vec, i + 1);
  ...
  hid_t dataset =
      H5Dcreate(fileP, name.str().c_str(), H5T_NATIVE_INT, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  dataset = H5Dcreate(
      fileP, name.str().c_str(), H5T_NATIVE_DOUBLE, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
