...
//-----------------------------------------------------------------------
// PetscMatrix members
void PetscMatrixM::init(const  int m,
                        const  int n,
                        const  int m_l,
                        const  int n_l,
                        const  int /*nnz*/,
...
void PetscMatrixM::zero() {
  ...
  int ierr=0;  ierr = MatZeroEntries(_mat);
  CHKERRABORT(MPI_COMM_WORLD,ierr);
}
...
void PetscMatrixM::zero_rows(std::vector<int> & rows, double diag_value) {
  assert(this->initialized());  semiparallel_onlyM(); int ierr=0;
...
double PetscMatrixM::l1_norm() const {
  assert(this->initialized());  semiparallel_onlyM();
  int ierr=0;  PetscReal petsc_value;  double value;
  assert(this->closed());  ierr = MatNorm(_mat, NORM_1, &petsc_value);
  CHKERRABORT(MPI_COMM_WORLD,ierr);
  value = static_cast<double>(petsc_value);  return value;
}
...
// ===================================================
void PetscMatrixM::print_personal(
...
void PetscMatrixM::add_matrix(const DenseMatrixM& dm,
  ...
  assert(this->initialized());
  const  int m = dm.m(); assert((int)rows.size() == m);
  const  int n = dm.n(); assert((int)cols.size() == n);
...
//---  
  ...
  ierr = ISDestroy(&isrow);    CHKERRABORT(MPI_COMM_WORLD,ierr);
  ierr = ISDestroy(&iscol);    CHKERRABORT(MPI_COMM_WORLD,ierr);
...
// ===========================================================
void PetscMatrixM::get_transpose(
  SparseMatrixM& dest  // output transpose matrix
) const {
