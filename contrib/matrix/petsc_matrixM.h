...
class PetscMatrixM : public SparseMatrixM {
 ...
 public:
  ...
  PetscMatrixM(const ParallelM::Communicator& comm) : SparseMatrixM(comm), _destroy_mat_on_exit(true) {}
  /// Constructor II.
  PetscMatrixM(Mat m, const ParallelM::Communicator& comm)
      : SparseMatrixM(comm), _destroy_mat_on_exit(false) {
    this->_mat = m;
    this->_is_initialized = true; /* this->_communicator=comm;*/
  }
  ...
  void zero_rows(  ///< set  rows to zero
      std::vector<int>& rows, double diag_value = 0.0);
  ...
  void update_sparsity_pattern(
      int m, int n, int m_l, int n_l,  ///<   sparsity patter update (petsc)
  ...
  void print_personal(  ///< print
      const std::string name = "NULL") const;
  ...
  virtual void get_diagonal(  ///< Copies the diagonal part of the matrix
      NumericVectorM& dest) const;
  virtual void get_transpose(  ///< Transpose Matrix.
      SparseMatrixM& dest) const;
...
inline void PetscMatrixM::set(const int i, const int j, const double value) {
  assert(this->initialized());
...
inline void PetscMatrixM::add(
    const double a_in,   // double constant
...
// ========================================================
inline int PetscMatrixM::MatGetRowM(
    const int i_val,  // index i
...
// =========================================
inline void PetscMatrixM::swap(PetscMatrixM& m  // pointer to swap
) {                                             // =========================================
