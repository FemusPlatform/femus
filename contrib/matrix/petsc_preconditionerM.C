...
void PetscPreconditionerM::set_petsc_preconditioner_type(
  ...
  switch (preconditioner_type) {
    ...
    case LU_PRECONDM: {
      ...
      //    ierr = PCFactorSetMatSolverPackage(pc,MATSOLVERSUPERLU_DIST);  CHKERRABORT(MPI_COMM_WORLD,ierr);
      ...
      //    //here we set the MUMPS parallel direct solver package break;
      //
      //
      //   case MCC_PRECONDM:
    ...
    default:
      std::cerr << "ERROR:  Unsupported PETSC Preconditioner: " << preconditioner_type << std::endl
                << "Continuing with PETSC defaults" << std::endl;
  }

  // Let the commandline override stuff
  if (preconditioner_type != AMG_PRECONDM /*&& preconditioner_type != MG_PRECONDM*/)
    PCSetFromOptions(pc);  //!!!!!!
}
