...
class SparseMMatrixM :
 ...
 public:
  ...
  virtual void add(
      const double /*c*/, SparseMMatrixM& /*B*/  ///< dense matrix
      ) = 0;
  ...
  virtual void read_len_hdf5(
      const std::string namefile, const std::string mode, int len_row[],
      int leng_off_row[]);  ///< Read lengths
  virtual void read_pos_hdf5(
      const std::string namefile, const std::string mode, int pos[], double val[]);  ///< Read pos
  virtual int read_dim_hdf5(
      const std::string namefile, const std::string mode, int dim[]);  ///< Read dimensions

  // Submatrices ------------------------------------------------------
  /// SubMatrix creation
  virtual void create_submatrix(
      SparseMMatrixM& submatrix, const std::vector<int>& rows, const std::vector<int>& cols) const {
 ...
 protected:
  ...
  virtual void _get_submatrix(
      SparseMMatrixM&, const std::vector<int>&, const std::vector<int>&, const bool) const {
...
inline SparseMMatrixM::SparseMMatrixM(const ParallelM::Communicator& comm_in  ///< parallel communicator
                                      )
    : ParallelObjectM(comm_in), _is_initialized(false) {}
