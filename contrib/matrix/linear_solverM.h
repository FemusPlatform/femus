...
class LinearSolverM :
 ...
 public:
  ...
  LinearSolverM(const ParallelM::Communicator& comm_in  ///< parallel communicator <-
  );
  /// Destructor.
  ...
  static std::unique_ptr<LinearSolverM> build(
      const ParallelM::Communicator& comm_in, const SolverPackageM solver_package = LSOLVER);
  ...
  virtual std::pair<int, double> adjoint_solve(
      SparseMatrixM&,   // System Matrix
  ...
  std::pair<int, double> solve(
      SparseMatrixM& matrix, SparseMatrixM* precond_matrix,
      NumericVectorM&,  // Solution vector
      NumericVectorM&,  // RHS vector
      const double,     // Stopping tolerance
      const int,
...
// =============================================
inline LinearSolverM::LinearSolverM(const ParallelM::Communicator& comm_in)
...
// ==============================
inline bool LinearSolverM::get_same_preconditioner() { return same_preconditioner; }

// =====================================================
inline std::pair<int, double> LinearSolverM::solve(
    SparseMatrixM& mat, SparseMatrixM* pc_mat, NumericVectorM& sol, NumericVectorM& rhs, const double tol,
    const int n_iter, const int clearing) {
  if (pc_mat)
    return this->solve(mat, *pc_mat, sol, rhs, tol, n_iter, clearing);
  else
    return this->solve(mat, sol, rhs, tol, n_iter, clearing);
}
