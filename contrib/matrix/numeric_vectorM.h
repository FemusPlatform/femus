...
class NumericVectorM :
 ...
 public:
  ...
  static std::unique_ptr<NumericVectorM> build(
      const ParallelM::Communicator& comm_in,        ///< parallel communicator <-
  ...
  virtual void init(const int, const int, const bool = false, const ParallelTypeM = AUTOMATICM) = 0;

  /// call init with n_local = N,
  virtual void init(const int, const bool = false, const ParallelTypeM = AUTOMATICM) = 0;
  ...
  virtual void init(const NumericVectorM& other, const bool fast = false) = 0;
  ...
  virtual void insert(const std::vector<double>& v, const std::vector<int>& dof_indices) = 0;
  /// \f$U=V\f$, and specify WHERE to insert
  virtual void insert(const NumericVectorM& V, const std::vector<int>& dof_indices) = 0;
  /// \f$ U=V \f$ insert
  virtual void insert(const DenseVectorM& V, const std::vector<int>& dof_indices) = 0;
  /// \f$ U=V \f$ and specify WHERE to insert it
  virtual void insert(const DenseSubVectorM& V, const std::vector<int>& dof_indices) = 0;
  // =====================================
  // RETURN FUNCTIONS
  // =====================================
  ...
  virtual double el(const int i) const { return (*this)(i); }
  ...
  virtual void add_vector(const std::vector<double>& v, const std::vector<int>& dof_indices) = 0;
  /// \f$U+=V\f$, where U and V are type
  virtual void add_vector(const NumericVectorM& V, const std::vector<int>& dof_indices) = 0;
  ...
  virtual void resid(
      const NumericVectorM& /*rhs_in*/, const NumericVectorM& /*A*/, const SparseMatrixM& /*V*/) = 0;
  ...
  //
  /// \f$ U+=V \f$ where U and V are type
  virtual void add_vector(const DenseVectorM& V, const std::vector<int>& dof_indices) = 0;
  ...
  virtual void add_vector_transpose(const NumericVectorM&, const SparseMatrixM&) = 0;
  ...
  virtual void localize(NumericVectorM& v_local, const std::vector<int>& send_list) const = 0;
  /// Updates a local vector with selected values from neighboring
  virtual void localize(
      const int first_local_idx, const int last_local_idx, const std::vector<int>& send_list) = 0;
  /// Creates a local copy of the global vector
  virtual void localize_to_one(std::vector<double>& v_local, const int proc_id = 0) const = 0;
  /// @returns \p -1 when \p this is equivalent to \p other_vector,
  virtual int compare(const NumericVectorM& other_vector, const double threshold = TOLERANCEM) const;
  /// @returns \p -1 when \p this is equivalent to \p other_vector,
  ...
  virtual int local_relative_compare(
      const NumericVectorM& other_vector, const double threshold = TOLERANCEM) const;
  ...
  virtual void pointwise_mult(const NumericVectorM& vec1, const NumericVectorM& vec2) = 0;
...
// ==============================================
inline NumericVectorM::NumericVectorM(const ParallelM::Communicator& comm_in, const ParallelTypeM type)
    : ParallelObjectM(comm_in), _is_closed(false), _is_initialized(false), _type(type) {}
// ==============================================
inline NumericVectorM::NumericVectorM(
    const ParallelM::Communicator& comm_in, const int /*n*/, const ParallelTypeM type)
    : ParallelObjectM(comm_in), _is_closed(false), _is_initialized(false), _type(type) {
...
// ==============================================
inline NumericVectorM::NumericVectorM(
    const ParallelM::Communicator& comm_in, const int /*n*/, const int /*n_local*/, const ParallelTypeM type)
    : ParallelObjectM(comm_in), _is_closed(false), _is_initialized(false), _type(type) {
  std::cout << "Abstract base class! ";
  exit(0);  // Abstract base class!
...
// ==============================================
inline NumericVectorM::NumericVectorM(
    const ParallelM::Communicator& comm_in, const int /*n*/, const int /*n_local*/,
    const std::vector<int>& /*ghost*/, const ParallelTypeM type)
    : ParallelObjectM(comm_in), _is_closed(false), _is_initialized(false), _type(type) {
  std::cout << "Abstract base class! ";
  exit(0);  // Abstract base class!
...
// ==============================================
inline void NumericVectorM::get(const std::vector<int>& index, std::vector<double>& values) const {
...
inline void NumericVectorM::print(std::ostream& os) const {
  ...
  os << "Size\tglobal =  " << this->size() << "\t\tlocal =  " << this->local_size() << std::endl;
  os << "#\tValue" << std::endl;
