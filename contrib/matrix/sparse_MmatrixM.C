...
// ===========================================================
void SparseMMatrixM::vector_mult (
  NumericVectorM& dest,
  const NumericVectorM& arg
) const{
...
void SparseMMatrixM::vector_mult_add (
  NumericVectorM& dest, 
  const NumericVectorM& arg
) const{
...
// -------------------------------------------------------
  ...
  hid_t status=H5Dread(dataset,H5T_NATIVE_INT,H5S_ALL,H5S_ALL,H5P_DEFAULT,len_row); assert(status==0);
...
void SparseMMatrixM::read_pos_hdf5(
  ...
  hid_t  file_id = H5Fopen(namefile.c_str(),H5F_ACC_RDWR
