...
class PetscLinearSolverM : public LinearSolverM {  // ============================================
 ...
 public:
  ...
  //     SparseMatrixM& precond_in, NumericVectorM& solution_in, NumericVectorM& rhs_in,  NumericVectorM&
  //     res_in, SparseMMatrixM& prol_in, SparseMMatrixM& rest_in, const int nlevels, const int nup_its, const
  //     int ndown_its,
  ...
  virtual void restrict_solve_to(
      const std::vector<int>* const dofs, const SubsetSolveModeM subset_solve_mode = SUBSET_ZERO);
  ...
  std::pair<int, double> solve(
      ...
      const int m_its, const int clearing) {
  ...
  std::pair<int, double> solve(
      SparseMatrixM& /*matrix*/, SparseMatrixM& /*preconditioner*/, NumericVectorM& /*solution*/,
      NumericVectorM& /*rhs*/, const double /*tol*/, const int /*Niter*/, const int clearing);
...
/*----------------------- functions ----------------------------------*/
// =================================================
inline PetscLinearSolverM::PetscLinearSolverM(const ParallelM::Communicator& comm)
    : LinearSolverM(comm),
...
// =============================
inline size_t PetscLinearSolverM::_restrict_solve_to_is_local_size(void) const {
...
// // ==========================================
inline void PetscLinearSolverM::_create_complement_is(const NumericVectorM& vec_in) {
  assert(_restrict_solve_to_is);
  if (_restrict_solve_to_is_complement == NULL) {
    int ierr = ISComplement(
        _restrict_solve_to_is, vec_in.first_local_index(), vec_in.last_local_index(),
