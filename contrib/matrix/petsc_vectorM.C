...
// ===============================================================
void PetscVectorM::add_vector(const std::vector<double>& v, const std::vector<int>& dof_indices) {
  this->_restore_array();
...
// ===========================================================
void PetscVectorM::add_vector(const NumericVectorM& V, const std::vector<int>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  for (int i = 0; i < (int)V.size(); i++) this->add(dof_indices[i], V(i));
}

// =========================================================
void PetscVectorM::add_vector(const NumericVectorM& V_in, const SparseMatrixM& A_in) {
  this->_restore_array();
...
// =========================================================
void PetscVectorM::add_vector(const NumericVectorM& V_in, const SparseMMatrixM& A_in) {
  this->_restore_array();
  // Make sure the data passed in are really of Petsc types
  const PetscVectorM* V = dynamic_cast<const PetscVectorM*>(&V_in);
...
// ====================================================
void PetscVectorM::add_vector(const DenseVectorM& V, const std::vector<int>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  for (int i = 0; i < (int)V.size(); i++) this->add(dof_indices[i], V(i));
}
// ==========================================
void PetscVectorM::add_vector_transpose(const NumericVectorM& V_in, const SparseMatrixM& A_in) {
  this->_restore_array();
  // Make sure the data passed in are really of Petsc types
  const PetscVectorM* V = dynamic_cast<const PetscVectorM*>(&V_in);
  const PetscMatrixM* A = dynamic_cast<const PetscMatrixM*>(&A_in);
...
// =========================================================
void PetscVectorM::resid(const NumericVectorM& b_in, const NumericVectorM& x_in, const SparseMatrixM& A_in) {
  this->_restore_array();
  // Make sure the data passed in are really of Petsc types
  ...
  ierr = MatMult(const_cast<PetscMatrixM*>(A)->mat(), x->_vec, _vec);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
...
// ==============================================
void PetscVectorM::add(const NumericVectorM& v) { this->add(1., v); }
...
// ========================================================
void PetscVectorM::insert(const std::vector<double>& v, const std::vector<int>& dof_indices) {
  assert(v.size() == dof_indices.size());
  for (int i = 0; i < (int)v.size(); i++) this->set(dof_indices[i], v[i]);
}

// =======================================================
void PetscVectorM::insert(const NumericVectorM& V, const std::vector<int>& dof_indices) {
  assert((int)V.size() == (int)dof_indices.size());
  for (int i = 0; i < (int)V.size(); i++) this->set(dof_indices[i], V(i));
}

// ========================================================
void PetscVectorM::insert(const DenseVectorM& V, const std::vector<int>& dof_indices) {
  assert(V.size() == dof_indices.size());
  for (int i = 0; i < (int)V.size(); i++) this->set(dof_indices[i], V(i));
}

// =========================================================
void PetscVectorM::insert(const DenseSubVectorM& V, const std::vector<int>& dof_indices) {
  assert(V.size() == dof_indices.size());
  for (int i = 0; i < (int)V.size(); i++) this->set(dof_indices[i], V(i));
}
...
void PetscVectorM::localize(NumericVectorM& v_local_in  //  local vector
                            ) const {                   // ===================================================
...
void PetscVectorM::localize(
  ...
  if (v_local_in.type() == GHOSTEDM && this->type() == PARALLELM) {
    v_local_in = *this;
    return;
  }
  this->_restore_array();
  ...
  // Create the index set & scatter object
  if (idx.empty())
    ierr = ISCreateGeneral(this->comm().get(), n_sl + this->local_size(), PETSC_NULL, PETSC_USE_POINTER, &is);
  else
    ierr = ISCreateGeneral(this->comm().get(), n_sl + this->local_size(), &idx[0], PETSC_USE_POINTER, &is);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  ierr = VecScatterCreate(_vec, is, v_local->_vec, is, &scatter);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
...
void PetscVectorM::localize(
...

  {
    ...
    // Create the index set & scatter object
    ierr =
        ISCreateGeneral(this->comm().get(), local_size, local_size ? &idx[0] : NULL, PETSC_USE_POINTER, &is);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
    ierr = VecScatterCreate(_vec, is, parallel_vec._vec, is, &scatter);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
    // Perform the scatter
#if PETSC_VERSION_LESS_THAN(2, 3, 3)
    ierr = VecScatterBegin(_vec, parallel_vec._vec, INSERT_VALUES, SCATTER_FORWARD, scatter);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
    ierr = VecScatterEnd(_vec, parallel_vec._vec, INSERT_VALUES, SCATTER_FORWARD, scatter);
    CHKERRABORT(MPI_COMM_WORLD, ierr);
#else  // API argument order change in PETSc 2.3.3
...
//  ===========================================================
void PetscVectorM::localize_to_one(std::vector<double>& v_local, const int pid) const {
  this->_restore_array();
  int ierr = 0;
  PetscScalar* values;
  const int n = size();
...
// =======================================================
void PetscVectorM::pointwise_mult(const NumericVectorM& vec1, const NumericVectorM& vec2) {
  this->_restore_array();
  ...
  if (this->type() != GHOSTEDM) {
    ierr = VecPointwiseMult(
        this->vec(), const_cast<PetscVectorM*>(vec1_petsc)->vec(),
...
void PetscVectorM::print_hdf5(std::string file) const {
  ...
  hid_t dataset = H5Dcreate(fileP, name.str().c_str(), H5T_NATIVE_INT, dataspace, H5P_DEFAULT);
  ...
  dataset = H5Dcreate(fileP, name.str().c_str(), H5T_NATIVE_DOUBLE, dataspace, H5P_DEFAULT);
...
void PetscVectorM::print_hdf5(std::string file) const {
  ...
  hid_t dataset = H5Dcreate(
      fileP, name.str().c_str(), H5T_NATIVE_INT, dataspace, H5P_DEFAULT
  ...
  dataspace = H5Screate_simple(2, dimsf, NULL);
  dataset = H5Dcreate(
      fileP, name.str().c_str(), H5T_NATIVE_DOUBLE, dataspace, H5P_DEFAULT
#if HDF5_VERSIONM != 1808
      ,
      H5P_DEFAULT, H5P_DEFAULT
#endif
...
void PetscVectorM::create_subvector(
  ...
  ierr = ISCreateGeneral(this->comm().get(), rows.size(), (int*)&rows[0], PETSC_USE_POINTER, &parent_is);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  ierr = ISCreateGeneral(this->comm().get(), rows.size(), (int*)&idx[0], PETSC_USE_POINTER, &subvector_is);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  // Construct the scatter object
  ierr = VecScatterCreate(this->_vec, parent_is, petsc_subvector->_vec, subvector_is, &scatter);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  // Actually perform the scatter
  ierr = VecScatterBegin(scatter, this->_vec, petsc_subvector->_vec, INSERT_VALUES, SCATTER_FORWARD);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  ierr = VecScatterEnd(scatter, this->_vec, petsc_subvector->_vec, INSERT_VALUES, SCATTER_FORWARD);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  // Clean up
