...
class LaspackMatrixM : public SparseMatrixM {
 ...
 public:
  ...
  void init(
      const uint m, const uint n, const uint m_l, const uint n_l, const unsigned int nnz = 0,
  ...
  friend std::ostream& operator<<(
      std::ostream& os, const SparseMatrixM& m);  /// Same as the print method above
...
// ===========================================
inline void LaspackMatrixM::add_matrix(const DenseMatrixM& dm, const std::vector<uint>& dof_indices) {
  this->add_matrix(dm, dof_indices, dof_indices);
}
// ===========================================
inline void LaspackMatrixM::add_matrix(
    const DenseMatrixM& dm, const std::vector<uint>& rows, const std::vector<uint>& cols) {
  assert(this->initialized());
  assert(dm.m() == rows.size());
  assert(dm.n() == cols.size());
  for (unsigned int i = 0; i < rows.size(); i++) {
    int irow = rows[i] + 1;
    for (unsigned int k = 0; k < Q__GetLen(&_QMat, irow); k++)
      _uncomp_row[Q__GetPos(&_QMat, irow, k) - 1] = k;
...
inline unsigned int LaspackMatrixM::pos(const uint i, const uint j) const {
  ...
  for (uint k = 0; k < length; k++)
    if (Q__GetPos(&_QMat, i + 1, k) == j) return k;
