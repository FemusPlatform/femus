...
class Histogram : public ParallelObjectM {
 ...
 public:
  // Constructor
  explicit Histogram(const ParallelM::Communicator& comm, const std::vector<KeyType>& d);
...
inline const std::vector<IdxType>& Histogram<KeyType, IdxType>::get_histogram() const {
  return hist;
}

template <typename KeyType, typename IdxType>
inline IdxType Histogram<KeyType, IdxType>::n_bins() const {
  if (bin_iters.empty()) return 0;

  return std::static_cast<IdxType>(bin_iters.size() - 1);
}

template <typename KeyType, typename IdxType>
inline IdxType Histogram<KeyType, IdxType>::local_bin_size(const IdxType bin) const {
...
template <typename KeyType, typename IdxType>
inline IdxType Histogram<KeyType, IdxType>::global_bin_size(const IdxType bin) const {
...
template <typename KeyType, typename IdxType>
inline double Histogram<KeyType, IdxType>::lower_bound(const IdxType bin) const {
...
template <typename KeyType, typename IdxType>
inline double Histogram<KeyType, IdxType>::upper_bound(const IdxType bin) const {
  assert((bin + 1) < bin_bounds.size());
