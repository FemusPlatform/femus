...
#include "auto_ptrM.h"
#include "elemM.h"
#include "location_mapsM.h"
#include "mesh_baseM.h"
#include "parallelM.h"
 ...
 *                  std::vector<sync::datum>& data)
 * The user must define ParallelM::StandardType<sync::datum> if
...
void sync_dofobject_data_by_xyz(
    const Communicator& communicator, const Iterator& range_begin, const Iterator& range_end,
    LocationMap<DofObjType>* location_map, SyncFunctor& sync);
...
void sync_dofobject_data_by_id(
    const Communicator& communicator, const Iterator& range_begin, const Iterator& range_end,
    SyncFunctor& sync);
 ...
 */
template <typename Iterator, typename SyncFunctor>
void sync_element_data_by_parent_id(
    MeshBase& mesh, const Iterator& range_begin, const Iterator& range_end, SyncFunctor& sync);
...
void sync_dofobject_data_by_xyz(
  ...
  assert(!need_map_update);
#endif

  // Count the objectss to ask each processor about
  std::vector<dof_id_type> ghost_objects_from_proc(communicator.size(), 0);

  for (Iterator it = range_begin; it != range_end; ++it) {
    DofObjType* obj = *it;
    assert(obj);
    processor_id_type obj_procid = obj->processor_id();
    if (obj_procid != DofObject::invalid_processor_id) ghost_objects_from_proc[obj_procid]++;
  }

  // Request sets to send to each processor
  std::vector<std::vector<Real> > requested_objs_x(communicator.size()),
      requested_objs_y(communicator.size()), requested_objs_z(communicator.size());
  // Corresponding ids to keep track of
  std::vector<std::vector<dof_id_type> > requested_objs_id(communicator.size());
  ...
  for (Iterator it = range_begin; it != range_end; ++it) {
    DofObjType* obj = *it;
    processor_id_type obj_procid = obj->processor_id();
    if (obj_procid == communicator.rank() || obj_procid == DofObject::invalid_processor_id) continue;
  ...
  for (processor_id_type p = 1; p != communicator.size(); ++p) {
    ...
    const processor_id_type procup =
        std::static_cast<processor_id_type>((communicator.rank() + p) % communicator.size());
    const processor_id_type procdown = std::static_cast<processor_id_type>(
        (communicator.size() + communicator.rank() - p) % communicator.size());
    std::vector<Real> request_to_fill_x, request_to_fill_y, request_to_fill_z;
    communicator.send_receive(procup, requested_objs_x[procup], procdown, request_to_fill_x);
    communicator.send_receive(procup, requested_objs_y[procup], procdown, request_to_fill_y);
    communicator.send_receive(procup, requested_objs_z[procup], procdown, request_to_fill_z);
    ...
    for (std::size_t i = 0; i != request_to_fill_x.size(); ++i) {
      Point pt(request_to_fill_x[i], request_to_fill_y[i], request_to_fill_z[i]);
    ...
    communicator.send_receive(procdown, data, procup, received_data);
    assert_equal_to(requested_objs_x[procup].size(), received_data.size());
...
void sync_dofobject_data_by_id(
  ...
  // space for each.
  for (processor_id_type p = 0; p != communicator.size(); ++p)
    if (p != communicator.rank()) { requested_objs_id[p].reserve(ghost_objects_from_proc[p]); }
  for (Iterator it = range_begin; it != range_end; ++it) {
    DofObject* obj = *it;
    processor_id_type obj_procid = obj->processor_id();
    if (obj_procid == communicator.rank() || obj_procid == DofObject::invalid_processor_id) continue;
  ...
  for (processor_id_type p = 1; p != communicator.size(); ++p) {
    ...
    communicator.send_receive(procup, requested_objs_id[procup], procdown, request_to_fill_id);
    ...
    std::vector<typename SyncFunctor::datum> received_data;
    communicator.send_receive(procdown, data, procup, received_data);
    assert_equal_to(requested_objs_id[procup].size(), received_data.size());
...
void sync_element_data_by_parent_id(
  ...
  std::vector<std::vector<unsigned char> > requested_objs_child_num(communicator.size());
  ...
  for (Iterator it = range_begin; it != range_end; ++it) {
    ...
    if (!parent || !elem->active()) continue;
    ...
    requested_objs_child_num[obj_procid].push_back(
        std::static_cast<unsigned char>(parent->which_child_am_i(elem)));
  }
  ...
  for (processor_id_type p = 1; p != communicator.size(); ++p) {
    ...
    communicator.send_receive(procup, requested_objs_parent_id[procup], procdown, request_to_fill_parent_id);
    communicator.send_receive(procup, requested_objs_child_num[procup], procdown, request_to_fill_child_num);
...
#else
template <typename Iterator, typename SyncFunctor>
void sync_element_data_by_parent_id(MeshBase&, const Iterator&, const Iterator&, SyncFunctor&) {}
#endif  // LIBMESH_ENABLE_AMR

}  // namespace ParallelM

// This struct can be created and passed to the
// ParallelM::sync_dofobject_data_by_id() function.
struct SyncNodalPositions {
  ...
  explicit SyncNodalPositions(MeshBase& m);

  // The datum typedef is required of this functor, so that the
  // ParallelM::sync_dofobject_data_by_id() function can create e.g.
  // std::vector<datum>.
  typedef Point datum;
