  ...
  for (processor_id_type p=1; p != communicator.size(); ++p)
    {
      ...
      const processor_id_type procup =
        std::static_cast<processor_id_type>
      ...
      assert_equal_to (requested_objs_x[procup].size(),
                               received_data.size());
  ...
  for (processor_id_type p=1; p != communicator.size(); ++p)
    {
      ...
      communicator.send_receive(procdown, data,
                             procup, received_data);
      assert_equal_to (requested_objs_id[procup].size(),
                               received_data.size());
