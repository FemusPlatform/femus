...
#define parallel_onlyM()                                    \
  do {                                                      \
    assert(CommWorld.verify(std::string(__FILE__).size())); \
    assert(CommWorld.verify(std::string(__FILE__)));        \
    assert(CommWorld.verify(__LINE__));                     \
  } while (0)
#else
#define parallel_onlyM() ((void)0)
#endif

#undef parallel_onlyM
#ifndef NDEBUG
#define parallel_onlyM(comm_obj) ((void)0)
/* // // #define parallel_onlyM(comm_obj) do {                            \
// //     assert((comm_obj).verify(std::string(__FILE__).size()));    \
// //     assert((comm_obj).verify(std::string(__FILE__)));           \
// //     assert((comm_obj).verify(__LINE__)); } while (0)  */
#else
#define parallel_onlyM(comm_obj) ((void)0)
#endif
...
#undef parallel_onlyM_on
#ifndef NDEBUG
#define parallel_onlyM_on(comm_arg)                                   \
  do {                                                                \
    deprecated();                                                     \
    assert(CommWorld.verify(std::string(__FILE__).size(), comm_arg)); \
    assert(CommWorld.verify(std::string(__FILE__), comm_arg));        \
    assert(CommWorld.verify(__LINE__), comm_arg);                     \
  } while (0)
#else
#define parallel_onlyM_on(comm_arg) ((void)0)
#endif

#undef parallel_onlyM_on
#ifndef NDEBUG
#define parallel_onlyM_on(comm_obj, comm_arg)                        \
  do {                                                               \
    assert(comm_obj.verify(std::string(__FILE__).size(), comm_arg)); \
    assert(comm_obj.verify(std::string(__FILE__), comm_arg));        \
    assert(comm_obj.verify(__LINE__), comm_arg);                     \
  } while (0)
#else
#define parallel_onlyM_on(comm_obj, comm_arg) ((void)0)
#endif
...
typedef MPI_Datatype data_type;  // Data types for communication
typedef MPI_Request request;     // Request object for non-blocking I/O
typedef MPI_Status status;       // Status object for querying messages
typedef MPI_Comm communicator;   // Communicator object for talking with subsets of processors
...
const unsigned int any_source = static_cast<unsigned int>(MPI_ANY_SOURCE);
...
class MessageTag {
 public:
   ...
   */
  explicit MessageTag(int tagvalue = invalid_tag) : _tagvalue(tagvalue), _comm(NULL) {}
 ...
 private:
  ...
  MessageTag(int tagvalue, const Communicator* comm) : _tagvalue(tagvalue), _comm(comm) {}
...
//-------------------------------------------------------------------
// ======================================================================================
//  Structure false type  A C++03-compatible replacement for std::false_type
// ======================================================================================
...
class StandardType : public DataType {
  ...
  static_assert(
      ...
      "Only specializations of StandardType may be used, did you forget to include a header file (e.g. "
      "parallel_algebra.h)?");
#endif
...
// ======================================================================================
// Encapsulates the MPI_Status struc
// ======================================================================================
...
// ======================================================================================
//  Encapsulates the MPI_Request (changed)
// ======================================================================================
class Request {
 private:
  request _request;
  ...
  // multiple requests into a single Request new 2016
 ...
 * array.
 *
 * ParallelM::pack() has no default implementation, and must be
 ...
 *
 * ParallelM::packable_size() has no default implementation, and must
 ...
 *
 * ParallelM::packed_size() has no default implementation, and must
 * be specialized for each class which is to be communicated via
 * packed ranges.
 *
 ...
 * data array.
 *
 * ParallelM::unpack() has no default implementation, and must be
 * specialized for each class which is to be communicated via packed
 * ranges.
 */
...
inline void unpack_range(const typename std::vector<buffertype>& buffer, Context* context, OutputIter out);
...
inline Iter pack_range(
    const Context* context, Iter range_begin, const Iter range_end, typename std::vector<buffertype>& buffer);
...
inline std::size_t packed_range_size(const Context* context, Iter range_begin, const Iter range_end);
...
class Communicator {
 ...
 private:
  ...
  // mutable used_tag_values - not thread-safe, but then ParallelM::
 ...
 public:
  ...
  template <typename T>
  void send(
      const unsigned int dest_processor_id, T& buf, const DataType& type,
  ...
  template <typename T>
  void send(
      const unsigned int dest_processor_id, T& buf, const DataType& type, Request& req,
      const MessageTag& tag = no_tag) const;
  ...
  template <typename T>
  void receive(
      const unsigned int dest_processor_id, T& buf, Request& req, const MessageTag& tag = any_tag) const;
   ...
   *
   * void ParallelM::pack(const T*, vector<int>& data, const Context*)
   * is used to serialize type T onto the end of a data vector.
   *
   * unsigned int ParallelM::packable_size(const T*, const Context*) is
   * used to allow data vectors to reserve memory, and for additional
   * error checking
   */
  template <typename Context, typename Iter>
  void send_packed_range(
  ...
  void send_packed_range(
      const unsigned int dest_processor_id, const Context* context, Iter range_begin, const Iter range_end,
      Request& req, const MessageTag& tag = no_tag) const;
   ...
   * allows the receive to be dealt with solely by ParallelM::unpack(),
  ...
  void send_receive(
      ...
      const MessageTag& send_tag = no_tag, const MessageTag& recv_tag = any_tag) const;
   ...
   *
   * void ParallelM::pack(const T1*, vector<int>& data, const Context1*)
   ...
   */
  template <typename T>
  inline void gather(const unsigned int root_id, T send, std::vector<T>& recv) const;
   ...
   */
  template <typename T>
  inline void gather(const unsigned int root_id, std::vector<T>& r) const;
   ...
   */
  template <typename T>
  inline void allgather(T send, std::vector<T>& recv) const;
   ...
   */
  template <typename T>
  inline void allgather(std::vector<T>& r, const bool identical_buffer_sizes = false) const;
  ...
  inline void broadcast_packed_range(
      const Context* context1, Iter range_begin, const Iter range_end, OutputContext* context2,
      OutputIter out, const unsigned int root_id = 0) const;
   ...
   * the redundancy.
   */
#include "parallel_communicator_specializations_M"
...
class FakeCommunicator {
  operator Communicator&() {
    std::cout << " ParallelM.h: operator Communicator Not implemented";
...
struct PostWaitCopyBuffer : public PostWaitWork {
  PostWaitCopyBuffer(const Container& buffer, const OutputIter out) : _buf(buffer), _out(out) {}
...
struct PostWaitUnpackBuffer : public PostWaitWork {
  ...
  virtual void run() { ParallelM::unpack_range(_buf, _context, _out); }
...
extern ParallelM::FakeCommunicator CommWorld;
#define CAN_DEFAULT_TO_COMMWORLD
#else
extern ParallelM::Communicator CommWorld;
#define CAN_DEFAULT_TO_COMMWORLD = CommWorld
#endif
...
#include "parallel_implementationM.h"
// **********************************************************************
