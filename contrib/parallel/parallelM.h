...
public:
    ...
    template <typename T>     bool verify ( const T &r ) const;

    // Verify that a local pointer points to the same value on all
    // processors where it is not NULL.
    // Containers must have the same value in every entry.
    template <typename T>    bool semiverify ( const T *r ) const;
    ...
    template <typename T>    void min ( T &r ) const;
     ...
     */
    template <typename T>    void minloc ( T &r,  unsigned int &min_id ) const;
     ...
     */
    template <typename T>    void minloc ( std::vector<T> &r,
     ...
     * on all processors.  Containers are replaced element-wise.
     */
    template <typename T>    void max ( T &r ) const;
     ...
     */
    template <typename T>    void maxloc ( T &r, unsigned int &max_id ) const;
...
struct PostWaitCopyBuffer : public PostWaitWork {
    ...
    virtual void run() {
        std::copy ( _buf.begin(), _buf.end(), _out );
    }
...
struct PostWaitUnpackBuffer : public PostWaitWork {
    ...
    virtual void run() {
        ParallelM::unpack_range ( _buf, _context, _out );
    }
