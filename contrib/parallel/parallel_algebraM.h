...
class StandardType<TypeVector<T> > : public DataType {
 public:
  explicit StandardType(const TypeVector<T>* example = NULL) {
...
class StandardType<VectorValue<T> > : public DataType {
 public:
  explicit StandardType(const VectorValue<T>* example = NULL) {
    // We need an example for MPI_Address to use
    VectorValue<T>* ex;
...
class StandardType<Point> : public DataType {
 public:
  explicit StandardType(const Point* example = NULL) {
    // We need an example for MPI_Address to use
    Point* ex;
    AutoPtr<Point> temp;
    if (example)
...
class StandardType<TypeTensor<T> > : public DataType {
 public:
  explicit StandardType(const TypeTensor<T>* example = NULL)
...
class StandardType<TensorValue<T> > : public DataType {
 public:
  explicit StandardType(const TensorValue<T>* example = NULL)
      : DataType(StandardType<T>(example ? &((*example)(0, 0)) : NULL), M_DIM * M_DIM) {}
