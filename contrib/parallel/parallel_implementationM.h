...
inline void send_receive_vec_of_vec (
    ...
    const ParallelM::MessageTag &send_tag,
    const ParallelM::MessageTag &recv_tag,
...
//       // STOP_LOG("send_receive()", "ParallelM");
    ...
    MPI_Pack_size ( 1,ParallelM::StandardType<unsigned int>(),
                    comm.get(),   &packedsize );
    ...
    for ( std::size_t i=0; i<send.size(); i++ ) {
        ...
        MPI_Pack_size ( static_cast<unsigned int> ( send[i].size() ),
                        ParallelM::StandardType<T1>
...
//     assert ( sendsize /* should at least be 1! */ );
    ...
    MPI_Pack ( &sendsize, 1,
               ParallelM::StandardType<unsigned int>(),
               &sendbuf[0], static_cast<unsigned int> ( sendbuf.size() ),
               &pos, comm.get() );
    ...
    for ( std::size_t i=0; i<send.size(); i++ ) {
        ...
        MPI_Pack ( &sendsize, 1, ParallelM::StandardType<unsigned int>(),
                   &sendbuf[0], static_cast<unsigned int> ( sendbuf.size() ), &pos,
                   comm.get() );
        ...
        if ( !send[i].empty() )
            MPI_Pack ( &send[i][0],
                       ...
                       ParallelM::StandardType<T1> ( &send[i][0] ),
                       &sendbuf[0],
                       static_cast<unsigned int> ( sendbuf.size() ), &pos,
                       comm.get() );
    }
    ...
    pos=0;
    MPI_Unpack ( &recvbuf[0], static_cast<unsigned int> ( recvbuf.size() ), &pos,
                 &sendsize, 1, ParallelM::StandardType<unsigned int>(),
                 comm.get() );
    ...
    for ( std::size_t i=0; i<recv.size(); i++ ) {
        MPI_Unpack ( &recvbuf[0],
                     static_cast<unsigned int> ( recvbuf.size() ), &pos,
                     &sendsize, 1,
                     ParallelM::StandardType<unsigned int>(),
                     comm.get() );
        ...
        if ( !recv[i].empty() ) MPI_Unpack ( &recvbuf[0],
                                                 static_cast<unsigned int> ( recvbuf.size() ), &pos,
                                                 &recv[i][0],
                                                 static_cast<unsigned int> ( recv[i].size() ),
                                                 ParallelM::StandardType<T2> ( &recv[i][0] ),
                                                 comm.get() ) ;
    }
  ...
  std::vector<buffertype>& buffer 
    ...
    for ( ; range_stop != range_end; ++range_stop ) {
        std::size_t next_buffer_size =
            ParallelM::packable_size ( *range_stop, context );
    ...
    for ( ; range_begin != range_stop; ++range_begin ) {
        ...
        unsigned int my_packable_size =
            ParallelM::packable_size ( *range_begin, context );
        unsigned int my_packed_size =
            ParallelM::packed_size ( *range_begin, buffer.begin() +
                                     old_size );
...
#ifdef HAVE_MPI
inline void Communicator::split (
    int color,
    int key,
    Communicator &target
) const
...
inline void Communicator::assign (
...
// ======================================================================================
inline bool Communicator::verify (
    const std::string & r
) const
{
    if ( this->size() > 1 ) {
...
// ======================================================================================
template <typename T>
inline bool Communicator::semiverify (
    const T *r
) const
{// ======================================================================================
    if ( this->size() > 1 && Attributes<T>::has_min_max == true ) {
        T tempmin, tempmax;
        if ( r ) { tempmin = tempmax = *r;        } 
        else {
...
{// ====================================================================================
    if ( this->size() > 1 && !r.empty() ) {   // START_LOG("min(vector)", "ParallelM");
        ...
        std::vector<T> temp ( r );
        MPI_Allreduce ( &temp[0], &r[0], static_cast<int>( r.size() ),
...
//       // START_LOG("minloc(vector)", "ParallelM");
        ...
        MPI_Allreduce ( &in[0], &out[0], static_cast<int> ( r.size() ),dataplusint_type<T>(),
                        MPI_MINLOC,this->get() );
        for ( std::size_t i=0; i != r.size(); ++i ) {
            r[i]      = out[i].val;
            min_id[i] = out[i].rank;
...
//       // START_LOG("minloc(vector<bool>)", "ParallelM");
        ...
        MPI_Allreduce ( &in[0], &out[0], static_cast<int> ( r.size() ),
...
{
    if ( this->size() > 1 ) {
        DataPlusInt<int> in;        in.val = r;        in.rank = this->rank();
        DataPlusInt<int> out;
        MPI_Allreduce ( &in, &out, 1, dataplusint_type<int>(),MPI_MAXLOC, this->get());
        r = out.val;        max_id = out.rank;
    } 
                                   ...
                                   std::vector<unsigned int> &max_id ) const
{
    if ( this->size() > 1 && !r.empty() ) {
        ...
        std::vector<DataPlusInt<T> > in ( r.size() );
        for ( std::size_t i=0; i != r.size(); ++i ) {
            in[i].val  = r[i];      in[i].rank = this->rank();
        }
        std::vector<DataPlusInt<T> > out ( r.size() );
        MPI_Allreduce ( &in[0], &out[0], static_cast<int> ( r.size() ),
                        ...
                        this->get() );
        for ( std::size_t i=0; i != r.size(); ++i ) {
            r[i]      = out[i].val;    max_id[i] = out[i].rank;
        }
...
) const
{
    ...
    if ( this->size() > 1 && !r.empty() ) {
        ...
        std::vector<DataPlusInt<int> > out ( r.size() );
        MPI_Allreduce ( &in[0], &out[0], static_cast<int> ( r.size() ),
                        StandardType<int>(), MPI_MAXLOC,   this->get() );
        for ( std::size_t i=0; i != r.size(); ++i )        {
            r[i]      = out[i].val;     max_id[i] = out[i].rank;
        }
...
// ======================================================================================
template <typename T> inline void Communicator::send ( 
   const unsigned int dest_processor_id,
   T &buf,
   Request &req,
...
const {
    ...
    ( ( this->send_mode() == SYNCHRONOUS ) ? MPI_Issend : MPI_Isend ) 
      ( dataptr,1,StandardType<T> ( dataptr ),
                                   dest_processor_id,
                                   tag.value(),
                                   this->get(),
                                   req.get() ) ;
                                 ...
                                 const MessageTag &tag ) const
{
    ...
    ( new ParallelM::PostWaitDeleteBuffer<std::vector<T> > ( vecbuf ) );
        ...
        const MessageTag &tag ) const
{
    ...
    std::size_t total_buffer_size =
        ParallelM::packed_range_size ( context, range_begin, range_end );
        ...
        const MessageTag &tag ) const
{
    ...
    *total_buffer_size_buffer = total_buffer_size;
    ...
    while ( range_begin != range_end ) {
        ...
        ( new ParallelM::PostWaitDeleteBuffer<std::vector<buffer_t> >
          ( buffer ) );
                                    ...
                                    const MessageTag &tag ) const
{
    ...
    req.add_post_wait_work
    ( new ParallelM::PostWaitCopyBuffer<std::vector<T>,
    ...
    req.add_post_wait_work
    ( new ParallelM::PostWaitDeleteBuffer<std::vector<T> > ( tempbuf ) );
                                      ...
                                      const MessageTag &tag ) const
{
    ...
    MPI_Recv ( buf.empty() ? NULL : &buf[0],
                 static_cast<int> ( buf.size() ), type, stat.source(),
                                    ...
                                    const MessageTag &tag ) const
{
    ...
    MPI_Irecv ( buf.empty() ? NULL : &buf[0],
                  static_cast<int> ( buf.size() ), type, src_processor_id,
                  tag.value(), this->get(), req.get() ) ;
...
// ======================================================================================
template <typename T> inline void Communicator::gather ( 
...
{// ======================================================================================
    ...
    if ( this->size() > 1 ) {
        ...
        MPI_Gather ( &sendval, 1, send_type, recv.empty() ? NULL : &recv[0],
                     1, send_type, root_id, this->get() );
    } 
    else {        recv[0] = sendval;  }
}
...
{
    if ( this->size() == 1 ) {assert ( !this->rank() ); assert ( !root_id ); return; }
    assert( root_id< this->size() );
...
{// =====================================================================================
   ...
   recv.resize ( this->size() );
    ...
    else if ( comm_size > 0 ) {        recv[0] = sendval;    }
}
  ...
  const bool identical_buffer_sizes ) const
{
    ...
    if ( identical_buffer_sizes ) {        
        ...
        MPI_Allgather ( &r_src[0], static_cast<int> ( r_src.size() ),
                          send_type, &r[0], ( int ) ( r_src.size() ),
                          send_type, this->get() );
...
// ======================================================================================
template <typename T> inline void Communicator::alltoall ( 
 std::vector<T> &buf 
) const
{// =====================================================================================
...
// ======================================================================================
template <> inline void Communicator::broadcast (
    bool &data,
    const unsigned int root_id
) const
{// =====================================================================================
...
// ======================================================================================
template <typename T> inline void Communicator::broadcast ( 
...
{// =====================================================================================
    ...
    if ( this->rank() != root_id ){data.clear();data.insert ( vecdata.begin(), vecdata.end() );}
...
// ======================================================================================
template <typename T> inline bool Communicator::verify ( const T & ) const{return true;}
// ======================================================================================
template <typename T> inline bool Communicator::semiverify ( const T * ) 
const {return true;}
// ======================================================================================
template <typename T> inline void Communicator::min ( T & ) const {}
// ======================================================================================
template <typename T> inline void Communicator::minloc ( T &, unsigned int &min_id ) 
const{    min_id = 0;}
// ======================================================================================
template <typename T> inline void Communicator::minloc
( std::vector<T> &r, std::vector<unsigned int> &min_id ) const
{ for ( std::size_t i=0; i!= r.size(); ++i ) {min_id[i] = 0;  }
}
// ======================================================================================
template <typename T> inline void Communicator::max ( T & ) const {}
// ======================================================================================
template <typename T> inline void Communicator::maxloc ( T &, unsigned int &max_id ) 
const{    max_id = 0;}
// ======================================================================================
template <typename T> inline void Communicator::maxloc
( std::vector<T> &r, std::vector<unsigned int> &max_id ) const
{    for ( std::size_t i=0; i!= r.size(); ++i ) {max_id[i] = 0; }
}
// ======================================================================================
template <typename T> inline void Communicator::sum ( T & ) const {}
// ======================================================================================
template <typename T> inline void Communicator::set_union ( T& ) const {}
// ======================================================================================
template <typename T> inline void Communicator::set_union ( T&, const unsigned int root_id )
const{    assert ( root_id==0 );}
// ======================================================================================
// We do not currently support probes on one processor without MPI.
inline status Communicator::probe ( const unsigned int, const MessageTag& ) 
const{    std::cout << " Not implemented " ;std::abort();status s;return s;}
// ======================================================================================
...
{    std::cout << " Not implemented " ;    std::abort();}
// ======================================================================================
template <typename T> inline void Communicator::send ( const unsigned int, T&, Request&,
...
{   std::cout << " Not implemented " ;   std::abort();}
// ======================================================================================
template <typename Context, typename Iter> inline void Communicator::send_packed_range
...
{ std::cout << " Not implemented " ;    std::abort();    return Status();}
// ======================================================================================
template <typename T> inline void Communicator::receive
( const unsigned int, T&, Request&, const MessageTag& ) const
{ std::cout << " Not implemented " ;  std::abort();}
// ======================================================================================
template <typename T> inline Status Communicator::receive
( const unsigned int, T&, const DataType&, const MessageTag& ) const
{  std::cout << " Not implemented " ;  std::abort();  return Status();
}
// ======================================================================================
...
inline void Communicator::send_receive ( const unsigned int send_tgt,
        T1 &send_val,  const unsigned int recv_source,T2 &recv_val,
        const MessageTag &,  const MessageTag & ) const
{    assert ( send_tgt==0 );  assert ( recv_source==0 );  recv_val = send_val;}
// ======================================================================================
...
template <typename T>
inline void Communicator::gather 
( const unsigned int root_id,  T send_val, std::vector<T> &recv_val ) const
{  assert( root_id==0 );    recv_val.resize ( 1 );    recv_val[0] = send_val;}
// ======================================================================================
template <typename T> inline void Communicator::gather 
( const unsigned int root_id, std::vector<T>& ) const
{   assert( root_id==0 );}
// ======================================================================================
template <typename T> inline void Communicator::allgather (T send_val,std::vector<T> &recv_val) 
const{  recv_val.resize ( 1 );   recv_val[0] = send_val;}
// ======================================================================================
template <typename T> inline void Communicator::allgather ( std::vector<T> &, const bool )
const {}
// ======================================================================================
template <typename T> inline void Communicator::alltoall ( std::vector<T> & ) const {}
// ======================================================================================
template <typename T> inline void Communicator::broadcast (T &,const unsigned int root_id) 
const{assert(root_id==0);}
// ======================================================================================
// ======================================================================================
#endif // HAVE_MPI
