...
#ifdef HAVE_STD_THREAD
#define TLS_TYPE(type) thread_local type
#define TLS_REF(value) (value)
#else
#ifdef HAVE_TBB_API
#include "tbb/enumerable_thread_specific.h"
#define TLS_TYPE(type) tbb::enumerable_thread_specific<type>
#define TLS_REF(value) (value).local()
#else  // Maybe support gcc __thread eventually?
#define TLS_TYPE(type) type
#define TLS_REF(value) (value)
#endif
#endif
...
class BoolAcquire {
 public:
  explicit BoolAcquire(bool& b) : _b(b) {
    assert(!_b);
    _b = true;
  }
...
inline void parallel_for(const Range& range, const Body& body) {
  ...
  const bool logging_was_enabled = perflog.logging_enabled();

  if (n_threads() > 1) perflog.disable_logging();
#endif
...
#ifdef ENABLE_PERFORMANCE_LOGGING
  if (n_threads() > 1 && logging_was_enabled) perflog.enable_logging();
#endif
}
...
inline void parallel_for(const Range& range, const Body& body, const Partitioner& partitioner) {
  BoolAcquire b(in_threads);
 ...
 */
template <typename Range, typename Body>
inline void parallel_reduce(const Range& range, Body& body) {
  BoolAcquire b(in_threads);
 ...
 */
template <typename Range, typename Body, typename Partitioner>
inline void parallel_reduce(const Range& range, Body& body, const Partitioner& partitioner) {
  BoolAcquire b(in_threads);
 ...
 */
template <typename Range, typename Body, typename Partitioner>
inline void parallel_for(const Range& range, const Body& body, const Partitioner&) {
  BoolAcquire b(in_threads);
  body(range);
}
 ...
 */
template <typename Range, typename Body, typename Partitioner>
inline void parallel_reduce(const Range& range, Body& body, const Partitioner&) {
  BoolAcquire b(in_threads);
  body(range);
}
...
class BlockedRange {
 public:
   ...
   * execution.
   */
  explicit BlockedRange(const unsigned int new_grainsize = 1000) : _grainsize(new_grainsize) {}
   ...
   * vector.
   */
  BlockedRange(const BlockedRange<T>& r) : _end(r._end), _begin(r._begin), _grainsize(r._grainsize) {}
  ...
  BlockedRange(BlockedRange<T>& r, Threads::split)
      : _end(r._end), _begin(r._begin), _grainsize(r._grainsize) {
    const_iterator beginning = r._begin, ending = r._end, middle = beginning + (ending - beginning) / 2u;
