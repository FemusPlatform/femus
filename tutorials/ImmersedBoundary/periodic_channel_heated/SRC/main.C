...
MEDCoupling::MEDCouplingFieldDouble* GetFieldComponent(
    MEDCoupling::MEDCouplingFieldDouble* fieldToExtract, int compID);
...
int main(int argc, char** argv) {
  ...
  PeriodicProjector->InitFe();
  PeriodicProjector->FillParameters(OutletMesh, InletMesh, Boundary);

  MEDCoupling::MEDCouplingFieldDouble* origTin = P.getValuesOnInterface(16, "T", 1, 0);
  for (int itime = itime_0 + 1; itime <= itime_0 + n_steps; itime++) {
    ...
    MEDCoupling::MEDCouplingFieldDouble* OutT = P.getValuesOnInterface(14, "T", 1, 0);
    double meanTout = PeriodicProjector->Integrate(OutT, 2, 1, 0, Mean);
    OutT->applyFunc(1, "x - " + std::to_string(meanTout - 573.));

    MEDCoupling::MEDCouplingFieldDouble* InT = PeriodicProjector->InterpolatedField(OutT);
    P.setFieldSource(16, 2, InT);
    P.write_Boundary_value(16, "T", 1, 0);
    OutT->decrRef();
    InT->decrRef();
  }
...
MEDCoupling::MEDCouplingFieldDouble* GetFieldComponent(
  ...
  Arr2->alloc(nodes, 1);
  for (int i = 0; i < nodes; i++) Arr2->setIJ(i, 0, Arr->getIJ(i, compID));
