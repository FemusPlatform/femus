...
class MeshC {
 ...
 public:
  ...
  MeshC(
    ...
    _hx = 0.;
    _hy = 0.;
    _hz = 0.;  // Cartesian dimension domain Vol=hx hy hz
  ...
  MeshC(
      int nex_in, int ney_in, int nez_in, int hx_in, int hy_in,
      int hz_in  // Cartesian dimension domain Vol=hx hy hz
  ) {
    _nx = nex_in;
    _ny = ney_in;
    _nz = nez_in;
    _hx = hx_in;
    _hy = hy_in;
    _hz = hz_in;  // Cartesian dimension domain Vol=hx hy hz
    int n_nodes_mesh_c = (_nx + 1) * (_ny + 1) * (_nz + 1);
    _x = new double[n_nodes_mesh_c];
    _y = new double[n_nodes_mesh_c];
    _z = new double[n_nodes_mesh_c];
  ...
  void print_mesh_med2hdf5(std::string file_name  ///< file name (where to print)
  );
};
