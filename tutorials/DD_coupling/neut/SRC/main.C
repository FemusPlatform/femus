...
int main(
  ...
  std::vector<int>  IDSvec(1); IDSvec[0]=2;
  ...
  int nfa=N.get_n_fuel_array();  int na=N.get_n_array();
  std::cout << " \n  ==========  Number of fuel array " <<  nfa
            << " Number of array " <<  na  << "====== \n";
...
//        int node_med   = map_med[i_med];  // element id in neutronic "mesh"
    ...
    if(tc<500) {std::cout <<i_med <<" elemnt "<<tc<<"T_c bound  500" <<"\n "; tc= 500;}
    if(tc>700) {std::cout <<i_med <<" elemnt "<<tc <<"T_c bound 700" <<"\n "; tc= 700;}
    double tf=tc+450;
    if(tf>1150) {std::cout<<i_med<<" elemnt "<<tf<< "T_f limited 1150" <<"\n "; tf=1150;}
  ...
  for(int itime=itime_0; itime<n_steps; itime++) {
    ...
    std::string name_cmp[2]; name_cmp[0]="power"; name_cmp[1]="burnup";
    std::ostringstream file_name_h5; file_name_h5<<"RESU/pw."<<itime<<".h5";
    if(iproc==0) {
...
//    Get interface 8 Temperature (P1) -----------------------------------------
   ...
   // setting fuel, coolant  temperature and coolant density field --------------
    for(int i_med=0; i_med <n_elem; i_med++) {
      double tc= array_bdy->getIJ(i_med,1);
      if(tc<500){std::cout<<i_med<<" elemnt "<<tc<<" T_c bounded 500" <<"\n ";tc= 500;}
      if(tc>700){std::cout<<i_med<<" elemnt "<<tc<<" T_c bounded 700" <<"\n ";tc= 700;}
      double tf=tc+450;
      if(tf>1150){std::cout<<i_med<<" elemnt "<<tf<<" T_f bounded 1150" <<"\n ";tf=1150;}
      array->setIJ(i_med,0,tf);  // Temperature fuel
      array->setIJ(i_med,1,0.65); // Density Coolant
      array->setIJ(i_med,2,tc);  // Temperature Coolant
    }
