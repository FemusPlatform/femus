...
void InterfaceFunctionDD::set_mesh_BFinterface_nodeID(
  ...
  //         n_bd_nodes++;
  //         for(int idim=0; idim<dim_mg; idim++)  xyz_mg.push_back(mesh->_xyz[i+idim*n_nodes_mg]); //
  //         d->getIJ(i, idim); nodeid_mg.push_back(i);
  //       }
  //     }
  //
  //     break;
...
void InterfaceFunctionDD::set_mesh_interface_nodeID(
  ...
  //     mesh for(int i=0; i<n_nodes_mg; i++) { // cycle on mg nodes
...
void InterfaceFunctionDD::set_mesh_interface_elemID(
  ...
  _map_med = new int[_n];
  _map_mg = new int[_n];
  int nex = mesh_c.get_nex();
  int ney = mesh_c.get_ney();
  int nez = mesh_c.get_nez();
  for (int i_m = 0; i_m < _n; i_m++) {
    const double xx = xyz_med[dim_med * i_m];
    const double yy = xyz_med[dim_med * i_m + 1];
    const double zz = xyz_med[dim_med * i_m + 2];
    int kjx = 0;
    while (xx > mesh_c._x[kjx]) kjx++;
    kjx--;
    int kjy = 0;
    while (yy > mesh_c._y[kjy * (nex + 1) + kjx]) kjy++;
    kjy--;
    int kjz = 0;
    while (zz > mesh_c._z[kjz * (ney + 1) * (nex + 1) + kjx * kjy]) kjz++;
    kjz--;
...
void InterfaceFunctionDD::set_analytic_field(
  ...
  if (symbolic_eq == std::string("")) {
    std::cout << "InterfaceFunctionDD::set_analytic_field: NULL function";
    abort();
  }
...
void InterfaceFunctionDD::printOn(
  ...
  out << std::endl
      << "================================================== \n"
      << std::endl
      << " InterfaceFunctionDD =" << id << std::endl
      << std::endl;
  // ---------------------------------------------------------------------
  // ---------------- Mapping ---------------------------------------------
