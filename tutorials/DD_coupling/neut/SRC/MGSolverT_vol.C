...
void MGSolT::vol_integral(
    DenseMatrixM& KeM, DenseVectorM& FeM, const int el_ndof2, const int el_ngauss, const int mode,
    double WallDist[],
    double AlphaTurb
        []) {  // ==============================================================================================
    ...
    for (int i = 0; i < el_ndof2; i++) {
      ...
      if (_bc_el[i] == 1) {
        double Phi_supg = 0., Lap_expl = 0., Lap_supg = 0.,
               Adv_expl = 0.;  // supg, Lap explicit , Lap explicit supg
        for (int idim = 0; idim < _nTdim; idim++) {
        ...
        if (mode == 1) {  // rhs
          ...
          FeM(i) += dtxJxW_g * (phii_g + Phi_supg) *
                    (0. + 10.e-6 * .5176156e+8 * _qheat * phii_g  // source term Source qheat  1.176156e+08
                    );
        ...
        for (int j = 0; j < el_ndof2; j++) {
          ...
          if (_T_parameter._SolveSteady == 0)
            KeM(i, j) += dtxJxW_g * rhocp * phij_g * (phii_g + Phi_supg) / _dt;
