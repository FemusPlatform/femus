...
public:
 ...
 // Set ----------------------------------------------------------------------------
  ...
  void set_up(
      std::string  filename_in // name init power  (ex IniPowCompo)
  );
#ifdef HAVE_MED
void set_mesh_med(
  const  MEDCoupling::MEDCouplingUMesh  & mg_mesh_med_in ///< MEDMesh (in)
...
/// 2. This function retrun the pointer of the InterfaceFunctionC from the id name  
  InterfaceFunctionDD * get_interface_fun(
    ...
    return (_interfaceFunMap.find(id) == _interfaceFunMap.end() ? 
                                                           NULL :_interfaceFunMap[id]);
  }
// ======================================================================================
