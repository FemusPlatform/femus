...
void DONDRA::set_up(
    std::string  filename_in ///< name init power  (ex IniPowCompo_.c2m)
) {
...
//   Cle2000Ptr IniPowCompo = Cle2000Ptr(new Cle2000(filename.str().c_str(), 0, ipLifo1));
  ...
  IniPowCompo->exec();  cout << "IniPowCompo execution completed" << endl;
...
//   _mesh_c._zzz = new double [n_nodes_mesh_c];
  ...
  for(int k=0; k<=nz; k++) {
    for(int j=0; j<=ny; j++) {
      for(int i=0; i<=nx; i++) {
        ...
        _mesh_c->_x[ind]= meshxTab[i]*_Lref;   _mesh_c->_y[ind]= meshyTab[j]*_Lref;
  ...
  hsize_t dimsf[2]; dimsf[0] = n_nodes_mesh_c;  dimsf[1] = 1;
  hid_t dtsp = H5Screate_simple(2, dimsf, NULL);
  hid_t dtsetx = H5Dcreate(file,"X1",H5T_NATIVE_DOUBLE,dtsp,
			   H5P_DEFAULT,H5P_DEFAULT,
			   H5P_DEFAULT);
  hid_t dtsety = H5Dcreate(file,"X2",H5T_NATIVE_DOUBLE,dtsp,
			   H5P_DEFAULT,H5P_DEFAULT,
			   H5P_DEFAULT);
  hid_t dtsetz = H5Dcreate(file,"X3",H5T_NATIVE_DOUBLE,dtsp,
			   H5P_DEFAULT,H5P_DEFAULT,
			   H5P_DEFAULT);
  // Write the data to the dataset using default memory space, file space, and transfer properties.
  hid_t status=
         H5Dwrite(dtsetx,H5T_NATIVE_DOUBLE,H5S_ALL,H5S_ALL,H5P_DEFAULT,&_mesh_c->_x[0]);
  ...
  // clean ------------------------------------------------------------------------------
   H5Dclose( dtsetx); H5Dclose( dtsety);H5Dclose( dtsetz);
   H5Fclose(file);
...
void DONDRA::solve_onestep() {
  ...
  //iteration loop
  ClcmPtr Flux, Thm;
  float_32 densB = 2000.; float Keff_conv = 1.0;
  int iter = 1;
//    int continueLoop = 1;
...
//   PowComponent = Cle2000Ptr(new Cle2000(filename_pow.str().c_str(), 0, ipLifo2));
  ...
  if(iter == 1) {ipLifo2->pushEmpty("Flux", "LCM");/*ipLifo2->pushEmpty("MacroP", "LCM");*/}
  else { ipLifo2->push("Flux", Flux);/*ipLifo2->push("MacroP", _MacroP);*/}
    ...
    for(int ielem=0; ielem< nElement_med; ielem++) {
      ...
      for(int inode=0; inode< n_nod_el; inode++) {
        for(int im=0; im<dim_med; im++) {
          x_m[im] += d->getIJ(nodes1[inode],im);
...
//   double val[2];
  for(int k=0; k<n_element_mesh_c; k++) {
    double val=0;
    if(indexfmTab[k]!=0) {val=powerTab[counter_ipower];  counter_ipower++;}
    pwr[k]=val;
  }
...
// Print -------------------------------------------------------------------------------
  std::cout <<  "\n getValuesPW_elem with " <<counter_ipower<< " fuel elements over "
            << nElem << "total elements" << std::endl;
...
MEDCoupling::MEDCouplingFieldDouble * DONDRA::getValuesPWBUP_elem(
  ...
  double val[2]; int counter_ipower=0;
  double potmax=0.;
  for(int k=0; k<n_element_mesh_c; k++) {
    if(indexfmTab[k]!=0) {
  ...
  array->decrRef();     // delete array
  delete []pwr; delete []irr;

  // print ------------------------------------------------------------------------------
  std::cout <<  "\n GetValuesPWBup_elem: " <<counter_ipower<< " fuel elements ("
            << nElem_med << " total)" << std::endl;
  return f;
...
//   _n_fuel_array = stateVector[0] * stateVector[1];
  ...
  int counter_ipower=0;
  for(int k=0; k<n_element_mesh_c; k++) {
    if(indexfmTab[k]!=0) {map11[k]=counter_ipower; counter_ipower++;}
    else map11[k]=-1;
  }
  ...
  for(int i_mg=0; i_mg < nElement_med; i_mg++) {
    int index_2=(int)(xyz_med[3*i_mg+0]*ne_x/3.66)+
                ((int)(xyz_med[3*i_mg+1]*ne_y/3.66))*ne_x+((int)(xyz_med[3*i_mg+2]*ne_z/6.67))*ne_y*ne_x;
//     int gl_node_bd = map_mg[i_mg];  // mg  node
    if(indexfmTab[index_2]!=0) {
//     int node_med  = map_med[i_mg];  // med node
      fct->eval(i_mg,3, bc_value);
//
      int  indx54=map11[index_2];  if(indx54 <0 || indx54>_n_fuel_array) {
...
//   _n_fuel_array = stateVector[0] * stateVector[1];
  ...
  FloatPtr myArray_td(new float_32[_n_fuel_array]);
  FloatPtr myArray_tc(new float_32[_n_fuel_array]);
  double max=0.; double min=100000000.;
...
//   int  n_cmp,            ///< number of components
  const MEDCoupling::MEDCouplingFieldDouble *field
) {// =======================================================================
...
void  DONDRA::print_power_med2hdf5(
  ...
  for(int k=0; k<n_element_mesh_c; k++) {
    if(indexfmTab[k]!=0) {
      ...
      counter_ipower++;
    } else { pwr[k]=-1.e-12; irr[k]=-1.e-12;flux[k]=1.e-12;}
  }
  double potmax1=0.;
  for(int k=0; k<n_element_mesh_c; k++) {
    if(indexfmTab[k]!=0) {
  ...
  hsize_t dimsf[2]; dimsf[0] =n_element_mesh_c;  dimsf[1] = 1;// dataspace dimension vector dimsf

  // power
  hid_t dtsp = H5Screate_simple(2, dimsf, NULL);      // dataspace dtsp
  hid_t dtsetxp = H5Dcreate(file,"power",H5T_NATIVE_DOUBLE,dtsp,
  ...
  dtsetxp = H5Dcreate(file,"burnup",H5T_NATIVE_DOUBLE,dtsp,
#if HDF5_VERSIONM == 1810
                      H5P_DEFAULT,H5P_DEFAULT,
#endif
                      H5P_DEFAULT);
  ...
  // flux
  dtsp = H5Screate_simple(2, dimsf, NULL);      // dataspace dtsp
  dtsetxp = H5Dcreate(file,"flux",H5T_NATIVE_DOUBLE,dtsp,
#if HDF5_VERSIONM == 1810
                      H5P_DEFAULT,H5P_DEFAULT,
#endif
                      H5P_DEFAULT);
...
void  DONDRA::print_power_xmf(
  ...
  std::string elem_type("Hexahedron");  int n_node_elem=8;
  // 3D setup
  int nxyz[3]; nxyz[0]=_mesh_c->get_nex(); nxyz[1]=_mesh_c->get_ney();
  nxyz[2]=_mesh_c->get_nez();
  // 2D setup
  if(DIMENSION ==2) { elem_type="Quadrangular" ; n_node_elem=4; nxyz[2]=0;}
  //  Mesh ------------------------------------------------------------------------------
  //   const MGMesh& mgmesh=_mgmesh;
  int n_nodes_lin = (nxyz[0]+1)*(nxyz[1]+1)*(nxyz[2]+1);     //
  std::cout<< "DONDRA::print_power_xmf -> nx ny nz "
           <<nxyz[0]<<" "<<nxyz[1]<<" "<<nxyz[2]<<std::endl;
  int n_elements_lin =_mesh_c->get_n_elements();//
  std::cout<< "DONDRA::print_power_xmf -> n_nodes " << n_nodes_lin << "  n_elements  "<< n_elements_lin << std::endl;
//
//   // get parameters
...
// //   conn_file << ".h5";
  ...
  coord_time_file /*<<femus_dir  << "/"<< output_dir << "/"*/ << basemesh << "."
      << std::setw(ndigits) << std::setfill('0') << t_step << ".h5";
...
// //  ++++++++++++ Header ++++++++++++++
  ...
  out << " <DataStructure DataType=\"Float\" Precision=\"8\" Dimensions=\""<< n_nodes_lin << " 1\" Format=\"HDF\"> ";
  out << "mesh_c.h5:X1 \n";
  out << " </DataStructure> \n";
  out << " <DataStructure DataType=\"Float\" Precision=\"8\" Dimensions=\""<< n_nodes_lin << " 1\" Format=\"HDF\"> ";
  out << "mesh_c.h5:X2 \n";
...
//   // ++++  Attributes ++++++++++++
  ...
  out << "<DataItem  DataType=\"Float\" Precision=\"8\" Dimensions=\""<< n_elements_lin << " 1\" Format=\"HDF\">  \n";
...
void  DONDRA::print_med(std::string namefile,
                        const MEDCoupling::MEDCouplingFieldDouble & field /* Level1*/) {
