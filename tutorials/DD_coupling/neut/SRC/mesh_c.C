...
void  MeshC::print_mesh_med2hdf5(
...
//   if(n_cmp == 0) std::cout << " DONDRA::print_power_med2hdf5: we have  n_cmp=0 !!!";
 ...
 hsize_t dimsf[2]; dimsf[0] =n_nodes_mesh_c;  dimsf[1] = 1;// dataspace dimension vector dimsf
 hid_t dtsp = H5Screate_simple(2, dimsf, NULL);      // dataspace dtsp
 // coors x ------------------------------------------------------------------ 
  hid_t dtsetxp = H5Dcreate(file,"X1",H5T_NATIVE_DOUBLE,dtsp,
   ...
   // coors y ------------------------------------------------------------------ 
    dtsetxp = H5Dcreate(file,"X2",H5T_NATIVE_DOUBLE,dtsp,
 #if HDF5_VERSIONM == 1810
                               H5P_DEFAULT,H5P_DEFAULT,
 #endif
                               H5P_DEFAULT);
   status=H5Dwrite(dtsetxp,H5T_NATIVE_DOUBLE,H5S_ALL,H5S_ALL,H5P_DEFAULT,& _y[0]);
    H5Dclose(dtsetxp); // close dataspace
       // coors z ------------------------------------------------------------------ 
    dtsetxp = H5Dcreate(file,"X3",H5T_NATIVE_DOUBLE,dtsp,
 #if HDF5_VERSIONM == 1810
                               H5P_DEFAULT,H5P_DEFAULT,
 #endif
                               H5P_DEFAULT);
 ...
 // connectivity ------------------------------------------------------------------------
   ...
   dimsf[0] =n_elements_mesh_c*8;  dimsf[1] = 1;// dataspace dimension vector dimsf
   dtsp = H5Screate_simple(2, dimsf, NULL);      // dataspace dtsp
 // connectivity storage  
  dtsetxp = H5Dcreate(file,"MESH0CONN",H5T_NATIVE_INT,dtsp,
 #if HDF5_VERSIONM == 1810
                               H5P_DEFAULT,H5P_DEFAULT,
 #endif
                               H5P_DEFAULT);
