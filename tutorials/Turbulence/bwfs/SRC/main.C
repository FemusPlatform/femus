...
void PrintTime(double begin_time, double par_time, int itime) {
  ...
  std::cout << " Step n. " << itime;

  if (minuti < 1.) {
    std::cout << " Computational time until now " << secondi << " s  \n";
  } else if (minuti < 60. && minuti > 0.9999) {
    std::cout << " Computational time until now " << (int)minuti << " m and " << secondi - (int)minuti * 60
              << " s  \n";
  } else {
    std::cout << " Computational time until now " << (int)ore << " h and " << (int)(minuti - 60 * (int)ore)
              << " m and " << secondi - (int)minuti * 60 << " s  \n";
  }
}
