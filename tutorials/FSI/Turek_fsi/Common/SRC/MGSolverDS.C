...
MGSolDS::MGSolDS(
    ...
    : MGSolDA(mg_equations_map_in, nvars_in, eqname_in, varname_in),
      ...
      _rhos(_mgutils._mat_prop["rhos"]),         // parameter solid density
      _disp_d(mg_equations_map_in.get_par("disp_d")),
      _Emod(_mgutils._mat_prop["Es"]),
      _ni(_mgutils._mat_prop
              ["nis"])  //  =========================================================================
{
    ...
    )              // ===============================================
{
  ...
  double x_m[DIMENSION], l_m[DIMENSION];
  double normal[DIMENSION];
  // gauss integration  -----------------------------------------------------------------------------

  double u_old[NDOF_FEM * DIMENSION];
  double p_old;
  ...
  for (int iel = 0; iel < (nel_e - nel_b); iel++) {
    ...
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[SDSX_F]]->get_el_nonl_sol(
        0, 1, el_ndof[2], el_conn, offset, 0, u_old);  //  non linear vel
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[SDSX_F + 1]]->get_el_nonl_sol(
        0, 1, el_ndof[2], el_conn, offset, 1, u_old);  //  non linear vel
    int myNode[1];
    myNode[0] = 230;
    double tempDisp[1];
    double myDisp[2];
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[SDSX_F]]->get_el_sol(
        0, 0, 1, 1, myNode, offset, 0, tempDisp);  //  non linear vel
    myDisp[0] = tempDisp[0];
    tempDisp[0] = 0.;
    _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[SDSX_F + 1]]->get_el_sol(0, 0, 1, 1, myNode, offset, 0, tempDisp);
    myDisp[1] = tempDisp[0];
    //         cout<<myDisp[0]<<"\t"<<myDisp[1]<<endl;
    // ----------------------------------------------------------------------------------
    ...
    for (int iside = 0; iside < el_sides; iside++) {
      if (el_neigh[iside] == -1) {
        ...
        set_bc_matrix(
            KeM, FeM, dir_maxnormal, sur_toply, el_ndof, elb_ndof, elb_ngauss, normal,
    ...
    if (_FF_idx[NS_F] >= 0) {
      for (int idim = 0; idim < _nDSdim; idim++) {
        for (int d = 0; d < NDOF_FEM; d++)
          vel_g[idim] += _data_eq[2].ub[NDOF_FEM * (_FF_idx[NS_F] + idim) + d] / NDOF_FEM;
      }
    } else {
      vel_g[_nDSdim - 1] = 0.;
      vel_g[0] = 1.;
    ...
    // volume integral
    if (phase == 0) {
      matrixrhsvol_liq_ds(KeM, FeM, el_ndof, flag_group, u_old, myDisp);
    } else {  // ==========================  solid ===================================================
...
void MGSolDS::MGTimeStep_no_up(
  ...
  if (_SolveDS) {
    ...
    if (_mgutils.get_name() != 1) { MGSolve(1.e-6, 40); }
#if PRINT_TIME == 1
    end_time = std::clock();
    ...
    //     x_old [0][_NoLevels - 1]->localize(*d_aux [0]);
  }
  return;
}  // =======================================================================================

void MGSolDS::MGUpdateStep() {
  if (_SolveDS) {
...
/// This function assembles the matrix and the rhs:
//  ====================================================
void MGSolDS::MoveMesh(const int Level  // Level <-
                       )                // ===============================================
{
  const int offset = _mgmesh._NoNodes[_NoLevels - 1];  // mesh nodes
  const ParallelM::Communicator& comm1 = _mgmesh._comm.comm();
...
// ===================================================
void DS_param::read_param(MGUtils& mgutils)  // ==================================================
{
  ...
  std::string GroupString =
      mgutils._sim_config["BoundaryGroups"];  // from Simulation configuration (es "20,21,13")
