...
class MGSolFSI : public MGSolDA {
 ...
 private:
  ...
  /*!
   * For each element, before performing the loop over boundary sides with #MGSolFSI::set_bc_matrix,
   * we set the values of #MGSolFSI::_bc_el equal to _BdFlagId in order to highlight boundary nodes
   * and to see if an equation has already been assigned to a matrix row. After looping over boundary sides
   * no row can have a value of _bc_el equal to _BdFlagId. See #MGSolFSI::_bc_el for admissible values
   */
  ...
  int _AxiSym;
  int _pres_order;
  double _u_2Ts[NDOF_FEM * DIMENSION];
  ...
  /*!
   * Tangent vector can be calculated by geometric construction, namely, given normal vector \f$ \bf{n}
   * \f$,<br>
 ...
 public:
  ...
  void ic_read(
      int bc_gam, int bc_mat, double xp[],  // point coordinates
      int iel, double u_value[]             // point field values
  );

  /// This function reads ic
  void bc_read(
      int bc_gam, int bc_mat, double x[],  // point coordinates
  ...
  void get_el_field_data(
      int iel, int Level, int el_conn[], int offset, int el_dof[], int ndof_lev, double u_old[],
      double u_oold[], double u_nl[], double p_proj[], double dp_proj[]);

  // ==============================================================================================
  /// This function computes drag and lift with a volumetric approach
  void calc_stress_vol(
      double u_old[],
      int sur_toply[],  ///< boundary topology map
      int el_ndof[],    ///< number of volume dofs
      int elb_ndof[]    ///< number of boundary dofs
   ...
   * The normal vector is taken from #MGSolFSI::_normal_pt .
   ...
   */
  void CalcTangDir(
      int PhiNumber,   ///< [in] ID of node for which we are writing the equation - local element numbering
      ...
      int TangType     ///< [in] Method for calculation of tangent vectors, see #MGSolFSI::TangType
  );
   ...
   * for normal and tangential directions. The flags are written into #MGSolFSI::_bc_el
   */
  void SetBCFlags(double normal[], int sur_toply[], int el_ndof, int el_conn[]);

  //! Correction of BC flags
  /*!
   * This function corrects the values of #MGSolFSI::_bc_el <br>
   * It is used when #MGSolFSI::_nNSdim == 3 in order to switch the rows taking the equations along
   * directions \f$ \bf{t_1} \f$ and \f$ \bf{t_2} \f$ for corvengence issues.     <br>
   * \f{bmatrix}{  \dots row = i  \dots \\ \\ \dots row = i + el\_ dof \dots \\ \\ \dots row = i + 2 el\_ dof
   * \dots  \f} Equation for normal direction is written in \f$ row = i + j*el\_ dof \f$ where j is the
   * maximum normal vector component. In a similar way equations for directions \f$ \bf{t_1} \f$ and \f$
   * \bf{t_2} \f$ are written in \f$ row = i + k*el\_ dof \f$ and \f$ row = i + l*el\_ dof \f$ where k and l
   * are the maximum vector components of \f$ \bf{t_1} \f$ and \f$ \bf{t_2} \f$, respectively.
   */
  void CorrectBCFlags(
      double Tang1[],   ///< [in, out] Array containing components of vector \f$ \bf{t_1} \f$
