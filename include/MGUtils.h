...
class MGUtils {
 ...
 public:
  std::map<std::string, double>
      _geometry;  ///< Geometry properties: bounding box for libmesh mesh generation, axisym, wall_dist
  std::map<std::string, double>
      _mat_prop;  ///< Material properties: density, viscosity and other physical prop.
  std::map<std::string, std::string> _mgfiles;  ///< Files map
  std::map<std::string, std::string>
      _sim_config;  ///< Simulation properties: dt, print step, n. of steps, etc.
  std::map<std::string, std::string>
      _temp_info;  ///< Simulation properties: dt, print step, n. of steps, etc.
  ...
  inline void set_par(const std::string& name, double value) { _param_utils[name] = value; }
  inline void set_geom_par(const std::string& name, double value) { _geometry[name] = value; }
  inline void set_mat_par(const std::string& name, double value) { _mat_prop[name] = value; }
  inline void set_sim_par(const std::string& name, std::string value) { _sim_config[name] = value; }
  ...
  inline void ClearTemp() { _temp_info.clear(); }
  // #endif
  ///@}
  //-----------------------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------------------
  hid_t print_Dhdf5(
      hid_t file, const std::string& name, hsize_t dimsf[], double data[]);  ///< Print vector of double
  hid_t print_Ihdf5(
      hid_t file, const std::string& name, hsize_t dimsf[], int data[]);  ///< Print vector of integer
