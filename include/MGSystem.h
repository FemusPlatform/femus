...
class MGSystem {
 ...
 public:
  ...
  inline void set_par(const std::string& name, double value) { parameters.insert(make_pair(name, value)); }
  ///@}
  //-------------------------------------------------------------
  ///@{ \name READ-PRINT
  ...
  void init_data(const int mode = 0);
  ...
  /// Point data field
  double F_ext(const double xx[], const int ivarq) const;
  /// Return point data
  inline double get_sys_data(const int inode, const int iflag) const { return _sys_data[iflag][inode]; }
  ///@}
