 ...
 */
enum ConversionMode { MeanValue = 0, MidPoint = 1 };
...
class MedUtils {
 public:
  ...
  // PRINT FUNCTION
  void PrintMed(
      const MEDCoupling::MEDCouplingUMesh* SourceV_update,
  ...
  void PrintMed(MEDCoupling::MEDCouplingFieldDouble* f, std::string FileName, int n = 1);

  MGFE* _fe[3];
  void BuildCanonicalElementNodesMap(int NodesPerCell, std::map<int, int>& Mappa);

  double Integrate(
      const MEDCoupling::MEDCouplingFieldDouble* Field, int order = 2,
  ...
  virtual void GaussLoop(
      std::vector<double> NodeVar, std::vector<double> Velocity, bool BulkMedium, const int dim,
      const int DimRelToMax, const int XOrd, const int FOrd, const int VOrd, const int NodesPerCell,
  ...
  MEDCoupling::MEDCouplingFieldDouble* GetCellField(
      const MEDCoupling::MEDCouplingFieldDouble* SourceField, int ConversionMode = MeanValue);
  void InitFe();

  inline void setProcId(int procId) { _proc = procId; }

  // ======================================================================================================================
  virtual MEDCoupling::MEDCouplingFieldDouble* InterpolatedField(
      const MEDCoupling::MEDCouplingFieldDouble*
          SourceField, /**< Source mesh field that have the solution used for the interpolation */
      int order = 2);
  // ======================================================================================================================
