...
class TurbUtils
  ...
  public:
    ...
    int _Nagano, _Wilcox;
    int _numod, _emod, _ehmod, _WallFunction;
    double _vmid, _diameter, _Tmean, _Fraction;
...
//   FLAGS
  ...
  public:
    TurbUtils ();
    TurbUtils ( double wall_dist, double TurbModel[], double nu, double alpha =
                  1.e-4 );
    TurbUtils ( int proc, int levels, bool DynTurb, bool TherTurb );
    TurbUtils ( int proc, int levels, bool DynTurb, bool TherTurb, int MeshID );
    ~TurbUtils ();
    void read_file ();
    void print_par ();
    ...
    void CalcThermTurSourceAndDiss ( double KappaAndOmega[],
                                     ...
                                     double & alphaturb,
                                     double source[2],
                                     double diss[2], double meccterm[2] );

    void FillParameters ( double wall_dist,
                          double TurbModel[], double nu, double alpha = 1.e-4 );
    void FillParameters ( double wall_dist,
                          std::string val1,
                          std::string val2,
                          std::string val3,
                          std::string val4,
                          double nu = 1.e-4, double alpha = 1.e-4 );
    void FillParameters ( double wall_dist,
    ...
    void DynTurNearWallValues ( double & kappa, double & omega, double WallDist,
                                double Utau );
    void ThermTurNearWallValues ( double & kappa, double & omega, double WallDist,
                                  double Utau );

    void DynTurInitValues ( double & kappa, double & omega, double WallDist,
                            bool FlatProfile );
    void DynTurInitValues ( double & kappa, double & omega, double WallDist,
                            double Utau );
    void TherTurInitValues ( double & kappaT, double & omegaT, double WallDist,
                             double NormFlux, double AvVel, double Diameter,
                             bool FlatProfile );
    void CalcWallFuncKappaAndOmega ( double KappaAndOmega[], int NodeOnBound,
                                     double WallDist, double utau );
    void CalcWallFuncThermalKappaAndOmega ( double KappaAndOmega[],
                                            int NodeOnBound, double WallDist,
                                            double utau );
    inline bool CheckIfFilled ()
      {
      return _IsFilled;
      }
    inline void SetAvVel ( double vel )
      {
      _vmid = vel;
      }
    ...
    double IntegrateMusker ( double xp );
  };
