...
class EquationSystemsExtendedM : public MGEquationsSystem {
 ...
 public:
...
#ifdef HAVE_MED
  const MEDCoupling::MEDCouplingUMesh* getMeshMED() const { return _med_mesh; }
  ...
  void add_interface_fun(
      const int interface_name, const int bd_name_int,  ///<[in] boundary id (int)     (in)
  ...
  void setBC(
      ...
      const MEDCoupling::MEDCouplingFieldDouble*
          NumericField /**<[in] MED field containing the values to write */
  );
  ...
  void write_Boundary_value(
      int id_name,                ///< [in] Identity interface name
   ...
   */
  void write_Boundary_value(
      std::string mgsystem_name,                    ///[in] Equation name
   ...
   */
  MEDCoupling::MEDCouplingFieldDouble* getValuesOnBoundary_nodes(
      int id,                   ///< [in] Interface ID
   ...
   *  The values are relative to mesh cells of interface with name <id>
   */
  MEDCoupling::MEDCouplingFieldDouble* getValuesOnBoundary_elem(
      int id,                   ///< [in] Interface ID
      const char* system_name,  ///< [in] Equation name
      int n_cmp,                ///< [in] Number of components
      int first_cmp = 0         ///< [in] First component
  ...
  MEDCoupling::MEDCouplingFieldDouble* getDisplacement(
      int id, const char* system_name, int ncmp, int first_cmp = 0);
   ...
   */
  MEDCoupling::MEDCouplingFieldDouble* getProcValues(
      MEDCoupling::MEDCouplingFieldDouble* NodeMap,  ///<[in] MED field containing the map of proc nodes
  ...
  inline void getEqsNames(std::vector<string>& FieldNames) { get_eqs_names(FieldNames); };
