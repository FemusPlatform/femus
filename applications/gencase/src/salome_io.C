...
void init_eletypes() {
  if (eletypes_exp.empty() && eletypes_imp.empty()) {
    ...
    // 0 DIMENSIONAL GEOM ***************************************************************
    {
      ...
      eledef.nnodes = 1;
      eledef.nfaces = 0;     // number of nodes/faces
    ...
    // 1 DIMENSIONAL GEOM ***************************************************************
    {
      ...
      eledef.type = EDGE2;
      eledef.exptype = 1;  // element libmesh type/name
      eledef.nnodes = 2;
      eledef.nfaces = 2;     // number of nodes/faces
      eledef.nodes.clear();  // clean function
    ...
    }
    {
      ...
      eledef.type = EDGE3;
      eledef.exptype = 8;  // element libmesh name/number
      eledef.nnodes = 3;
      eledef.nfaces = 2;                       // number of nodes/faces
      eledef.nodes.clear();                    // clean function
    ...
    // 2 DIMENSIONAL GEOM ***************************************************************
    {
      ...
      eledef.type = TRI3;
      eledef.exptype = 2;  // element libmesh name/number
      eledef.nnodes = 3;
      eledef.nfaces = 3;  // number of nodes/faces
      // map
    ...
    }
    {
      ...
      eledef.type = TRI6;
      eledef.exptype = 9;  // element libmesh name/number
      eledef.nnodes = 6;
      eledef.nfaces = 3;                                // number of nodes/faces
    ...
    }
    {
      ...
      eledef.type = QUAD4;
      eledef.exptype = 3;  // element libmesh name/number
      eledef.nnodes = 4;
      eledef.nfaces = 4;                          // number of nodes/faces
    ...
    }
    {
      ...
      eledef.type = QUAD8;
      eledef.exptype = 100;  // element libmesh name/number
      eledef.nnodes = 8;
      eledef.nfaces = 4;                                      // number of nodes/faces
    ...
    }
    {
      ...
      eledef.type = QUAD9;
      eledef.exptype = 10;  // element libmesh name
      eledef.nnodes = 9;
      eledef.nfaces = 4;                                         // number of nodes
    ...
    }
    {
      ...
      eledef.exptype = 12;
      eledef.nnodes = 27;
      const unsigned int nodes[] = {4, 7,  3,  0,  5,  6, 2,  1,  19, 15, 11, 12, 17, 14,
...
// =========================================================
void SalomeIO::read_mesh(std::istream&  // istream file to read  <-
) {                                     // =====================================================
  std::cout << "Use read";
  abort();
  return;
...
// ============================================================================
// SalomeIO  members
void SalomeIO::read(const std::string& namefile  // namefile <-
) {  // =========================================================================
  ...
  if (eletype.dim != DIMENSION) {
    std::cout << "\nWARNING!!! Mesh dimension is (from eletype) " << eletype.dim << " and DIMENSION "
              << DIMENSION << std::endl;
  ...
  for (int j = 0; j < (int)n_nodes; j++) {
    for (int idim = 0; idim < DIMENSION; idim++)
      xyz3[idim] = xyz_med[j + n_nodes * idim];  // coordinate table conversion
    if (DIMENSION == 3)
      mesh.add_point(Point(xyz3[0], xyz3[1], xyz3[2]), j);  // 3D
    else
      mesh.add_point(Point(xyz3[0], xyz3[1]), j);  // 2D
  }
  delete[] xyz_med;
  ...
  read_bc_mat(
      file_id, el_fem_type_vol /*(std::string) el_fem_type[index_vol]*/,
  ...
  //   if(msh_count2%2==0)  bc_casedir << femus_dir <<  "/USER_APPL/"  <<  myapp_name
  //   <<"/RESU/bc_case.msh1.h5"; else if(msh_count2%2==1)  bc_casedir << femus_dir <<  "/USER_APPL/"  <<
  //   myapp_name  <<"/RESU/bc_case.msh2.h5"; else if(msh_count2%2==2)  bc_casedir << femus_dir <<
  //   "/USER_APPL/"  <<  myapp_name  <<"/RESU/bc_case.msh3.h5"; else if(msh_count2%2==3)  bc_casedir <<
  //   femus_dir <<  "/USER_APPL/"  <<  myapp_name  <<"/RESU/bc_case.msh4.h5"; msh_count2++;
...
//         std::cout << "SalomeIO::read Group name " << as << "= group " << str3 << " id " <<
//         bdgroup_name[atoi(str3.c_str())] << '\n';
//
//       }
//     }
//   }
...
//   else if(msh_count2%2==1)  bc_casedir << femus_dir <<  "/USER_APPL/"  <<  myapp_name
//   <<"/RESU/bc_case.msh2.h5"; else if(msh_count2%2==2)  bc_casedir << femus_dir <<  "/USER_APPL/"  <<
//   myapp_name  <<"/RESU/bc_case.msh3.h5"; else if(msh_count2%2==3)  bc_casedir << femus_dir <<
//   "/USER_APPL/"  <<  myapp_name  <<"/RESU/bc_case.msh4.h5"; msh_count2++;
//
...
void SalomeIO::read_bc_mat(
  ...
  for (int iname_dir = 0; iname_dir < (int)n_bdgroup; iname_dir++) {
    ...
    if (n_bdgroup_sub > 0) {
      for (int iname_sub = 0; iname_sub < (int)n_bdgroup_sub; iname_sub++) {
        ...
        //  char array format
        hid_t array_tid = H5Tarray_create(
            H5T_NATIVE_CHAR, dim[0], array_dim
#if HDF5_VERSIONM == 188
            ,
            NULL
#endif
  ...
  dimsf[0] = n_nodes;
  dimsf[1] = 1;
  print_Ihdf5(file_id2, "bc_flag", dimsf, bc_flag);
  // print bc_flag
  dimsf[0] = n_elements;
  dimsf[1] = 1;
  print_Ihdf5(file_id2, "material", dimsf, mat_flag);

  // family identities and group names
  dimsf[0] = bdgroup_name.size();
  dimsf[1] = 1;
  int icount = 0;
  // family vectors
  int* family_id = new int[dimsf[0]];
  int* group_names = new int[dimsf[0]];
  std::map<int, int>::iterator itr;
  for (itr = bdgroup_name.begin(); itr != bdgroup_name.end(); ++itr) {
    family_id[icount] = itr->first;
    group_names[icount] = itr->second;
    icount++;
  }
...
// ============================================================================
// This function reads the boundary condition and mat groups
int SalomeIO::read_fem_type(hid_t file_id, std::string& el_fem_type_vol, std::string& el_fem_type_bd) {
  ...
  fem_type_vol["TR6"] = 9;
  fem_type_vol["SE2"] = 1;
  fem_type_vol["SE3"] = 8;  // not valid in 3D
  ...
  fem_type_bd["H27"] = 0;
  fem_type_bd["TE4"] = 0;
  fem_type_bd["T10"] = 0;  // not valid in 2D
  fem_type_bd["QU4"] = 3;
  fem_type_bd["QU8"] = 19;
  fem_type_bd["QU9"] = 10;
  fem_type_bd["TR3"] = 2;
  ...
  int itype_vol = fem_type_vol[el_fem_type[index_vol]];
  assert(itype_vol != 0);
...
hid_t SalomeIO::print_Ihdf5(
  ...
  hid_t dataset = H5Dcreate(
      file, name.c_str(), H5T_NATIVE_INT, dataspace, H5P_DEFAULT
#if HDF5_VERSIONM != 1808
      ,
