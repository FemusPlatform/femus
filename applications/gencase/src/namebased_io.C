...
void NameBasedIO::read(const std::string& name) {
  ...
  if (name.rfind(".nem") + 4 == name.size() || name.rfind(".n") + 2 == name.size()) {
    ...
    full_name << name << '.' << mymesh.n_processors() << '.' << std::setfill('0') << std::setw(field_width)
              << mymesh.processor_id();

    std::ifstream in(full_name.str().c_str());

    if (!in.good()) libmesh_error_msg("ERROR: cannot locate specified file:\n\t" << full_name.str());
  } else if (name.rfind(".cp")) {
  }  // Do error checking in the reader
  else {
  ...
  if (is_parallel_file_format(name)) {
    ...
    if ((name.rfind(".xda") < name.size()) || (name.rfind(".xdr") < name.size())) {
      XdrIO xdr_io(mymesh);
      ...
      mymesh.allow_renumbering(false);
#endif
    } else if (name.rfind(".nem") < name.size() || name.rfind(".n") < name.size())
    ...
    if (mymesh.processor_id() == 0) {
      ...
      if (new_name.rfind(".mat") < new_name.size())
        ...
        UCDIO(mymesh).read(new_name);
          ...
          (new_name.rfind(".off") < new_name.size()) || (new_name.rfind(".ogl") < new_name.size()) ||
      ...
      else if ((new_name.rfind(".node") < new_name.size()) || (new_name.rfind(".ele") < new_name.size()))
        TetGenIO(mymesh).read(new_name);

      else if (new_name.rfind(".exd") < new_name.size() || new_name.rfind(".e") < new_name.size())
      ...
      if (name.size() - name.rfind(".bz2") == 4) std::remove(new_name.c_str());
      if (name.size() - name.rfind(".xz") == 3) std::remove(new_name.c_str());
    }
...
void NameBasedIO::write_nodal_data(
    const std::string& name, const std::vector<Number>& v, const std::vector<std::string>& vn) {
  MeshBase& mymesh = MeshInput<MeshBase>::mesh();
...
void NameBasedIO::write_equation_systems(
    const std::string& filename, const EquationSystems& es, const std::set<std::string>* system_names) {
  ...
  if (!system_names) {
    if (filename.rfind(".xda") < filename.size()) {
      es.write(filename, EquationSystems::WRITE_DATA | EquationSystems::WRITE_ADDITIONAL_DATA);
      return;
  ...
  MeshOutput<MeshBase>::write_equation_systems(filename, es, system_names);
}
