...
void MGGenCase::printMesh(
  ...
  for (int i = 0; i < _n_levels * _n_subdomains; i++) { n_nodes_sl[i] = 0; }
  ...
  for (int is = 0; is <= _n_subdomains * _n_levels; is++) { n_nodes_slp[is] = 0; }
  ...
  for (int i = 0; i < n_nodes; i++) {
    ...
    for (int k = 0; k < _N_NDV + 2; k++) { nod_sto[i][k] = -1; }
    nod_sto[i][2] = 100000;
  ...
  for (int knode = 0; knode < n_nodes; knode++) {
    ...
    if (nod_sto[knode][3] < _n_levels) { n_nodes_slp[_n_levels * nod_sto[knode][1] + nod_sto[knode][3]]++; }
    if (nod_sto[knode][3] == 0) { n_nodes_0p++; }
  ...
  for (int ilev = 0; ilev < _n_levels; ilev++) {
    ...
    int count = 0;
    for (int inode = 0; inode < n_nodes; inode++) { g_indexL[ilev][inode] = -1; }
    for (int iproc = 0; iproc < _n_subdomains; iproc++) {
      for (int inode = off_nd[0][iproc * _n_levels]; inode < off_nd[0][iproc * _n_levels + ilev + 1];
           inode++) {
  ...
  int count = 0;
  for (int inode = 0; inode < n_nodes; inode++) { g_indexL[_n_levels][inode] = -1; }
  for (int iproc = 0; iproc < _n_subdomains; iproc++) {
  ...
  //   print mesh
  print_mesh_h5(
      n_nodes_lev, map_mesh_in, n_nodes, nod_val, off_nd, n_elements, n_elements_lev, bd_n_elements,
  ...
  if (mgops_gen) {
    compute_and_print_MGOps(
        n_nodes_lev, maxelxnode, off_el, v_el, elem_sto, v_inv_nd, v_inv_el, g_indexL, off_nd);
  }
  // clean
  delete[] v_inv_elb;
  delete[] v_inv_el;
  ...
  delete[] elem_sto;
  for (int ind = 0; ind < _n_levels + 1; ind++) { delete[] g_indexL[ind]; }
  delete[] g_indexL;
  delete[] off_nd[1];
  delete[] n_nodes_lev;
// #endif
...
void MGGenCase::compute_and_print_MGOps(
  ...
  std::cout << " MGGenCase::compute_and_print_MGOps:  start \n";
  compute_matrix(n_nodes_lev, max_elnd, off_el, v_el, elem_sto, v_inv_nd, g_indexL, off_nd);
#ifdef PRINT_INFO
  std::cout << " MGGenCase::compute_and_print_MGOps: compute_matrix end \n";
#endif
  compute_prol(n_nodes_lev, off_el, v_el, elem_sto, v_inv_nd, v_inv_el, g_indexL, off_nd);
#ifdef PRINT_INFO
  std::cout << " MGGenCase::compute_and_print_MGOps: compute_prol end  \n";
#endif
  compute_rest(n_nodes_lev, max_elnd, off_el, v_el, elem_sto, v_inv_nd, v_inv_el, g_indexL, off_nd);
#ifdef PRINT_INFO
...
void MGGenCase::print_mesh_h5(
    int* n_nodes_lev, const int* map_mesh_in, int n_nodes, double* nod_val, int* off_nd[], int n_elements,
  ...
  for (int itp = 0; itp < n_meshes; itp++) { tdata[itp] = map_mesh_in[itp]; }
  // hdf5 sorage
  dimsf[0] = n_meshes;
  dimsf[1] = 1;
  ...
  for (int i = 0; i < _n_subdomains; i++) {
    ...
    for (int j = 0; j < _n_levels; j++) {
      std::cout << " Level Nodes " << j << " from " << off_nd[0][j + i * _n_levels] << " to "
                << off_nd[0][j + 1 + i * _n_levels] << "\n ";
  ...
  std::cout << " \n ";
  // node offset linear
  status = _mgutils.print_Ihdf5(file, "/NODES/MAP/OFF_ND1", dimsf, off_nd[1]);
  dimsf[0] = n_meshes + 1;
  dimsf[1] = 1;
  status = _mgutils.print_Ihdf5(file, "/NODES/MAP/NDxLEV", dimsf, n_nodes_lev);
  ...
  // +++++++++++++++++++++++++++++++++++
  subgroup_id = H5Gcreate(
      file, "/ELEMS/FEM0", H5P_DEFAULT
#if HDF5_VERSIONM != 1808
      ,
      H5P_DEFAULT, H5P_DEFAULT
#endif
  );
  hid_t subgroup_id2 = H5Gcreate(
      file, "/ELEMS/SUB", H5P_DEFAULT
#if HDF5_VERSIONM != 1808
      ,
      H5P_DEFAULT, H5P_DEFAULT
#endif
  ...
  dimsf[0] = _n_levels;
  dimsf[1] = 1;
  status = _mgutils.print_Ihdf5(file, "/ELEMS/FEM0/NExLEV", dimsf, &n_elements_lev[0]);
  ...
  dimsf[1] = 1;  // global mesh hdf5 storage
  status = _mgutils.print_Ihdf5(file, "/ELEMS/FEM0/EL_NEIG", dimsf, tempconn_el);
  // --------------------------------------------------------------------------
  ...
  for (int ilev = 0; ilev < _n_levels; ilev++) {
    ...
    status = _mgutils.print_Ihdf5(file, name.str(), dimsf, temp_material2);
  ...
  dimsf[0] = _n_levels;
  dimsf[1] = 1;
  status = _mgutils.print_Ihdf5(file, "/ELEMS/FEM1/NExLEV", dimsf, &n_elements_lev[_n_levels]);
  ...
  dimsf[0] = _n_subdomains * _n_levels + 1;
  dimsf[1] = 1;
  status = _mgutils.print_Ihdf5(file, "/ELEMS/FEM1/OFF_EL", dimsf, bd_off_el);
  ...
  for (int Level1 = 0; Level1 < _n_levels; Level1++) {
    ...
    for (int iql = 0; iql < 2; iql++) {
      ...
      for (int idom = 0; idom < _n_subdomains; idom++) {
        offM[iql][idom + 1] =
            offM[iql][idom] + (off_nd[iql][idom * _n_levels + Level1 + 1] - off_nd[iql][idom * _n_levels]);
      }
    }
    ...
    for (int kdom = 0; kdom < _n_subdomains; kdom++) {
      // piecewise
      nodes_top[2] += (off_el[kdom * _n_levels + Level1 + 1] - off_el[kdom * _n_levels + Level1]) * NDOF_K;
    }
    if (Level1 > 0) { nodes_top[1] = n_nodes_lev[Level1 - 1]; }
    ...
    for (int iql = 0; iql < 2; iql++) {
      for (int jql = 0; jql < 2; jql++) {
        ...
        int count_q = compute_mat_qq(
            Level1, level_row[iql], level_cln[jql], el_nodes[iql], el_nodes[jql], n_nodes, max_elnd, off_el,
            v_el, elem_sto, v_inv_nd, g_indexL, off_nd[iql], offM[qlt % 2], aux, mem, Mat_q, len_q, len_qoff);
        print_op_h5(
    ...
    int jql = 0;
    int qlt = 20;
    int count_q = compute_mat_qk(
        Level1, level_row[iql], level_cln[jql], el_nodes[iql], el_nodes[2], n_nodes, max_elnd, off_el, v_el,
        elem_sto, v_inv_nd, g_indexL, off_nd[iql], offM[iql], aux, mem, Mat_q, len_q, len_qoff);
    print_op_h5(
        name.str(), nodes_top[iql], nodes_top[2] /* *NDOF_K */, count_q, Mat_q, NULL, len_q, len_qoff, qlt);

    //  matrix   kq ====================================================
    iql = 0;
    jql = 0;
    qlt = 02;
    count_q = compute_mat_kq(
        Level1, level_row[iql], level_cln[jql], el_nodes[2], el_nodes[jql], n_nodes, max_elnd, off_el, v_el,
        elem_sto, v_inv_nd, g_indexL, off_nd[iql], offM[iql], aux, mem, Mat_q, len_q, len_qoff);
    print_op_h5(
        name.str(), nodes_top[2] /* *NDOF_K */, nodes_top[iql], count_q, Mat_q, NULL, len_q, len_qoff, qlt);

    //  matrix   lk ====================================================
    iql = 1;
    jql = 1;
    qlt = 10;
    count_q = compute_mat_lk(
        Level1, level_row[iql], level_cln[jql], el_nodes[jql], el_nodes[2], n_nodes, max_elnd, off_el, v_el,
        elem_sto, v_inv_nd, g_indexL, off_nd[1], offM[iql], aux, mem, Mat_q, len_q, len_qoff);
    print_op_h5(
        name.str(), nodes_top[iql], nodes_top[2] /* *NDOF_K */, count_q, Mat_q, NULL, len_q, len_qoff, qlt);
    ...
    jql = 0;
    qlt = 0;
    count_q = compute_mat_kk(
        Level1, level_row[iql], level_cln[jql], el_nodes[2], el_nodes[2], n_nodes, max_elnd, off_el, v_el,
        elem_sto, v_inv_nd, g_indexL, off_nd[iql], offM[iql], aux, mem, Mat_q, len_q, len_qoff);
    print_op_h5(
        name.str(), nodes_top[2] /* *NDOF_K */, nodes_top[2] /* *NDOF_K */, count_q, Mat_q, NULL, len_q,
        len_qoff, qlt);
...
void MGGenCase::compute_prol(
    ...
    int* v_inv_el, int** g_indexL, int* off_nd[]) {
  // name
  std::string input_dir = _mgutils._inout_dir;
  ...
  for (int Level1 = 1; Level1 < _n_levels; Level1++) {
    ...
    for (int jpr = 0; jpr < _n_subdomains; jpr++) {
      ...
      sum_lev += off_el[jpr * _n_levels + Level1 + 1] - off_el[jpr * _n_levels + Level1 + 0];
    }
    nodes_row[2] = sum_lev;
    ...
    for (int iql = 0; iql < 3; iql++) {
      ...
      if (iql < 2) {
        ...
        count_q = compute_prol_qq(
            Level1, level_row[iql], level_cln[iql], el_nodes[iql], nodes_row[iql], off_el, v_el, elem_sto,
            v_inv_nd, g_indexL, off_nd[iql], values_q, Prol_q, len_q, len_qoff);
        ...
        count_q = compute_prol_kk(
            Level1, level_row[iql], level_cln[iql], el_nodes[iql], nodes_row[iql], off_el, v_el, elem_sto,
            v_inv_nd, v_inv_el, g_indexL, off_nd[iql], values_q, Prol_q, len_q, len_qoff);
      }

      print_op_h5(
          name.str(), nodes_row[iql], nodes_cln[iql], count_q, Prol_q, values_q, len_q, len_qoff, 2 - iql);
      // clean
      delete[] Prol_q;
      delete[] values_q;
      delete[] len_q;
  ...
  for (int Level1 = 0; Level1 < _n_levels - 1; Level1++) {
    ...
    for (int iql = 0; iql < 3; iql++) {
      ...
      if (iql < 2) {
        ...
        count_q = compute_res_qq(
            Level1, level_row[iql], level_cln[iql], el_nodes[iql], nodes_row[iql], nodes_cln[iql], max_elnd,
            off_el, v_el, elem_sto, v_inv_nd, g_indexL, off_nd[iql], mem, values_q, Rest_q, len_q, len_qoff);
      } else {
        // set up
        max_elnd =
        ...
        count_q = compute_res_kk(
            Level1, level_row[iql], level_cln[iql], el_nodes[iql], nodes_row[iql], nodes_cln[iql], max_elnd,
            off_el, v_el, elem_sto, v_inv_nd, v_inv_el, g_indexL, off_nd[iql], mem, values_q, Rest_q, len_q,
            len_qoff);
      }
      // print ----------------------------------------------------
      print_op_h5(
          name.str(), nodes_row[iql], nodes_cln[iql], count_q, Rest_q, values_q, len_q, len_qoff, 2 - iql);
      // clean ----------------------------------------------------
      delete[] Rest_q;
      delete[] values_q;
...
int MGGenCase::compute_mat_qq(
    ...
    int** g_indexL, int* off_nd, int offM[], int aux[], int mem[],  // number elemets row
    ...
    for (int iel = off_el[pr * _n_levels + Level1]; iel < off_el[pr * _n_levels + Level1 + 1]; iel++) {
      ...
      for (int kk = 0; kk < ndof_row_ql; kk++) {
        ...
        for (int kj = 0; kj < ndof_clmn_ql; kj++) {
          ...
          Mat_q[ind_row + mem[i_row] + kj] = g_indexL[Level_clmn][opj];
        }
        mem[i_row] += NDOF_FEM;
      }
    }
  ...
  // compressing
  for (int kdom = 0; kdom < _n_subdomains; kdom++) {
    for (int iki = 0; iki < (int)(off_nd[kdom * _n_levels + Level1 + 1] - off_nd[kdom * _n_levels]); iki++) {
      // setting aux
      ...
      for (int kj = 0; kj < NDOF_FEM * max_elnd; kj++) {
        ...
        if (knod_q >= 0 && aux[knod_q] == 1) {
          ...
          count_q++;
          if (knod_q < (int)offM[kdom] || knod_q >= (int)offM[kdom + 1]) { count_qoff++; }
        }
      }
      len_q[ki + 1] = count_q;
...
int MGGenCase::compute_mat_kq(
  ...
  int n_element_lev = 0;
  for (int pr = 0; pr < _n_subdomains; pr++) {
    n_element_lev += off_el[pr * _n_levels + Level1 + 1] - off_el[pr * _n_levels + Level1];
  }
  // zeroing matrix
  for (int im = 0; im < n_element_lev * ndof_row_ql; im++) {
    mem[im] = 0;  // row index mem position
  }
  for (int im = 0; im < n_element_lev * NDOF_FEM; im++) { Mat_q[im] = -1; }
  ...
  for (int pr = 0; pr < _n_subdomains; pr++) {
    for (int iel = off_el[pr * _n_levels + Level1]; iel < off_el[pr * _n_levels + Level1 + 1]; iel++) {
      ...
      for (int kk = 0; kk < ndof_row_ql; kk++) {
        int i_row = (iel - off_el[pr * _n_levels + Level1] + sum_lev) * ndof_row_ql + kk;
  ...
  int ki = 0;
  // zeroing aux
  for (int i = 0; i < NDOF_FEM * n_element_lev; i++) { aux[i] = 0; }
  // compressing
  for (int kdom = 0; kdom < _n_subdomains; kdom++) {
    for (int iki = 0; iki < off_el[kdom * _n_levels + Level1 + 1] - off_el[kdom * _n_levels + Level1];
         iki++) {
...
int MGGenCase::compute_mat_kk(
    ...
    int ndof_clmn_ql,     // column nodes
    int /*n_nodes */,     // top nodes
    int /*max_elnd */, int* off_el, std::vector<std::pair<int, int>> /*v_el */, int** /*elem_sto */,
    int* /*v_inv_nd */, int** /*g_indexL */, int* /*off_nd */, int /*offM */[], int aux[], int mem[],
    int Mat_q[], int len_q[], int len_qoff[]) {
  // ================================================
  ...
  for (int im = 0; im < n_element_lev * ndof_clmn_ql; im++) { Mat_q[im] = -1; }
  ...
  for (int pr = 0; pr < _n_subdomains; pr++) {
    for (int iel = off_el[pr * _n_levels + Level1]; iel < off_el[pr * _n_levels + Level1 + 1]; iel++) {
      ...
      for (int ki = 0; ki < ndof_row_ql; ki++) {
        ...
        for (int kj = 0; kj < ndof_clmn_ql; kj++) { Mat_q[row_indx + kj] = idof_lev + kj; }
      }
    }
  }
  ...
  int ki = 0;
  // zeroing aux
  for (int i = 0; i < ndof_clmn_ql * n_element_lev; i++) { aux[i] = 0; }
  // compressing
  for (int kdom = 0; kdom < _n_subdomains; kdom++) {
    int delta_dof = (off_el[kdom * _n_levels + Level1 + 1] - off_el[kdom * _n_levels + Level1]) * ndof_row_ql;
    for (int iki = 0; iki < delta_dof; iki++) {
      ...
      for (int jpr = 0; jpr < kdom; jpr++) {
        dof_start_lev += off_el[jpr * _n_levels + Level1 + 1] - off_el[jpr * _n_levels + Level1];
      }
      ...
      for (int kj = 0; kj < ndof_clmn_ql; kj++) {
        ...
        if (knod_q >= 0 && aux[knod_q] == 1) {
          ...
          Mat_q[count_q] = knod_q;
          count_q++;
          if (knod_q < dof_start_lev || knod_q >= delta_dof + dof_start_lev) { count_qoff++; }
        }
      }
      len_q[ki + 1] = count_q;
      len_qoff[ki + 1] = count_qoff;
...
int MGGenCase::compute_mat_qk(
  ...
  // zeroing matrix
  for (int im = 0; im < n_nodes; im++) { mem[im] = 0; }
  for (int im = 0; im < n_nodes * max_elnd * ndof_clmn_ql; im++) { Mat_q[im] = -1; }
  ...
  for (int pr = 0; pr < _n_subdomains; pr++) {
    for (int iel = off_el[pr * _n_levels + Level1]; iel < off_el[pr * _n_levels + Level1 + 1]; iel++) {
      ...
      for (int kk = 0; kk < ndof_row_ql; kk++) {
        ...
        for (int kj = 0; kj < ndof_clmn_ql; kj++) {
          int opj = (iel - off_el[pr * _n_levels + Level1] + sum_lev) * ndof_clmn_ql +
                    kj;  // dof in each elem (pie const)
  ...
  len_qoff[0] = count_qoff;
  // zeroing aux
  for (int i = 0; i < max_elnd * n_nodes * ndof_clmn_ql; i++) { aux[i] = 0; }
    ...
    for (int iki = 0; iki < (int)(off_nd[kdom * _n_levels + Level1 + 1] - off_nd[kdom * _n_levels]); iki++) {
      ...
      for (int kj = 0; kj < max_elnd * ndof_clmn_ql; kj++) {
        ...
        if (knod_q >= 0 && aux[knod_q] == 1) {
          ...
          Mat_q[count_q] = knod_q;
          count_q++;
          if (knod_q < sum_lev * ndof_clmn_ql || knod_q >= (off_el[kdom * _n_levels + Level1 + 1] -
                                                            off_el[kdom * _n_levels + Level1] + sum_lev) *
                                                               ndof_clmn_ql) {
            count_qoff++;
          }
        }
...
int MGGenCase::compute_mat_kl(
    ...
    int n_nodes,       // top nodes
    int max_elnd, int* off_el, std::vector<std::pair<int, int>> v_el, int** elem_sto, int* v_inv_nd,
    int** g_indexL, int* off_nd, int offM[], int aux[], int mem[], int Mat_q[], int len_q[], int len_qoff[]) {
  // ================================================
...
int MGGenCase::compute_prol_qq(
    ...
    for (int iel = off_el[pr * _n_levels + Level1]; iel < off_el[pr * _n_levels + Level1 + 1]; iel++) {
      ...
      for (int kk = 0; kk < ndof_row_ql; kk++) {
        ...
        for (int kj = 0; kj < ndof_row_ql; kj++) {
          ...
          Prol_q[g_indexL[Level_row][el_n] * NDOF_FEM + kj] = g_indexL[Level_clmn][par_n];
  ...
  offPc[0] = 0;
  for (int idom = 0; idom < _n_subdomains; idom++) {
    offPc[idom + 1] = offPc[idom] + (off_nd[idom * _n_levels + Level1] - off_nd[idom * _n_levels]);
  }
  ...
  int ki = 0;
  for (int kdom = 0; kdom < _n_subdomains; kdom++) {
    for (int iki = 0; iki < off_nd[kdom * _n_levels + Level1 + 1] - off_nd[kdom * _n_levels]; iki++) {
      for (int kj = 0; kj < NDOF_FEM; kj++) {
...
int MGGenCase::compute_res_qq(
          ...
          for (int kj = 0; kj < ndof_row_ql; kj++) {
            ...
            if (fabs(val_qq) > 1.e-6) {
              ...
              for (int ks = 0; ks < indx_mem; ks++)
                if (Rest_q[indx0 + ks] == jcln) { flag = 1; }
  ...
  offPc[0] = 0;
  for (int idom = 0; idom < _n_subdomains; idom++) {
    offPc[idom + 1] = offPc[idom] + (off_nd[idom * _n_levels + Level1 + 2] - off_nd[idom * _n_levels]);
  }
  offPc[_n_subdomains] += 1;
