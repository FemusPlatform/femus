...
void init_eletypes() {
  if (eletypes_exp.empty() && eletypes_imp.empty()) {
    ...
    // HEX27 Gambit -> HEX27 (12) LIBMESH -----------------------
    {
      ...
      eledef.nfaces = 6;    // number of faces
      //  map
      const unsigned int nodes[] = {0,  8,  1,  11, 20, 9,  3, 10, 2,  12, 21, 13, 24, 26,
                                    22, 15, 23, 14, 4,  16, 5, 19, 25, 17, 7,  18, 6};
      // inverse map
      std::vector<unsigned int>(nodes, nodes + eledef.nnodes).swap(eledef.nodes);
...
// =======================================================
void GambitIO::read(const std::string& name  // namefile <-
) {                                          // ==================================================
  ...
  std::cout << std::setw(12) << numNodes << std::setw(12) << numElem << std::setw(12) << numCoords
            << std::endl;
  mesh.reserve_nodes(numNodes);  // mesh dim -> nodes
  mesh.reserve_elem(numElem);    // mesh dim -> elements
  int* mat_flag;
  mat_flag = new int[numElem];  // material flag
  int* bc_flag;
  bc_flag = new int[numNodes];  // boundary condition flag:
  ...
  for (unsigned int iel = 0; iel < numElem; ++iel) {
    ...
    switch (face) {
      ...
      case 7:  //"Pyramid"
        std::cout << "Pyramid geom elem not yet implemented" << std::endl;
        abort();
        break;
  ...
  if (msh_count == 0)
    bc_casedir << femus_dir << "/USER_APPL/" << myapp_name << "/RESU/bc_case.msh1.h5";
  else if (msh_count == 1)
    bc_casedir << femus_dir << "/USER_APPL/" << myapp_name << "/RESU/bc_case.msh2.h5";
  else if (msh_count == 2)
    bc_casedir << femus_dir << "/USER_APPL/" << myapp_name << "/RESU/bc_case.msh3.h5";
  else if (msh_count == 3)
    bc_casedir << femus_dir << "/USER_APPL/" << myapp_name << "/RESU/bc_case.msh4.h5";
  msh_count++;

  //    create a new file for storing material and boundary conditions flag
  hid_t file_id = H5Fcreate(bc_casedir.str().c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
