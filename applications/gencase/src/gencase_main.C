...
int main(int argc, char** argv) {
  ...
  for (int mesh = 0; mesh < NUM_MESH; mesh++)
    mg_utils.push_back(new MGUtils(mesh + 1));  // MGUtils class: read and print parameters
  ...
  for (int mesh = 0; mesh < NUM_MESH; mesh++) {
    ...
    if (!restart) {
      if (mgops_gen) {
        ...
        std::cout << "No GenCase because it was already run once" << std::endl;
      }
    } else {
      std::cout << "No GenCase because of restart flag !=0 " << std::endl;
      return 0;
    }
