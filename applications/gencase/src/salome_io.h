...
class SalomeIO
 ...
 private:
  ...
  // (as argument)
  int read_fem_type(hid_t file_id, std::string& el_fem_type_vol, std::string& el_fem_type_bd);
   ...
   */
  void write_post(
      const std::string&, const std::vector<Number>* = NULL, const std::vector<std::string>* = NULL);
  // void H5Lget_name_by_idx(hid_t arg1, const char* arg2,  arg3,  arg4, int arg5, char[3] arg6, int arg7,
  // arg8);
...
inline SalomeIO::SalomeIO(const MeshBase& mesh) : MeshOutput<MeshBase>(mesh), _binary(false) {}

inline SalomeIO::SalomeIO(MeshBase& mesh)
    : MeshInput<MeshBase>(mesh), MeshOutput<MeshBase>(mesh), _binary(false) {}

inline bool& SalomeIO::binary() { return _binary; }
