...
int main(int argc, char** argv) {
  ...
  for (int nDom = 0; nDom < NUM_MESH; nDom++) {
    ...
    if (proc == 0) {
      std::vector<std::string> meshNames =
          MEDLoader::GetMeshNames(mgutils[nDom]->_mesh_dir + mgutils[nDom]->_interface_mesh);
      std::vector<std::string> GroupNames = MEDLoader::GetMeshGroupsNames(
          mgutils[nDom]->_mesh_dir + mgutils[nDom]->_interface_mesh, meshNames[0]);
      ...
      for (int l = 0; l < FieldNames.size(); l++) {
        ...
        //
        SourceField = MEDLoader::ReadField(
            ParaMEDMEM::ON_NODES, QuadFieldsFile, MeshNames[0], 0, FieldNames[l], -1, -1);
        int order = 2;
        ...
        for (int i = 0; i < nComp; i++) {
          if ((FieldNames[l] == "NS0" || FieldNames[l] == "FSI0" || FieldNames[l] == "FSIA0" ||
               FieldNames[l] == "NSA0") &&
              i == nComp - 1)
            order = 1;
          ...
          std::cout << std::setprecision(7) << " Initial L2 Norm of component " << i << ":\t" << in_norm
                    << std::endl;
          std::cout << std::setprecision(7) << " Final L2 Norm of component " << i << ":\t\t" << out_norm
                    << std::endl;
          std::cout << std::setprecision(2)
                    << " Percentage of integral value variation from initial value\t \033[1;36m"
  ...
  In order to use the Interpolation application you need to define \n \
   \t\"HAVE_MED\" \n \
   inside the file $"
            << mgutils[0]->_app_dir << "/DATA/Solverlib_conf.h \n\
   -) Make a resu_clean \n\
   -) Set restart to 0    \n\
