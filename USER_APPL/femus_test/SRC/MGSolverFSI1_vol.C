...
void MGSolFSI::get_el_field_data(
  ...
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[CTRLX_F]]->get_el_old_disp(0,1,el_ndof[2],el_conn, offset,0,_ctrl_oold);      //ctrl soluion at it-2
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[CTRLY_F]]->get_el_old_disp(0,1,el_ndof[2],el_conn, offset,1,_ctrl_oold);      //ctrl soluion at it-2
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_oold_disp(0,2,el_ndof[2],el_conn, offset,0,_ctrl_f_oold);      //force at it-2
  return;
}
        ...
        if(_bc_el[indx] <=0) {
          for(int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++)     {  // Loop over velocity components for equation written in row indx
            ...
            for(int j=0; j<el_ndof2; j++) {
              ...
              if(_FF_idx[K_F]>=0 && _FSI_parameter._InterpolatedMuTurb==1)  // grad(nut)*[grad(u) + grad(u)^T]*phi_supg, diagonal part
                for(int kdim=0; kdim< _nNSdim; kdim++) {
              ...
              for(int  ivars=0; ivars<_nvars[2]; ivars++) {
                ...
                if(_FF_idx[K_F]>=0 && _FSI_parameter._InterpolatedMuTurb==1) {
                  KeM(indx,j+ivars*el_ndof2)  += -1.*dtxJxW_g*rho*_dphi_g[2][j+ivar*el_ndof2]*_IRe*MuTurbDxg[ivars]*Phi_supg;
                }
              }
...
//       MuTurb[0] = min (MuDurbin,MuTurb[0]);
//
//       if (_FSI_parameter._WallFunctionApproach == 1 || _FSI_parameter._InterpolatedMuTurb == 1) _mu_turb = MuTurb[0];
    }
    double IRe_eff = _mus;
    double f_upwind = CalcFUpwind(u_nlg, _dphi_g[2], IRe_eff, _nNSdim, el_ndof2);
        ...
        if(_bc_el[indx] <=0) {
          for(int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++)     {  // Loop over velocity components for equation written in row indx
            ...
            if(mode == 1) {  // rsh  terms
              ...
              double va_old=(_data_eq[2].ub[(_FF_idx[CTRL_F]+ivar)*NDOF_FEM+i]); //ctrl solution at it-1   
	      double va_oold=_ctrl_oold[i+ivar*el_ndof2];                        //ctrl solution at it-2 
              double f_old=_ctrl_f_old[i+ivar*el_ndof2];                         //distributed force at it-1 
              double f_oold=_ctrl_f_oold[i+ivar*el_ndof2];                       //distributed force at it-2 
              double Hess=1.;                                                    //Approx of hessian matrix
	      if (fabs(va_oold)>1.e-16) {
		Hess=((f_old-va_old)-(f_oold-va_oold))/(f_old-f_oold);
// 		if (qp==0/*&&el_conn[i]==64*/) cout<<"f_old "<<f_old<<" va_old "<<va_old<<" f_oold "<<f_oold<<"va_oold "<<va_oold<<
// 		                " Hess "<<Hess<<" elConn "<<el_conn[i]<<endl;
// 		if (Hess<0) 
// 		  int a=1;
	      }
              double df=_control*(f_old-va_old)/Hess;
// 	      if (qp==0/*&&el_conn[i]==64*/) cout<<f_old<<"  "<<f_old-df<<" elConn "<<el_conn[i]<<" hess "<<Hess<<endl;
              FeM(indx)  += dtxJxW_g*rho*(f_old-df)*(phii_g+Phi_supg);
              _ctrl_f[i+ivar*el_ndof2]=(f_old-df);
            }
            // -------------------------------------------------------------------------------------------------------------
            //-------------------------------------- Assemblying matrix ----------------------------------------------------
            for(int j=0; j<el_ndof2; j++) {
