...
void MGSolFSI::set_bc_matrix(
    ...
    if(bd_face == 88 || bd_face == 84) {
        if(_FF_idx[K_F]>=0) {
            if(_FSI_parameter._WallFunctionApproach == 1) {
                _WallElement = 1;
            }
                    ...
                    if(OrDir) {
                        if(bd_node == 88 || bd_node == 66) {
                            ...
                            for(int   jvar=0; jvar< _nNSdim; jvar++)    {    //  -beta_n*n*( u.n)
                                ...
                                if(!NormBC) {
                                    alpha_beta = (_bc_el[indx_row] == 1) ? dir_tang1[jvar]:dir_tang2[jvar];
                                }
...
//       double p_ooold=u_oold[sur_toply[i]];   // boundary pressure field at ooold time
//       double dp=0.;
//       double BHess=1; double va_new=(_data_eq[2].ub[(_FF_idx[CTRL_F]+1)*NDOF_FEM+sur_toply[i]]);
// //       if (fabs(va_old[sur_toply[i]])>1.e-16){  //QUASI-NEWTON METHOD
// //       BHess=((p_ooold-normal[1]*va_new/beta)-(p_veryold[sur_toply[i]]-normal[1]*va_old[sur_toply[i]]/beta))/(p_ooold-p_veryold[sur_toply[i]]);
// //       cout<<BHess<<"   ";
// //       cout<<(p_ooold-normal[1]*va_new/beta)<<"\t"<<(p_ooold-normal[1]*va_old[sur_toply[i]]/beta)<<"  ";
// //       cout<<p_ooold<<"\t"<<p_veryold[sur_toply[i]]<<endl;
// 
// //       }
//       dp=(p_ooold-normal[1]*va_new)/BHess;
// 
//       if(_xx_qnds[sur_toply[i]+1*NDOF_FEM]>0 ) dp =0;
// //       else 
// //         cout<<"  "<<p_ooold<<"  "<<-_control*dp<<"  "<<p_ooold-_control*dp<<"  "<<va_new<<endl;
//       FeM(indx)  += rhs_p*Ipenalty*(p_ooold-_control*dp) ;
    ...
    CalcTangDir(tang1, tang2, normal,1);
                        ...
                        for(int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++) { // LOOP OVER VELOCITY COMPONENTS FOR EQUATION WRITTEN IN ROW indx
                            ...
                            if(_bc_el[indx] == -1 || _bc_el[indx] == -2) {
                                if(bc_vartg_flag !=1) {
                                    ...
                                    double tau = (_bc_el[indx] == -1) ? _FSI_parameter._Tg1_stress:_FSI_parameter._Tg2_stress ; //utau*utau/vel_bound;
                                    double stress = alpha_beta*(pen_cond*tau - lambda1)*dtJxW_g*_phi_g[2][i];
//                 FeM (indx) += /*bc_rhs**/ -1.*stress*VelOnGauss[ivar];
                                    ...
                                    if(bc_varn_flag == 4)
                                        for(int j=0; j<elb_ndof[2]; j++)
                                            for(int jvar=0; jvar<_nNSdim; jvar++) {
                                                KeM(indx, sur_toply[j] + jvar*el_ndof[2]) -= alpha_beta*normal[ivar]*normal[jvar]*beta*dtJxW_g*_phi_g[2][j]*_phi_g[2][i];
                                            }
                                } //  ---------------------------------------------------------------------------------------------------
...
//                       FeM(indx)  -= det*_phi_g[2][i]*_phi_g[1][j]*_data_eq[_pres_order].ub[(_FF_idx[FS_F]) *NDOF_FEM+_pres_order*sur_toply[j]];
                                    }
                                } //  ---------------------------------------------------------------------------------------------------
