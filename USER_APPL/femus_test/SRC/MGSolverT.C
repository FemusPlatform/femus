...
{// ==================================================
 ...
 if(_FileMap ["SolveSteady"]!="") { _SolveSteady  = stoi(_FileMap["SolveSteady"]); }
   else { std::cout <<" Tproperties.in: default value for _T_parameter._SolveSteady (in UserT.h) \n"; }
  std::cout <<" Tproperties.in: default value for _T_parameter._SolverType  (in UserNS.h) \n"; 
  if(_FileMap ["DynamicUnderRelaxation"]!="") { _UnderRelaxation =stof(_FileMap["DynamicUnderRelaxation"]); }
  else { std::cout <<" Tproperties.in: default value for _T_parameter._DynamicUnderRelaxation (in UserNS.h)\n"; }
  if(_FileMap ["Supg"]!="")   _Supg= stoi(_FileMap["Supg"]); 
  else { std::cout <<" Tproperties.in:  default value for _T_parameter._Supg (in UserNS.h)\n"; }
  if(_FileMap ["Upwind"]!="")  _Upwind=stof(_FileMap["Upwind"]);
  else { std::cout <<" Tproperties.in: default value for _T_parameter._Upwind (in UserNS.h)\n"; }
  if(_FileMap ["ReactionNumberBased"]!="") { _ReactionNumberBased     = stoi(_FileMap["ReactionNumberBased"]); }
  else { std::cout <<" Tproperties.in: default value for _T_parameter._ReactionNumberBased  (in UserNS.h)\n"; }

  if(_FileMap ["FlatProfile"]!="")  _FlatProfile = stoi(_FileMap["FlatProfile"]);
  else { std::cout <<" Tproperties.in: default value for _T_parameter._FlatProfile (in UserNS.h)\n"; }
  if(_FileMap ["Prt"]!="") { _Prt=stof(_FileMap["Prt"]); }
  else { std::cout <<" Tproperties.in: default value for _T_parameter.Prt (in UserNS.h)\n"; }
...
void T_param::read_file(
  ...
  //  getting file name --------------------------------------------------------------------
  std::ostringstream file;   file << getenv("FEMUS_DIR")<<"/USER_APPL/"<<getenv("FM_MYAPP")<<"/DATA/Tproperties.in";
  ...
  } else { std::cerr << "T_param.read_file(): no parameter file found" << std::endl;   abort(); }
