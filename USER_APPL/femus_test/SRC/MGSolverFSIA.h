...
class MGSolFSIA:public MGSolDA
{
/// Class for mg Navier-Stokes equation  with name FSIA_EQUATIONS.
...
private:
  ...
  /*!
   * For each element, before performing the loop over boundary sides with #MGSolFSIA::set_bc_matrix,
   * we set the values of #MGSolFSIA::_bc_el equal to _BdFlagId in order to highlight boundary nodes
   * and to see if an equation has already been assigned to a matrix row. After looping over boundary sides
   * no row can have a value of _bc_el equal to _BdFlagId. See #MGSolFSIA::_bc_el for admissible values
   */
  const int _BdFlagId = -8;
  /// a) MGSolFSIA element data
  // mesh
...
//   //  fields at gaussian points
  ...
  // ======================  start functions ===================================
public:
    FSIA_param _FSIA_parameter;
//  DenseMatrixM _KeM;
//      DenseVectorM _FeM;
  /// b)   Init MGSolFSIA functions (constructor,destructor,external fields):
    MGSolFSIA (			///< Constructor
	      MGEquationsSystem & mg_equations_map,	///< equation map class (Mesh and parameters)
	      int nvars_in[],	///< KLQ number of variables
	      std::string eqname_in = "FSIA0",	///< base name system
  ...
  /// d)  Assemblying MGSolFSIA Operators
...
// Multigrid function ------------------------------------------------------------------
  ...
  // ==============================================================================================
  void matrixrhs_liq_vol (DenseMatrixM & KeM, DenseVectorM & FeM,
		     int el_ndof[],
		     double u_old[],
		     double u_nl[],
    ...
    // ==============================================================================================
  void matrixrhs_sol_vol (DenseMatrixM & KeM, DenseVectorM & FeM,
                     int el_ndof[],
                     double u_old[],
                     double u_nl[],
                     const int unsteady_flag,
                     const int mode, double MuTurb[], int el_conn[],int flag_group[],int iel);
   ...
   * The normal vector is taken from #MGSolFSIA::_normal_pt .
  ...
  void CalcTangDir (int PhiNumber,	///< [in] ID of node for which we are writing the equation - local element numbering
		    ...
		    int TangType   ///< [in] Method for calculation of tangent vectors, see #MGSolFSIA::TangType
                   );   
   ...
   * for normal and tangential directions. The flags are written into #MGSolFSIA::_bc_el
   */
  ...
  /*!
   * This function corrects the values of #MGSolFSIA::_bc_el <br>
   * It is used when #MGSolFSIA::_nNSdim == 3 in order to switch the rows taking the equations along 
