...
//         if(_FF_idx[K_F]>=0) {
//             if(_FSIA_parameter._WallFunctionApproach == 1) {
//                 _WallElement = 1;
//             }
...
//           _AlreadyWrittenDirBC[indx_row] = true;
                    ...
                    const double vart= bc_rhs*_data_eq[2].ub[indx_row+_FF_idx[FSA_F]*el_ndof[2]];
                    ...
                    if(OrDir) {
                        if(bd_node == 88 || bd_node == 66) {
                            ...
                            for(int   jvar=0; jvar< _nNSdim; jvar++)    {    //  -beta_n*n*( u.n)
                                ...
                                FeM(indx_row) += _data_eq[2].ub[(_FF_idx[FSA_F]) *NDOF_FEM + indj]*bc_rhs*alpha_beta;
                            }
                        }
                    } else {
    ...
    for(int  qp=0; qp< elb_ngauss; qp++) {  // START LOOP OVER GAUSS POINTS ===============================================
        ...
        for(int i=0; i< elb_ndof[2]; i++) {  // LOOP OVER TEST FUNCTIONS ===================================================
            ...
            if(bd_face==bd_node) {
                ...
                double lambda1=(1-pen_cond)*_FSIA_parameter._Penalty_tg;
                double beta=lambda1*_FSIA_parameter._Penalty_n;
                        ...
                        for(int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++) { // LOOP OVER VELOCITY COMPONENTS FOR EQUATION WRITTEN IN ROW indx
                            ...
                            if(_bc_el[indx] == -1 || _bc_el[indx] == -2) {
                                if(bc_vartg_flag !=1) {
                                    ...
                                    double tau = (_bc_el[indx] == -1) ? _FSIA_parameter._Tg1_stress:_FSIA_parameter._Tg2_stress ; //utau*utau/vel_bound;
...
//                       FeM(indx)  -= det*_phi_g[2][i]*_phi_g[1][j]*_data_eq[_pres_order].ub[(_FF_idx[FSA_F]) *NDOF_FEM+_pres_order*sur_toply[j]];
                                    }
                                } //  ---------------------------------------------------------------------------------------------------
