...
// ============================================
#ifdef CTRL_EQUATIONS // 3D-2D Energy equation
// ============================================
...
MGSolCTRL::MGSolCTRL(
  ...
  {//  =========================================================================
   ...
   _CTRL_parameter.read_param(_mgutils); 
   _ctrl_alpha=_CTRL_parameter._ctrl_alpha;
   _ctrl_beta=_CTRL_parameter._ctrl_beta;
    
  /// A) reading parameters  for field coupling (in _FF_idx[])
  _nDSdim=DIMENSION;
  _dir=0;
  if(!varname_in.compare("cx")) { _dir=0; }  if(!varname_in.compare("cy")) { _dir=1; }
  if(!varname_in.compare("cz")) { _dir=2; }
#if DIMENSION ==2
  ...
  for(int l=0; l<_NoLevels; l++) { _solver[l]->set_solver_type(_CTRL_parameter._SolverType); }

  /// D) Setting nondimensional parameters _alpha _IPrdl _IRe .....
    _SolveCTRL=(_mgutils._sim_config["SolveControl"].compare("yes") == 0) ? true: false;
  return;
}
...
#ifdef PRINT_INFO
  std::cout<< " Matrix Assembled(CTRL)  for  Level "<< Level << " dofs " << A[Level]->n() <<"\n";
#endif
...
/// A) Set up the time step
  ...
  /// C) Solution of the linear MGsystem (MGSolCTRL::MGSolve).
 if(_mgutils.get_name() != 1){
  ...
  /// D) Update of the old solution at the top Level  (MGSolCTRL::OldSol_update),
...
double  MGSolCTRL::MGFunctional(double p1,  double & p2) {
  // ===================================================================================================
  //                                    A) Set up                                                      
  // ===================================================================================================
  //
...
//       _fe[2]->get_ddphi_gl_g(_nFSIdim,qp,_InvJac2,_ddphi_g[2]);         // global second derivatives
      ...
      for(int idim=0; idim<  _nDSdim; idim++) {  // old  Velocity at gaussina point q
         ...
         value  += JxW_g2*_weight_ctrl[iel+nel_b]* sqrt((disp_g[0]-_disp_d) *(disp_g[0]-_disp_d)     );   // functional
...
// ===================================================
void CTRL_param::read_param(
  MGUtils &mgutils)
{// ==================================================

  read_file();  // Reading parameters from file CTRLproperties.in
  ...
  for(int i=0; i<_BoundaryGroupsIDs.size(); i++) {
    BDcond = "CTRLgroup"+to_string(_BoundaryGroupsIDs[i]);
    _map_CTRLgroup[_BoundaryGroupsIDs[i]]=_BoundMap[_FileMap[BDcond]];
  }
  // end boundary condition block --------------------------------------------------------
  
  // SETTING OTHER PARAMETERS ------------------------------------------------------------
    std::cout<<" CONTROL PARAMETER map (_CTRL_parameter) in  CTRLproperties.in + UserCTRL.h :\n";
//     std:cout << " solve"<< _FileMap["SolveSteady"];
 if(_FileMap ["SolveSteady"]!="") { _SolveSteady  = stoi(_FileMap["SolveSteady"]); }
   else { std::cout <<" CTRLproperties.in: default value for _CTRL_parameter._SolveSteady (in UserCTRL.h) \n"; }
  std::cout <<" CTRLproperties.in: default value for _CTRL_parameter._SolverType  (in UserCTRL.h) \n"; 
  if(_FileMap ["DynamicUnderRelaxation"]!="") { _UnderRelaxation =stof(_FileMap["DynamicUnderRelaxation"]); }
  else { std::cout <<" CTRLproperties.in: default value for _CTRL_parameter._DynamicUnderRelaxation (in UserCTRL.h)\n"; }
  if(_FileMap ["Supg"]!="")   _Supg= stoi(_FileMap["Supg"]); 
  else { std::cout <<" CTRLproperties.in:  default value for _CTRL_parameter._Supg (in UserCTRL.h)\n"; }
  if(_FileMap ["Upwind"]!="")  _Upwind=stof(_FileMap["Upwind"]);
  else { std::cout <<" CTRLproperties.in: default value for _CTRL_parameter._Upwind (in UserCTRL.h)\n"; }
  if(_FileMap ["ReactionNumberBased"]!="") { _ReactionNumberBased     = stoi(_FileMap["ReactionNumberBased"]); }
  else { std::cout <<" CTRLproperties.in: default value for _CTRL_parameter._ReactionNumberBased  (in UserCTRL.h)\n"; }

  if(_FileMap ["FlatProfile"]!="")  _FlatProfile = stoi(_FileMap["FlatProfile"]);
  else { std::cout <<" CTRLproperties.in: default value for _CTRL_parameter._FlatProfile (in UserCTRL.h)\n"; }
  if(_FileMap ["Prt"]!="") { _Prt=stof(_FileMap["Prt"]); }
  else { std::cout <<" CTRLproperties.in: default value for _CTRL_parameter.Prt (in UserCTRL.h)\n"; }
   if(mgutils._sim_config["ctrl_alpha"]!="") { _ctrl_alpha=stof ( mgutils._sim_config["ctrl_alpha"] ); }
  else {_ctrl_alpha=0.; std::cout <<" CTRLproperties.in: default value for _CTRL_parameter.ctrl_alpha (in UserCTRL.h)\n"; }
   if(mgutils._sim_config["ctrl_beta"]!="") { _ctrl_beta=stof ( mgutils._sim_config["ctrl_beta"] ); }
  else {_ctrl_beta=0.; std::cout <<" CTRLproperties.in: default value for _CTRL_parameter.ctrl_beta (in UserCTRL.h)\n"; }
...
void CTRL_param::read_file(
) {//  ===========================================================

  //  getting file name --------------------------------------------------------------------
  std::ostringstream file;   file << getenv("FEMUS_DIR")<<"/USER_APPL/"<<getenv("FM_MYAPP")<<"/DATA/CTRLproperties.in";
  ...
  } else { std::cerr << "CTRL_param.read_file(): no parameter file found" << std::endl;   abort(); }
...
#ifdef TBK_EQUATIONS
// =====================================================
void  MGSolCTRL::f_mu(double val[]) {
...
// =============================================================================================================
// double   MGSolCTRL::heff(double u_nlg_av[]) {
