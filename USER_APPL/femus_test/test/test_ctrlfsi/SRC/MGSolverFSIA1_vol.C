...
#include "MGSolverFSIA.h"       // Navier-Stokes class header file
// config file -------------------------------------------------
...
void MGSolFSIA::get_el_field_data(
  int iel, int Level,
  ...
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FSA_F]]->get_el_sol(_nNSdim,1,el_ndof[1],el_conn, offset,0,_data_eq[1].ub);    // pressure
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FSA_F]]->get_el_sol(0,_nNSdim,el_ndof[2],el_conn, offset,0,u_old);             // old vel
  _data_eq[2].mg_eqs[_data_eq[2].tab_eqs[FS_F]]->get_el_nonl_sol(0,_nNSdim,el_ndof[2],el_conn, offset,0,u_nl);         //  non linear vel
  return;
}

// ==============================================================================================
void MGSolFSIA::matrixrhs_liq_vol(
  ...
  double MuTurb[1];     double x_m[DIMENSION];
  double MuTurbDxg[DIMENSION];   double tang1[DIMENSION], tang2[DIMENSION];
  double rho=1.;
  int el_ngauss = _fe[2]->_NoGauss1[ _nNSdim-1];                //elem gauss points
  const int el_ndof2=el_ndof[2];
  double dt=_dt;    if(dt>1 || _FSIA_parameter._SolveSteady == 1) {  dt=1. ;    }    if(dt<1.e-10) {        dt=1.e-10;    }
  ...
  for(int qp=0; qp<  el_ngauss; qp++) {
    ...
    if(_nvars[1]>0) {
       ...
       double JxW_g1 =det2*_fe[1]->_weight1[ _nNSdim-1][qp]*dt;           // quadratic weight
    ...
    interp_el_gdx(ua_old,0, _nNSdim,_dphi_g[2],el_ndof2,vel_gdx);    // derivatives  vel_gdx[DIM][DIM]
    interp_el_gddx(ua_old,0, _nNSdim,_ddphi_g[2],el_ndof2,vel_gddx);
    ...
    for(int idim=0; idim<  _nNSdim; idim++) {
      vela_g[idim]=0.;  x_m[idim]=0.;           u_nlg[idim]=0.; // old and  non linear Velocity at gaussina point qp
      for(int k=0; k< NDOF_FEM; k++) {
        const int dnode=k+idim*NDOF_FEM;      x_m[idim] +=_xx_qnds[dnode]/el_ndof[2];
        u_nlg[idim] += u_nl[dnode]*_phi_g[2][k];     vela_g[idim] += ua_old[dnode]*_phi_g[2][k];
      }
    ...
    MuTurb[0]=0.;
    for(int dir=0; dir<_nNSdim; dir++) MuTurbDxg[dir] = 0.;
...
//     }
    ...
    for(int i=0; i< el_ndof2; i++) {  // LOOP OVER TEST FUNCTIONS --------------------------------------------------
      ...
      for(int  row_shift=0; row_shift< _nvars[2]; row_shift++)    {  // LOOP OVER ROWS: i + row_shift*el_ndof2
        ...
        if(BoundEquation && !NormalEquation) { CalcTangDir(i, el_ndof[2], _bc_el[indx], qp, tang1, tang2, 1); }
        ...
        if(_bc_el[indx] <=0) {
          for(int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++)     {  // Loop over velocity components for equation written in row indx
            ...
            double Phi_supg = 0.;
            if(_bc_el[indx] == 0 && _FSIA_parameter._Supg == 1)
            ...
            // -------------------------------------- Assemblying rhs ------------------------------------------------------
            if(mode == 1) {  // rsh  terms
              if(_FSIA_parameter._SolveSteady == 0) { FeM(indx)  +=  dtxJxW_g* rho*vela_g[ivar+_dir]* (phii_g+Phi_supg)/_dt; }
            ...
            for(int j=0; j<el_ndof2; j++) {
              ...
              if(_AxiSym==1) {  // axysimmetry only --------------------------------------------------
                Lap_g =2.* (1- (ivar+_dir)) *
                       (IRe_eff+0.*_FSIA_parameter._Upwind*f_upwind*u_nlg[0]*u_nlg[0])
                       *phij_g* (phii_g+Phi_supg) / (_xyzg[0]*_xyzg[0]);
              } // --------------------------------------------------------------------------------

              for(int kdim=0; kdim< _nNSdim; kdim++) {
                dphijdx_g2[kdim] =_dphi_g[2][j+kdim*el_ndof2];
                double up = _FSIA_parameter._Upwind*f_upwind*u_nlg[kdim]*u_nlg[kdim];
...
// ==============================================================================================
void MGSolFSIA::matrixrhs_sol_vol(
  DenseMatrixM &KeM, DenseVectorM &FeM,
  int el_ndof[],
  double ua_old[],
  double u_nl[],
  ...
  double vel_gddx[DIMENSION*DIMENSION*DIMENSION];
  double vel_gdx[DIMENSION*DIMENSION];
  double vela_g[DIMENSION];  //adjoint velocity
  double u_nlg[DIMENSION];   //FSI velocity
  double l_g[DIMENSION];  //displacement
  double MuTurb[1];     double x_m[DIMENSION];
  double MuTurbDxg[DIMENSION];   double tang1[DIMENSION], tang2[DIMENSION];
  double rho=1.;
  int el_ngauss = _fe[2]->_NoGauss1[ _nNSdim-1];                //elem gauss points
  ...
  for(int qp=0; qp<  el_ngauss; qp++) {
    ...
    // quadratic fields (velocity)
    interp_el_sol(_data_eq[2].ub,0,_data_eq[2].indx_ub[_data_eq[2].n_eqs],_phi_g[2],el_ndof2,_ub_g[2]);    // field _ub_g[2][DIM]
    interp_el_gdx(u_nl,0, _nNSdim,_dphi_g[2],el_ndof2,vel_gdx);    // derivatives  vel_gdx[DIM][DIM]
    interp_el_gddx(u_nl,0, _nNSdim,_ddphi_g[2],el_ndof2,vel_gddx);
    // linear fields (pressure) -> projection and segregated
    ...
    double mu=1.; // Velocity
    for(int idim=0; idim<  _nNSdim; idim++) {
      vela_g[idim]=0.;  x_m[idim]=0.;  
      u_nlg[idim]=0.;   l_g[idim]=0.;  // old and  non linear Velocity at gaussian point qp
      for(int k=0; k< NDOF_FEM; k++) {
        const int dnode=k+idim*NDOF_FEM;      x_m[idim] +=_xx_qnds[dnode]/el_ndof[2];
        u_nlg[idim] += u_nl[dnode]*_phi_g[2][k];     vela_g[idim] += ua_old[dnode]*_phi_g[2][k];
	l_g[idim] +=_data_eq[2].ub[k+(idim+_FF_idx[SDSX_F])*NDOF_FEM]*_phi_g[2][k];// displacement
      }
      mod2_vel += u_nlg[idim]*u_nlg[idim]; // module velocity
      for(int jdim=0; jdim<  _nNSdim; jdim++) {
        _sP += 0.25* (vel_gdx[ jdim*_nNSdim+idim] +vel_gdx[ idim*_nNSdim+jdim]) *
    ...
    for(int dir=0; dir<_nNSdim; dir++) MuTurbDxg[dir] = 0.;
    // -------------------- Turbulence [K_F] -> (quad,_indx_eqs[K_F]) -------------------------------------
    double IRe_eff = _mus*mu;
    double f_upwind = CalcFUpwind(u_nlg, _dphi_g[2], IRe_eff, _nNSdim, el_ndof2);
    //     if (_xyzg[1]>-0.05 && _xyzg[1]<0.05) IRe_eff*=100;
    double lambda  =_lambda*_FSIA_parameter._Compressible; 
    // -------------------- Temperature[T_F] -> (quad,_indx_eqs[T_F]) -------------------------------------
//     if (_FF_idx[T_F]>=0) {
//       double Temp_g=_ub_g[2][_FF_idx[T_F]];
//       rho = (11065-1.293* (Temp_g+273.15)) / (_rhof);
    ...
    for(int i=0; i< el_ndof2; i++) {  // LOOP OVER TEST FUNCTIONS --------------------------------------------------
      ...
      for(int  row_shift=0; row_shift< _nvars[2]; row_shift++)    {  // LOOP OVER ROWS: i + row_shift*el_ndof2
        ...
        if(_bc_el[indx] <=0) {
	      ...
	      FeM(indx)  +=  -(1-ivar)*_weight_ctrl[iel]*(l_g[0]-0.07)*(phii_g + Phi_supg);    //functional 
// 	    if(el_conn[i]==94)  FeM(indx)  +=  (1-ivar)*100*(l_g[0]-0.05)*(phii_g + Phi_supg);    //functional 			     
            }
            // -------------------------------------------------------------------------------------------------------------
            //-------------------------------------- Assemblying matrix ----------------------------------------------------
            for(int j=0; j<el_ndof2; j++) {
              ...
              for(int kdim=0; kdim< _nNSdim; kdim++) {
                ...
                Supg_lap  += (IRe_eff+up) *_ddphi_g[2][j* _nNSdim* _nNSdim+kdim* _nNSdim+kdim];
              }
              ...
              KeM(indx,j+ivar*el_ndof2) += dtxJxW_g*rho* (
                                             -1*Adv_g * (phii_g+Phi_supg) // time + advection
...
//                 FeM(indx) -= IRe_eff*part*dphiidx_g2[ivars]*_data_eq[2].ub[_FF_idx[SDSX_F+ivars]*NDOF_FEM+j];
              ...
              //_Compressible
	       if (_FSIA_parameter._Compressible==1){
...
//                  FeM(indx)-=lambda*partC*dphiidx_g2[ivar+_dir]*_data_eq[2].ub[_FF_idx[SDSX_F+ivars]*NDOF_FEM+j];//	 
            ...
            for(int  jp=0; jp<el_ndof[1]; jp++) {
              ...
              //  B^T diagonal term
              KeM(indx,jp+ _nNSdim*el_ndof2) += dtxJxW_g*rho*(1-_FSIA_parameter._Compressible)*(  // MPascal
                                                  -1.*_phi_g[1][jp]*_dphi_g[2][i+ivar*el_ndof2] 
                                                +  _dphi_g[1][jp+ivar*el_ndof[1]]* (0.*phii_g + Phi_supg)
                                                );
              //  B^T non-diagonal term
   ...
   //------------------    QL    -----------------------------------------------------------
	...
	int i_flag_i= (fabs(flag_group[i])>999 && ikl>0 )?0:1;
        const double dtxJxWp_g = (_bc_el[indp] != 0) ? JxW_g2*(1-_FSIA_parameter._Compressible) : 0.;
           KeM(indp,indp) +=JxW_g2*_FSIA_parameter._Compressible*i_flag_i;	
        for(int j=0; j<el_ndof2; j++)     {
          for(int  jvar=0; jvar< _nvars[2]; jvar++) {    // linear -quad
            if(_AxiSym ==1 && _nNSdim == 2) {
              const double phij_g= _phi_g[2][j];
...
void MGSolFSIA::CalcTangDir(
  ...
  if(Type==1) {
    ...
    for(int kk=0; kk<DIMENSION; kk++) {
      ...
      Vel[kk]     = _data_eq[2].ub[(_FF_idx[FSA_F] + kk) *el_dof + (NDOF_FEM -1)];
