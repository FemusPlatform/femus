...
void MGSolNS::matrixrhsvol(
  ...
  double dt=_dt;    if(dt>1 || _NS_parameter._SolveSteady == 1) {  dt=1. ;    }    if(dt<1.e-10) {        dt=1.e-10;    }
...
//
//       if (_NS_parameter._WallFunctionApproach == 1 || _NS_parameter._InterpolatedMuTurb == 1) _mu_turb = MuTurb[0];
    }
      ...
      for(int  row_shift=0; row_shift< _nvars[2]; row_shift++)    {  // LOOP OVER ROWS: i + row_shift*el_ndof2
        ...
        if(_bc_el[indx] <=0) {
          for(int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++)     {  // Loop over velocity components for equation written in row indx
            ...
            double Phi_supg = 0.;
            if(_bc_el[indx] == 0 && _NS_parameter._Supg == 1)
            ...
            // -------------------------------------- Assemblying rhs ------------------------------------------------------
            if(mode == 1) {  // rsh  terms
              if(_NS_parameter._SolveSteady == 0) { FeM(indx)  +=  dtxJxW_g* rho*vel_g[ivar+_dir]* (phii_g+Phi_supg)/_dt; }
            ...
            for(int j=0; j<el_ndof2; j++) {
              ...
              if(_AxiSym==1) {  // axysimmetry only --------------------------------------------------
                Lap_g =2.* (1- (ivar+_dir)) *
                       (IRe_eff+0.*_NS_parameter._Upwind*f_upwind*u_nlg[0]*u_nlg[0])
                       *phij_g* (phii_g+Phi_supg) / (_xyzg[0]*_xyzg[0]);
              } // --------------------------------------------------------------------------------

              for(int kdim=0; kdim< _nNSdim; kdim++) {
                dphijdx_g2[kdim] =_dphi_g[2][j+kdim*el_ndof2];
                double up = _NS_parameter._Upwind*f_upwind*u_nlg[kdim]*u_nlg[kdim];
              ...
              for(int  ivars=0; ivars<_nvars[2]; ivars++) {
                ...
                if(_FF_idx[K_F]>=0 && _NS_parameter._InterpolatedMuTurb==1) {
