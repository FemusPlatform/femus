...
MGSolIB::MGSolIB(
  ...
  if (!varname_in.compare("Col")) {
    std::cout << "Activated system for equation " << eqname_in << " with variable " << varname_in
              << std::endl;
    _dir = 0;  // kappa
  ...
  for (int l = 0; l < _NoLevels; l++) { _solver[l]->set_solver_type(BICGSTABM); }
