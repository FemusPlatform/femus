...
IbUtils::IbUtils(
    MEDCoupling::MEDCouplingUMesh* SolidBodyMesh, const MEDCoupling::MEDCouplingUMesh* FluidMesh, int proc)
  ...
  for (int i = 0; i < DIMENSION; i++) _Velocity[i] = 0.;
  _iter = 0;
}

IbUtils::IbUtils(
    MEDCoupling::MEDCouplingUMesh* SolidBodyMesh, const MEDCoupling::MEDCouplingUMesh* FluidMesh, int p_femus,
    int proc)
    : InterfaceProjection(SolidBodyMesh, FluidMesh, proc, Volume),
      _Proc(proc),
      _ExtractedMesh(NULL),
...
void IbUtils::SetMeshes(
  ...
  if (_Proc == 0) MEDCoupling::WriteField("cell_color.med", _FluidCellColor, true);

  _SolidVelocity = MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_NODES);
  ...
  _TriangulatedNodesIds = MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_NODES);
  return;
}

void IbUtils::InitColor() {
  MEDCoupling::DataArrayDouble* MuFieldArray2 = MEDCoupling::DataArrayDouble::New();
  MuFieldArray2->alloc(_LinearSolidBodyMesh->getNumberOfCells(), 1);
  MuFieldArray2->fillWithValue(1.);
  _SolidCellColor = MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_CELLS);
  ...
  MEDCoupling::DataArrayDouble* MuFieldArray = MEDCoupling::DataArrayDouble::New();
  MuFieldArray->alloc(_SolidBodyMesh->getNumberOfNodes(), 1);
  MuFieldArray->fillWithValue(1.);
  _SolidNodeColor = MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_NODES);
...
void IbUtils::CalcInterface(MEDCoupling::MEDCouplingFieldDouble* ProjectedColor) {
  int Conn[4][4];
  Conn[1][0] = 4;
  Conn[1][1] = 1;
  Conn[1][2] = 5;
  ...
  _TriangulatedCellsPerSourceCell.clear();
  _iter++;
  double* Color = const_cast<double*>(_FluidCellColor->getArray()->getPointer());
  ...
  for (int i = 0; i < cells; i++) {
    if (Color[i] > 1.1) {
      Color[i] *= 0.5;
      std::cerr << "IBMover: Cell " << i << "  Color Value  " << Color[i] * 2. << std::endl;
    }
    if (Color[i] > _LowerThreshold && Color[i] < _UpperThreshold) InterfaceCells.push_back(i);
  ...
  if (_ExtractedColor != NULL) _ExtractedColor->decrRef();
  _ExtractedColor = ProjectedColor->buildSubPart(CellId, CellId + InterfaceCells.size());
  double* ExNodeColor = const_cast<double*>(_ExtractedColor->getArray()->getPointer());
  if (_ExtractedMesh != NULL) _ExtractedMesh->decrRef();
  _ExtractedMesh = _FluidMesh->buildPartOfMySelf(CellId, CellId + InterfaceCells.size());
  ...
  for (int j = 0; j < InterfaceCells.size(); j++) {
    ...
    for (int k = 0; k < CNodes; k++) {
      ...
      _ExtractedMesh->getCoordinatesOfNode(Connectivity[k], NodeCoordinates);
      for (int d = 0; d < dim; d++) {
        Coordinates[k + d * CNodes] = NodeCoordinates[d];
        FullCoordinates[k + d * (CNodes + 2)] = NodeCoordinates[d];
        if (k < 4) _CoordMatrix.push_back(NodeCoordinates[d]);
      }
    ...
    double xmin = (Coordinates[0] < Coordinates[2]) ? Coordinates[0] : Coordinates[2];
    double xmax = (Coordinates[0] > Coordinates[2]) ? Coordinates[0] : Coordinates[2];
    double ymin = (Coordinates[0 + CNodes] < Coordinates[2 + CNodes]) ? Coordinates[0 + CNodes]
                                                                      : Coordinates[2 + CNodes];
    double ymax = (Coordinates[0 + CNodes] > Coordinates[2 + CNodes]) ? Coordinates[0 + CNodes]
                                                                      : Coordinates[2 + CNodes];
    ...
    const double tri_area = (Case == 0) ? dym * dym * 0.5 / pen : dxm * dxm * 0.5 * pen;
    ...
    if (case2 == 1) case2 = (col_area > (par_area + tri_area)) ? 2 : 1;
    ...
    int LastNo = -1;
    for (int l = 0; l < NDOF_FEM; l++) {  // CALCULATION OF FLUID NODES: scalar product between distance
      // node-interaface node and interface normal
      double scal_prod = 0.;
      for (int dir = 0; dir < DIMENSION; dir++) {
        scal_prod += (Coordinates[l + dir * NDOF_FEM] - point1[dir]) * norm[dir];
      }
      if (scal_prod > 0.) {
        fluid_nodes.push_back(l);
        if (l == 8) center_included = 1;
        //                 for ( int dir=0; dir<DIMENSION; dir++ ) TriangulationCoords.push_back (
        //                 Coordinates[l +  dir*NDOF_FEM] );
        if (l != 8) Flag[l] = 1;
      } else {
    ...
    for (int l = LastNo; l < NDOF_FEM - 1 + LastNo;
         l++) {  // ordering elem connectivity from node close to interface
      int pos = l % (NDOF_FEM - 1);
      if (Flag[Order[pos]] == 1) OrderedNodes.push_back(Order[pos]);
    }

    std::vector<int> FinalConn;
    int rectangular_subdomains, act_fluid_nodes;
    if (center_included == 1) {  // SEARCHING FOR QUAD4 SUB ELEMENTS
      ...
      for (int rec = 0; rec < 4; rec++) {
        ...
        if (fluid_rec == 1) fluid_quad4.push_back(rec);
      }
      if (fluid_quad4.size() > 0) {
        ...
        for (int r = 0; r < numRec; r++) {  // ADDING RECTANGULAR SUB ELEMENTS
          std::vector<int> Sequence;
          for (int node = 0; node < 4; node++) {
            Sequence.push_back(Conn[fluid_quad4[r]][node]);
            SourceIDs.push_back(SourceConnectivity[Conn[fluid_quad4[r]][node]]);
          }
          InsertTriangulatedCell(
              InterfaceCells[j], NodesPerCell, ElemenConn, Sequence, TriangulationCoords, Coordinates,
              NDOF_FEM);
          //                     _TriangulatedCellsPerSourceCell[InterfaceCells[j]].push_back (
          //                     NodesPerCell.size() ); NodesPerCell.push_back (
          //                     NodesPerCell[NodesPerCell.size() -1] + 4 ); for ( int node=0; node<4; node++
          //                     )
          //                         for ( int dim=0; dim<DIMENSION; dim++ )
          //                             TriangulationCoords.push_back (
          //                             Coordinates[Conn[fluid_quad4[r]][node] +  dim*NDOF_FEM] );
        }
      ...
      fluid_quad4.clear();
    } else {  // no presence of rectangles
      rectangular_subdomains = 0;
      for (int l = 0; l < OrderedNodes.size(); l++) FinalConn.push_back(OrderedNodes[l]);
    }

    OrderedNodes.clear();
    OrderedNodes.resize(FinalConn.size() + 2);
    int First, Last;
    if (fabs(FullCoordinates[FinalConn[0]] - FullCoordinates[NDOF_FEM]) < 1.e-10 ||
        fabs(FullCoordinates[FinalConn[0] + NDOF_FEM + 2] - FullCoordinates[NDOF_FEM + NDOF_FEM + 2]) <
            1.e-10) {
      First = NDOF_FEM;
      Last = NDOF_FEM + 1;
    ...
    for (int i = 0; i < FinalConn.size(); i++) OrderedNodes[i + 1] = FinalConn[i];

    act_fluid_nodes = FinalConn.size();
    //         std::cout<<"Polygonal cell with "<<rectangular_subdomains<<" linear quad4 and
    //         "<<act_fluid_nodes<<" remaining fluid nodes \n"; for ( int i=0; i<FinalConn.size() +2; i++ )
    //         std::cout<<OrderedNodes[i]<<" "; std::cout<<"\n";
    ...
    switch (act_fluid_nodes) {
      case 5: {
        std::vector<int> Sequence1{0, 1, 2, 3};
        std::vector<int> Sequence2{3, 4, 5, 6};
        std::vector<int> Sequence3{0, 3, 6};
        InsertTriangulatedCell(
            InterfaceCells[j], NodesPerCell, OrderedNodes, Sequence1, TriangulationCoords, FullCoordinates,
            offset);
        InsertTriangulatedCell(
            InterfaceCells[j], NodesPerCell, OrderedNodes, Sequence2, TriangulationCoords, FullCoordinates,
            offset);
        InsertTriangulatedCell(
            InterfaceCells[j], NodesPerCell, OrderedNodes, Sequence3, TriangulationCoords, FullCoordinates,
            offset);
        for (int j = 0; j < Sequence1.size(); j++)
          SourceIDs.push_back(
              (OrderedNodes[Sequence1[j]] >= NDOF_FEM) ? -(1 + OrderedNodes[Sequence1[j]] % NDOF_FEM)
                                                       : SourceConnectivity[OrderedNodes[Sequence1[j]]]);
        for (int j = 0; j < Sequence2.size(); j++)
          SourceIDs.push_back(
              (OrderedNodes[Sequence2[j]] >= NDOF_FEM) ? -(1 + OrderedNodes[Sequence2[j]] % NDOF_FEM)
                                                       : SourceConnectivity[OrderedNodes[Sequence2[j]]]);
        for (int j = 0; j < Sequence3.size(); j++)
          SourceIDs.push_back(
              (OrderedNodes[Sequence3[j]] >= NDOF_FEM) ? -(1 + OrderedNodes[Sequence3[j]] % NDOF_FEM)
                                                       : SourceConnectivity[OrderedNodes[Sequence3[j]]]);
        Sequence1.clear();
        Sequence2.clear();
        Sequence3.clear();
      } break;
      case 4: {
        ...
        if (rectangular_subdomains == 2) {
          ...
          if (fabs(FullCoordinates[OrderedNodes[0]] - FullCoordinates[OrderedNodes[2]]) < 1.e-10 ||
              fabs(FullCoordinates[OrderedNodes[0] + offset] - FullCoordinates[OrderedNodes[2] + offset]) <
                  1.e-10) {
            Sequence1.resize(3);
            Sequence1 = {3, 4, 5};
            Sequence2.resize(4);
      ...
      case 3: {
        ...
        if (rectangular_subdomains == 0) {
          if (fabs(FullCoordinates[OrderedNodes[1]] - FullCoordinates[OrderedNodes[3]]) < 1.e-10 ||
              fabs(FullCoordinates[OrderedNodes[1] + offset] - FullCoordinates[OrderedNodes[3] + offset]) <
                  1.e-10) {
            ...
            Sequence2 = {4, 2, 3};
            InsertTriangulatedCell(
                InterfaceCells[j], NodesPerCell, OrderedNodes, Sequence3, TriangulationCoords,
        ...
        } else if (rectangular_subdomains == 2 || rectangular_subdomains == 3) {
          std::vector<int> Sequence3 = {0, 1, 2};
          Sequence1.resize(3);
          Sequence1 = {0, 2, 4};
          Sequence2.resize(3);
      ...
      case 2: {
        ...
        if ((fabs(FullCoordinates[OrderedNodes[0]] - FullCoordinates[OrderedNodes[1]]) > 1.e-10 &&
             fabs(FullCoordinates[OrderedNodes[0] + offset] - FullCoordinates[OrderedNodes[1] + offset]) >
                 1.e-10) ||
            (fabs(FullCoordinates[OrderedNodes[2]] - FullCoordinates[OrderedNodes[3]]) > 1.e-10 &&
             fabs(FullCoordinates[OrderedNodes[2] + offset] - FullCoordinates[OrderedNodes[3] + offset]) >
                 1.e-10)) {
      ...
      default:
        std::cout << "Unknown case with " << act_fluid_nodes << " active fluid nodes \n";
        //        int a=1;
        break;
    }
  }
  ...
  if (_TriangulatedNearBoundary != NULL) _TriangulatedNearBoundary->decrRef();
  _TriangulatedNearBoundary = MEDCoupling::MEDCouplingUMesh::New("RefinedMesh2", 2);
  MEDCoupling::DataArrayDouble* TrianCoordArray = MEDCoupling::DataArrayDouble::New();
  MEDCoupling::DataArrayDouble* SourceConnArray = MEDCoupling::DataArrayDouble::New();
  TrianCoordArray->alloc(NewNodes, DIMENSION);
  SourceConnArray->alloc(NewNodes, 1);
  _TriangulatedNearBoundary->allocateCells(NewCells);
  for (int i = 0; i < NewCells; i++) {  // REFINED CELLS ================================
    ...
    for (int k = 0; k < nodes; k++) {
      ...
      for (int d = 0; d < DIMENSION; d++)
        TrianCoordArray->setIJ(new_node_id, d, TriangulationCoords[new_node_id * DIMENSION + d]);
      SourceConnArray->setIJ(new_node_id, 0, SourceIDs[new_node_id]);
    }
    if (nodes == 4)
      _TriangulatedNearBoundary->insertNextCell(INTERP_KERNEL::NORM_QUAD4, nodes, connect);
    else if (nodes == 3)
      _TriangulatedNearBoundary->insertNextCell(INTERP_KERNEL::NORM_TRI3, nodes, connect);
    else
      _TriangulatedNearBoundary->insertNextCell(INTERP_KERNEL::NORM_POLYGON, nodes, connect);
    delete[] connect;
  }
  ...
  //     MEDLoader::WriteUMesh ( "RESU_MED/Triangulation_"+to_string ( _iter ) +".med",
  //     _TriangulatedNearBoundary, true );
  ...
  //     if(_Proc==0)MEDLoader::WriteField ( "RESU_MED/Triangulation_"+to_string ( _iter ) +".med",
  //     _TriangulatedNodesIds, true );

  int connect[2];
  int Intcells = InterfaceCoords.size() / (DIMENSION * 2);
  int Intnodes = InterfaceCoords.size() / (DIMENSION);

  if (_ReconInterface != NULL) _ReconInterface->decrRef();
  ...
  for (int i = 0; i < Intcells; i++) {  // REFINED CELLS ================================
    for (int k = 0; k < 2; k++) {
      int new_node_id = k + i * 2;
      connect[k] = new_node_id;
      for (int d = 0; d < DIMENSION; d++) {
        CoordArray->setIJ(connect[k], d, InterfaceCoords[2 * i * DIMENSION + k + d * 2]);
      }
    }
...
void IbUtils::CalcInterface2(MEDCoupling::MEDCouplingFieldDouble* ProjectedColor) {
  int Conn[4][4];
  Conn[1][0] = 4;
  Conn[1][1] = 1;
  Conn[1][2] = 5;
  ...
  for (int j = 0; j < InterfaceCells.size(); j++) {
    ...
    for (int k = 0; k < CNodes; k++) {
      for (int d = 0; d < dim; d++) {
        FullCoordinates_relative[k + d * (CNodes + 2)] =
            FullCoordinates[k + d * (CNodes + 2)] - FullCoordinates[d * (CNodes + 2)];
      }
    }
    ...
    diag02 = FullCoordinates_relative[2 + (CNodes + 2)] / (FullCoordinates_relative[2]);
    diag13 = (FullCoordinates_relative[3 + (CNodes + 2)] - FullCoordinates_relative[1 + (CNodes + 2)]) /
             (FullCoordinates_relative[3] - FullCoordinates_relative[1]);
    ...
    int RotCase = 0;
    if (int_incl > min_diag && int_incl < max_diag) RotCase = 1;
    ...
    double min_x, max_x;
    double x2, x3, y2, y3;
    if (RotCase == 0) {  // point 1 will be at (1,0) with normalized coordinates
      for (int k = 0; k < CNodes; k++) {
        ...
        FullCoordinates_relative[k + (CNodes + 2)] = (oldy * cos01 - oldx * sin01);
      }
    ...
    } else {  // point 3 will be at (-1,0) with normalized coordinates
      for (int k = 0; k < CNodes; k++) {
        double oldx = FullCoordinates_relative[k];
        double oldy = FullCoordinates_relative[k + (CNodes + 2)];
        FullCoordinates_relative[k] = (-oldx * cos03 - oldy * sin03);
        FullCoordinates_relative[k + (CNodes + 2)] = (-oldy * cos03 + oldx * sin03);
      }
    ...
    CutTypeT1 = 0;
    if (tilde_xt1 < min_x) {
      tilde_xt1 = (y3 - pen_23 * x3 + new_pen * min_x) / (new_pen - pen_23);
    ...
    CutTypeT2 = 0;
    if (tilde_xt2 > max_x) {
      tilde_xt2 = (y3 - pen_23 * x3 + new_pen * max_x) / (new_pen - pen_23);
    ...
    double net_area;
    if (dir_nx == 1) {
      if (col_area < AreaT1) {  // T1
        AreaCase = 0;
        net_area = col_area;
      } else if (col_area > AreaT1 && col_area < (AreaT1 + TrapArea)) {  // Trapezoid
        AreaCase = 1;
      ...
      if (col_area < AreaT2) {  // T2
        AreaCase = 2;
        net_area = col_area;
      } else if (col_area > AreaT2 && col_area < (AreaT2 + TrapArea)) {  // Trapezoid
        AreaCase = 1;
    ...
    if (AreaCase == 1) {  // Trapezoid
      ...
      if (fabs(pen_23) > 1.e-8) {
        ...
        double C = (Trap[0] - aux_x) * Trap[7] - 2 * (AuxArea + sign * task_area) / sign;
      ...
      } else {  // Considered parallelogramm
        double height = y2;
    ...
    if (AreaCase == 0) {  // Triangle T1
      double task_area = (dir_nx == 1) ? net_area : AreaT1 - net_area;

      double ratio = task_area / AreaT1;

      if (CutTypeT1 == 0) {  // tildex1 on x0-x1 -- interface crossing P0P3 and P0TildeX
    ...
    if (AreaCase == 2) {  // Triangle T2
      double task_area = (dir_nx == 1) ? AreaT2 - net_area : net_area;
      double ratio = task_area / AreaT2;

      if (CutTypeT2 == 0) {  // tildex1 on x0-x1 -- interface crossing P1P2 and P1TildeX
        ...
        intP2[1] = 0. + sqrt(ratio) * dyP2;
        //                 std::cout<<" cut0, start pen: "<<new_pen<<"   interface pen: "<< (
        //                 intP2[1]-intP1[1] ) / ( intP2[0]-intP1[0] ) <<std::endl;
      }
      if (CutTypeT2 == 1) {
        ...
        //                 std::cout<<" cut1, start pen: "<<new_pen<<"   interface pen: "<< (
        //                 intP2[1]-intP1[1] ) / ( intP2[0]-intP1[0] ) <<std::endl;
      }
    }
    ...
    if (RotCase == 0) {  // point 1 will be at (1,0) with normalized coordinates
      point1[0] = intP1[0] * cos01 - intP1[1] * sin01 + FullCoordinates[0];
      point1[1] = intP1[1] * cos01 + intP1[0] * sin01 + FullCoordinates[CNodes + 2];
      point2[0] = intP2[0] * cos01 - intP2[1] * sin01 + FullCoordinates[0];
      point2[1] = intP2[1] * cos01 + intP2[0] * sin01 + FullCoordinates[CNodes + 2];
    } else {  // point 3 will be at (-1,0) with normalized coordinates
      point1[0] = -intP1[0] * cos03 + intP1[1] * sin03 + FullCoordinates[0];
      point1[1] = -intP1[1] * cos03 - intP1[0] * sin03 + FullCoordinates[CNodes + 2];
      point2[0] = -intP2[0] * cos03 + intP2[1] * sin03 + FullCoordinates[0];
      point2[1] = -intP2[1] * cos03 - intP2[0] * sin03 + FullCoordinates[CNodes + 2];
    }
...
void IbUtils::MoveMeshWithStress(MEDCoupling::MEDCouplingFieldDouble* VelocityField, double dt) {
  std::cout << " -----------------------------------------------\n \
...
----------------------------------------------- \n";
  ...
  for (int i = 0; i < DIMENSION; i++) {
    acceleration[i] = Stress[i] / (_SolidArea * _rhos) /*+ _buoyancy*gravity[i]* ( 1-_rhof/_rhos ) */;
...
#if DIMENSION == 2
  _SolidVelocity->applyFunc(
      DIMENSION,
      "IVec*(x+" + to_string(acceleration[0] * dt) + ") + JVec*(y+" + to_string(acceleration[1] * dt) + ")");
#endif
#if DIMENSION == 3
  _SolidVelocity->applyFunc(
      DIMENSION, "IVec*(x+" + to_string(acceleration[0] * dt) + ") + JVec*(y+" +
                     to_string(acceleration[1] * dt) + ")  + KVec*(z+" + to_string(acceleration[2] * dt) +
                     ")");
#endif
...
void IbUtils::ComputeStress(MEDCoupling::MEDCouplingFieldDouble* ReducedVelocityField, double Stress[]) {
  ...
  for (int dir = 0; dir < DIMENSION; dir++) ViscStress[dir] = PressContrib[dir] = 0.;

  const int WeightDim = DIMENSION - 2;
  const int QPoints = _fe[2]->_NoGauss1[WeightDim];

  double InvJac[DIMENSION * DIMENSION], phi_g[3], xyz_g[DIMENSION], CanPos[DIMENSION],
      vel_dx_g[DIMENSION * DIMENSION], vel_on_nodes[DIMENSION * NDOF_FEM], Tensor[DIMENSION][DIMENSION],
      Normal[DIMENSION], Tangent[DIMENSION], VelTg[NDOF_FEM], Vel_tg_grad_norm_g;
  ...
  for (int cell = 0; cell < Cells; cell++) {  // loop over extracted mesh cells
    std::vector<int> conn;
    ...
    for (int s = 0; s < NDOF_FEM; s++) {
      ...
      for (int d = 0; d < DIMENSION; d++) {
        ...
        if (s < _SrcCoordInterpNodes) _CoordMatrix.push_back(coord[d]);
      }
    }

    double xmin = (FEM_coord[0] < FEM_coord[2]) ? FEM_coord[0] : FEM_coord[2];
    double ymin = (FEM_coord[0 + NDOF_FEM] < FEM_coord[2 + NDOF_FEM]) ? FEM_coord[0 + NDOF_FEM]
                                                                      : FEM_coord[2 + NDOF_FEM];
    double xmax = (FEM_coord[0] > FEM_coord[2]) ? FEM_coord[0] : FEM_coord[2];
    double ymax = (FEM_coord[0 + NDOF_FEM] > FEM_coord[2 + NDOF_FEM]) ? FEM_coord[0 + NDOF_FEM]
                                                                      : FEM_coord[2 + NDOF_FEM];
    ...
    for (int k = 0; k < NDOF_FEM; k++)           // node
      for (int dir = 0; dir < DIMENSION; dir++)  // direction
        vel_on_nodes[k + dir * NDOF_FEM] = Vel[FluidCellConn[k] * (DIMENSION + 1) + dir];

    for (int n = 0; n < NDOF_P; n++) Pressure[n] = Vel[FluidCellConn[n] * (DIMENSION + 1) + DIMENSION];
    ...
    if (fabs(_XiEtaChi[0]) < 1. + eps && fabs(_XiEtaChi[1]) < 1. + eps) {
      ...
      Pa = 0.;
      for (int n = 0; n < NDOF_P; n++) {  // number of dof
    ...
    for (int qp = 0; qp < QPoints; qp++) {
      ...
      for (int l = 0; l < DIMENSION * DIMENSION; l++) vel_dx_g[l] = 0.;
      for (int i = 0; i < DIMENSION; i++)     // velocity component  ->  [ux uy vx vy]
        for (int j = 0; j < DIMENSION; j++)   // direction of derivative
          for (int n = 0; n < NDOF_FEM; n++)  // number of dof
            vel_dx_g[i * DIMENSION + j] += vel_on_nodes[n + i * NDOF_FEM] * dphi[n + j * NDOF_FEM];

      Vel_tg_grad_norm_g = 0;
      for (int j = 0; j < DIMENSION; j++)   // direction of derivative
        for (int n = 0; n < NDOF_FEM; n++)  // number of dof
          Vel_tg_grad_norm_g += VelTg[n] * dphi[n + j * NDOF_FEM] * Normal[j];
      ...
      for (int i = 0; i < DIMENSION; i++) Stress[i] += ViscStress[i] - PressContrib[i];
  ...
  //       std::cout<<" ==================================== \n";
  //       std::cout<< "cd:  "<< cd2 <<"  cl: "<<cl2<<std::endl;
  //
  //     std::cerr<<" ==================================== \n";
  if (_Proc == 0) std::cerr << "   " << Pa - Pb << "   " << GlobViscContrib << "   " << GlobPressContrib;
  return;
}

void IbUtils::read_file(int p_femus) {
  ...
  if (p_femus == 0)
    ...
    file2 << getenv("APP_PATH") << "/DATA/DATA" << p_femus << "/MaterialProperties.in";
  ...
  for (int i = 0; i < FILES.size(); i++) {
    ...
    if (fin.is_open()) {
      ...
      std::cerr << "IBMover.read_file(): no parameter file found\t ->" << FILES[i] << std::endl;
      abort();
    }
    std::cout << "IBMover.read_file() End Reading file " << FILES[i] << std::endl;
    fin.close();
  }
  ...
  if (_FileMap["rho0"] != "") {
    ...
    _rhof = 1;
    std::cout << " IBMover: Fluid density not found. Assuming rho_fluid = 1 \n";
    std::cerr << " IBMover: Fluid density not found. Assuming rho_fluid = 1 \n";
  }
  if (_FileMap["mu0"] != "") {
    _muf = stof(_FileMap["mu0"]);
  } else {
    _muf = 1;
    std::cout << " IBMover: Fluid viscosity not found. Assuming mu_fluid = 1 \n";
    std::cerr << " IBMover: Fluid viscosity not found. Assuming mu_fluid = 1 \n";
  }
  if (_FileMap["buoyancy"] != "") {
    _buoyancy = stof(_FileMap["buoyancy"]);
  } else {
    _buoyancy = 0;
    std::cout << " IBMover: Buoyancy flag not found. Assuming no buoyancy \n";
    std::cerr << " IBMover: Buoyancy flag not found. Assuming no buoyancy \n";
  }
  if (_FileMap["rhosolid"] != "") {
    _rhos = stof(_FileMap["rhosolid"]);
  } else {
    _rhos = _rhof;
    std::cout << " IBMover: Solid density not found. Assuming rho_solid equal to rho_fluid \n";
    std::cerr << " IBMover: Solid density not found. Assuming rho_solid equal to rho_fluid \n";
  }
  if (_FileMap["LowerThreshold"] != "") {
    _LowerThreshold = stof(_FileMap["LowerThreshold"]);
  } else {
    _LowerThreshold = 0.001;
    std::cout << " IBMover: LowerThreshold not found. Assuming default 0.001 \n";
    std::cerr << " IBMover: LowerThreshold not found. Assuming default 0.001 \n";
  }
  if (_FileMap["UpperThreshold"] != "") {
    _UpperThreshold = stof(_FileMap["UpperThreshold"]);
  } else {
    _UpperThreshold = 0.999;
    std::cout << " IBMover: UpperThreshold not found. Assuming default 0.999 \n";
    std::cerr << " IBMover: UpperThreshold not found. Assuming default 0.999 \n";
  }
  if (_FileMap["SimpleIBMethod"] != "") {
    _SimpleIBMethod = stoi(_FileMap["SimpleIBMethod"]);
  } else {
    _SimpleIBMethod = 0;
    std::cout << " IBMover: SimpleIBMethod not found. Assuming default 0 \n";
    std::cerr << " IBMover: SimpleIBMethod not found. Assuming default 0 \n";
  }
  return;
}

void IbUtils::InitSolidVelocity(vector<double> Velocity) {
  int dim = Velocity.size();
  for (int i = 0; i < dim; i++) _Velocity[i] = Velocity[i];
  std::string VelField = "IVec*" + std::to_string(Velocity[0]) + " + JVec*" + std::to_string(Velocity[1]);
  if (dim == 3) VelField += " + KVec*" + std::to_string(Velocity[2]);
  _SolidVelocity->applyFunc(dim, VelField);
  return;
}

void IbUtils::InitPosition(double FirstTranslation[], MEDCoupling::MEDCouplingFieldDouble* VelocityField) {
...
void IbUtils::MoveWithGivenVectorAndRotation(
  ...
  for (int Node = 0; Node < Nnodes; Node++) {
    ...
    dist = sqrt(Coordinates[0] * Coordinates[0] + Coordinates[1] * Coordinates[1] + 1.e-20);
    mod = dist * (AngVel);
...
void IbUtils::UpdateInterpolatedFields(MEDCoupling::MEDCouplingFieldDouble* VelocityField) {
  if (_FluidCellColor != NULL) _FluidCellColor->decrRef();
  if (_FluidNodeColor != NULL) _FluidNodeColor->decrRef();
  if (_InterpolatedSolidVelocity != NULL) _InterpolatedSolidVelocity->decrRef();
  ...
  CoolerTemperature << "Target Mesh Cell # n : (Source Mesh Cell ID, Overlapping "
                       "Area)\n====================================================\n";
  ...
  for (int i = 0; i < cells; i++) {
    ...
    if (col > 1.1) {
      std::cout << "\033[1;21m IBMOVER: Col > 1 " << col << "  cell " << i << "\033[0m\n";
      for (auto& x : _RemapMatrix[i]) CoolerTemperature << "(" << x.first << ", " << x.second << "),\t";
      CoolerTemperature << "\n";
...
void IbUtils::UpdateFluidMesh(const MEDCoupling::MEDCouplingUMesh* NewMesh) {
  _FluidMesh->decrRef();
...
void IbUtils::UpdateSolidMesh(const MEDCoupling::MEDCouplingUMesh* NewMesh) {
...
void IbUtils::InsertTriangulatedCell(
  ...
  if (Sequence.size() == 3) {
    double vec_prod =
        (FullCoordinates[OrderedNodes[Sequence[1]]] - FullCoordinates[OrderedNodes[Sequence[0]]]) *
            (FullCoordinates[OrderedNodes[Sequence[2]] + offset] -
             FullCoordinates[OrderedNodes[Sequence[1]] + offset]) -
        (FullCoordinates[OrderedNodes[Sequence[1]] + offset] -
         FullCoordinates[OrderedNodes[Sequence[0]] + offset]) *
            (FullCoordinates[OrderedNodes[Sequence[2]]] - FullCoordinates[OrderedNodes[Sequence[1]]]);
  ...
  _TriangulatedCellsPerSourceCell[SourceCellId].push_back(NodesPerCell.size());
  NodesPerCell.push_back(NodesPerCell[NodesPerCell.size() - 1] + Sequence.size());
  for (int l = 0; l < Sequence.size(); l++)
    for (int dim = 0; dim < DIMENSION; dim++)
      TriangulationCoords.push_back(FullCoordinates[OrderedNodes[Sequence[l]] + dim * offset]);

  return;
}

void IbUtils::GetCellContainingPoint(int TargetCellId, double NodeCoord[], double CanPos[], int& CellId) {
  std::vector<int> SourceIDs;
  for (auto& x : _RemapMatrix[TargetCellId]) SourceIDs.push_back(x.first);

  int nCells = SourceIDs.size();
  int* IDs = new int[nCells];
  for (int i = 0; i < nCells; i++) IDs[i] = SourceIDs[i];

  MEDCoupling::MEDCouplingFieldDouble* Part = _SolidVelocity->buildSubPart(IDs, IDs + nCells);

  MEDCoupling::MCAuto<MEDCoupling::DataArrayInt> elts;
  MEDCoupling::MCAuto<MEDCoupling::DataArrayInt> eltsIndex;
  Part->getMesh()->getCellsContainingPoints(NodeCoord, 1, 1.e-5, elts, eltsIndex);
  ...
  if (NumPossibleCells > 0) {
    ...
    if (found) {
      //                     CellId = NewCell;
      for (int Mcomp = 0; Mcomp < _MeshDim; Mcomp++) { CanPos[Mcomp] = _XiEtaChi[Mcomp]; }
    }
    _CoordMatrix.clear();
    _XiEtaChi.clear();
  }  // end  if cell==-1
...
double IbUtils::TriangleArea(double* point1, double* point2, double* point3) {
  double Area = 0;

  double det = point1[0] * point2[1] + point2[0] * point3[1] + point3[0] * point1[1] -
...
double IbUtils::TriangleArea(double* points /* x1 x2 x3, y1 y2 y3 */) {
  double Area = 0.;
  double det = 0.;
  for (int i = 0; i < 3; i++) {
    int col_plus = (i + 1) % 3;
...
double IbUtils::QuadrangleArea(double* points /* x1 x2 x3 x4, y1 y2 y3 y4 */) {
  double Area = 0.;
  double det = 0.;
  for (int i = 0; i < 4; i++) {
    int col_plus = (i + 1) % 4;
...
MEDCoupling::MEDCouplingFieldDouble* IbUtils::InterpolateSolidOnFluid(
...
MEDCoupling::MEDCouplingFieldDouble* IbUtils::InterpolateFluidOnSolid(
    MEDCoupling::MEDCouplingFieldDouble* VelocityField,
...
MEDCoupling::MEDCouplingFieldDouble* IbUtils::ComputeStressField(
  ...
  double Stress[DIMENSION], StressOnCell[DIMENSION];
  Stress[0] = Stress[1] = 0.;
  StressOnCell[0] = StressOnCell[1] = 0.;
  Stress[DIMENSION - 1] = 0.;
  StressOnCell[DIMENSION - 1] = 0.;
  ...
  StressFieldBD = MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_CELLS);
  StressFieldBD->setMesh(_ExtractedVelocity->getMesh());
  ...
  for (int kj = 0; kj < DIMENSION; kj++)
    array->setInfoOnComponent(kj, std::to_string(kj));  // set info -> array
  ...
  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingMesh> ExtractedMesh = StressFieldBD->getMesh();
  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingUMesh> ExtractedUMesh(
      MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingMesh, MEDCoupling::MEDCouplingUMesh>(ExtractedMesh));

  MEDCoupling::MEDCouplingRemapper Remap;
  Remap.setPrecision(1e-11);
  Remap.setIntersectionType(INTERP_KERNEL::Triangulation);  // <- more accurate than triangulate
  Remap.prepare(ExtractedUMesh, ExtractedUMesh, "P0P1");

  MEDCoupling::MEDCouplingFieldDouble* StressFieldBDNodes;
...
MEDCoupling::MEDCouplingFieldDouble* IbUtils::GetFluidCellColor() { return _FluidCellColor->deepCopy(); }

MEDCoupling::MEDCouplingFieldDouble* IbUtils::GetFluidNodeColor() { return _FluidNodeColor->deepCopy(); }
