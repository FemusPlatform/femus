...
#include "MGSolverIB.h"  // Immersed Boundary class header file
...
/// It is called by MGSolIB::GenMatRhs in MGSolverT.C
...
void MGSolIB::bc_set(
  ...
  for (int lb_node = 0; lb_node < elb_ndof2; lb_node++)
    if (_bc_el[sur_toply[lb_node]] == 0) {
      ...
      FeM(lv_node) += bc_s * Ipenalty * _data_eq[2].ub[_FF_idx[IB_F] * NDOF_FEM + lv_node];
  ...
  if (_bc_vol[sur_toply[NDOF_FEMB - 1]] / 10 > 0) {  // bc 10-30
    ...
    double alpha_eff = _C_parameter.H_EXT;
    ...
    for (int qp = 0; qp < elb_ngauss; qp++) {
      ...
      for (int lsi_node = 0; lsi_node < elb_ndof2; lsi_node++) {  // local side loop (over the node face)
        ...
        FeM(lei_node) +=
            dtxJxW_g * bc_beta *
            (bc_alpha * alpha_eff * (_C_parameter.T_EXT) * phii_g  // Robin homog (k*dt/dn = h*T0)
             - (1 - bc_alpha) * sign_normal * phii_g               // Neumann heat flux bc
            );
        // Assemblying Matrix ---------------------------------
