...
void MGSolIB::vol_integral (
    ...
    double xx_qnds[],
    const int unsteady,const int mode, int el_conn[], double u_old[], int iel
) { // ==============================================================================================
...
//                 if ( _y_dist > _mgutils._IBParameter->_LowerThreshold ) {
                    if ( _bc_el[i]==1 ) {
                        ...
                        for ( int j=0; j<el_ndof2; j++ ) {
                            double Lap=0.;
                            for ( int idim=0; idim<  _nTdim; idim++ ) Lap += _dphi_g[2][j+idim*el_ndof2]*_dphi_g[2][i+idim*el_ndof2];
                            const double phij_g= _phi_g[2][j];
//                             FeM ( i )   += dtxJxW_g* ( ( 0.5*_dir + ( 1-_dir ) ) *u_old[j]*phij_g ) *phii_g;
