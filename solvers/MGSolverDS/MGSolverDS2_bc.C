...
void MGSolDS::set_bc_matrix(
  ...
  for (int lbnode = 0; lbnode < NDOF_FEMB; lbnode++) {
    ...
    if (bc_dn ==
      ...
      if (_dir ==
        ...
        FeM(indx_row) =
            bc_normal * bc_rhs * nii *
            _data_eq[2].ub[indx_sol + (_FF_idx[SDSX_F] + _dir) * el_ndof[2]];  // only normal non-homogeneous
        for (int jvar = _dir + 1; jvar < _nDSdim + _dir;
             jvar++) {  // non-diagonal part  ->  u_n=n(u*nx+v*ny+w*nz)
          ...
          FeM(indx_row) += (bc_rhs * bc_normal - 1) * nij *
                           valj;  //  KeM(indx_row,sur_toply[lbnode]+jvar1*el_ndof[2]) += aij;
        }                         // jvar loop
        ...
        FeM(indx_row) = Ipenalty * bc_rhs * bc_tg * (1 - nii) *
                        _data_eq[2].ub[indx_sol + (_FF_idx[SDSX_F] + _dir) * el_ndof[2]];
        // non-diagonal part
    ...
    if (bc_dn ==
      ...
      if ((bc_s % 3) < 4) {  // simmx=21,24 (x-plane),simmy=22,25(y-plane);simmz=23,26(z-plane);
        ...
        if (sim_dir == dir_maxnormal &&
          ...
          FeM(indx_row) = 0.;  // diagonal ->  u_n=n(u*nx+v*ny+w*nz)
  ...
  //   for(int  ivar=0; ivar< _nvars[2]; ivar++)
  {
    if (_bc_vol[sur_toply[NDOF_FEMB - 1]] / 10 == 1) { /*bc_dn=1; }*/
      ...
      int bc_normal = (bc_s % 2);
      int bc_tg = ((bc_s & 2) >> 1);
      int bc_rhs = ((bc_s & 4) >> 2);  // (0;?1?) tg (0;??1) normal (0;1??) nonhomogeneous

      //       if(bc_s !=3)    for(int k=0; k<elb_dof0 ; k++) { _bc_el[_pres_order*sur_toply[k]+
      //       _nDSdim*NDOF_FEM]= 0  ; }// pressure bc
      ...
      if (flag_normal) {  // normal
        ...
        double det;
        double sign = 1.;
        if (normal[dir_maxnormal] < 0) { sign = -1.; }
        for (int qp = 0; qp < elb_ngauss; qp++) {  // gaussian integration loop (n_gauss)
          ...
          for (int i = 0; i < elb_ndof[2]; i++) {  // Assemblying NS equation
            ...
            for (int kdim = 0; kdim < _nDSdim; kdim++) {
              un += _data_eq[2].ub[sur_toply[i] + (_FF_idx[SDSX_F] + kdim) * NDOF_FEM];
            }
            //               u_old[sur_toply[i]+kdim*NDOF_FEM]*normal[kdim]; }
            double ut = 0;
            for (int jvar = 0; jvar < _nDSdim; jvar++) {
              const double tmp =
                  _data_eq[2].ub[sur_toply[i] + (_FF_idx[SDSX_F] + jvar) * NDOF_FEM] - normal[jvar] * un;
              ut += tmp * tmp;
            }
            vel_bound[0] = sqrt(ut);
      ...
      //  -------------------------------------------------------------------------------------------------------------------
        ...
        for (int lbnode = 0; lbnode < elb_ndof[2]; lbnode++) {  // Assemblying NS equation
          ...
          if (_bc_el[sur_toply[lbnode] + ivar * NDOF_FEM] != 0) {
            ...
            _bc_el[sur_toply[lbnode] + ivar * NDOF_FEM] =
                ((bc_tg + bc_normal) == 0) ? 1 : 0;  // storage bc for vol part
            //          Dirichlet (homogenoues and non  homogenoues) Assemblying Matrix
            // u=u_n+u_t;   u_n=n(u*n) ; u_t=u-(u.n)n  flags=(?,bc_normal,bc_tg,bc_rhs)
            //  (?00?) single comp bc (?01?) Normal Dirichlet bc ; (?10?) tg Dirichlet bc
            // diagonal part
            ...
            KeM(indx_row, sur_toply[lbnode] + ivar * el_ndof[2]) +=
                -Ipenalty * normal[ivar + _dir] * normal[ivar + _dir] * (bc_tg + bc_normal);
            FeM(indx_row) +=
                bc_rhs * bc_tg * Ipenalty * _data_eq[2].ub[_FF_idx[SDSX_F] * NDOF_FEM + indx_sol] -
                bc_tg * bc_rhs * aii *
                    _data_eq[2]
                        .ub[_FF_idx[SDSX_F] * NDOF_FEM + indx_sol];  // Dirichlet tang  homogeneous only
            // non-diagonal part
            for (int jvar = ivar + _dir + 1; jvar < _nDSdim + ivar + _dir; jvar++) {
              int jvar1 = jvar % DIMENSION;
              double aij = _bc_el[sur_toply[lbnode] + ivar * NDOF_FEM] * Ipenalty * normal[jvar1] *
                           normal[ivar + _dir] * (bc_tg + bc_normal);
              double valj = _bc_el[sur_toply[lbnode] + ivar * NDOF_FEM] *
                            _data_eq[2].ub[(_FF_idx[SDSX_F] + jvar1) * NDOF_FEM + sur_toply[lbnode]];
