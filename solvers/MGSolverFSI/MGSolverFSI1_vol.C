...
void MGSolFSI::matrixrhs_liq_vol(
  ...
  double dt = _dt;
  if (dt > 1 || _FSI_parameter._SolveSteady == 1) { dt = 1.; }
  ...
  for (int qp = 0; qp < el_ngauss; qp++) {
    ...
    for (int i = 0; i < el_ndof2;
      ...
      for (int row_shift = 0; row_shift < _nvars[2]; row_shift++) {  // LOOP OVER ROWS: i + row_shift*el_ndof2
        ...
        if (_bc_el[indx] == -8) {
          _bc_el[indx] = 0;
          std::cout << "ATTENTION!!! el node " << i << " glob node " << el_conn[i] << " row " << indx
                    << " bc " << _bc_el[indx] << std::endl;
        }
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        bool BoundEquation = (_bc_el[indx] <= -1) ? true : false;
        bool NormalEquation = false;
        if (BoundEquation && _bc_el[indx] == -3) { NormalEquation = true; }

        if (BoundEquation && !NormalEquation) {
          CalcTangDir(i, el_ndof[2], _bc_el[indx], qp, tang1, tang2, 0);
        }
        int NComp = 1;  // Default: 1 vel comp for equation in row indx -> ivar = row_shift
        if (BoundEquation) {
          NComp = _nNSdim;
        }  // For boundary sides (tangential or normal required) linear combination of _nnsdim equations
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          for (int ivarN = row_shift; ivarN < row_shift + NComp;
            ...
            if (mode == 1) {  // rsh  terms
              if (_FSI_parameter._SolveSteady == 0) {
                FeM(indx) += dtxJxW_g * rho * vel_g[ivar + _dir] * (phii_g + Phi_supg) / _dt;
              }
            ...
            for (int j = 0; j < el_ndof2; j++) {
              ...
              if (_AxiSym == 1) {  // axysimmetry only --------------------------------------------------
                Lap_g = 2. * (1 - (ivar + _dir)) *
                        (IRe_eff + 0. * _FSI_parameter._Upwind * f_upwind * u_nlg[0] * u_nlg[0]) * phij_g *
                        (phii_g + Phi_supg) / (_xyzg[0] * _xyzg[0]);
              }  // --------------------------------------------------------------------------------
            ...
            for (int jp = 0; jp < el_ndof[1]; jp++) {
              ...
              if (_AxiSym == 1) {
                KeM(indx, jp + _nNSdim * el_ndof2) -=
                    dtxJxW_g * rho * (1 - (ivar + _dir)) * psij_g * phii_g / _xyzg[0];
              }
              //  B^T diagonal term
              KeM(indx, jp + _nNSdim * el_ndof2) +=
                  dtxJxW_g * rho *
        ...
        if (_bc_el[indx] <= 0) {
          for (int ivarN = row_shift; ivarN < row_shift + NComp;
            ...
            for (int j = 0; j < el_ndof2; j++) {
              ...
              if (_FSI_parameter._SolveSteady == 0) {
                KeM(indx, j + ivar * el_ndof2) +=
                    dtxJxW_g * rho * (1. / _dt + 2000.) * (phii_g + Phi_supg) * phij_g;
              }
              ...
              for (int ivars = 0; ivars < _nvars[2]; ivars++) {
                ...
                FeM(indx) -= IRe_eff * part * dphiidx_g2[ivars] *
                             _data_eq[2].ub[_FF_idx[SDSX_F + ivars] * NDOF_FEM + j];
                ...
                if (_FSI_parameter._Compressible == 1) {
                  ...
                  FeM(indx) -= lambda * partC * dphiidx_g2[ivar + _dir] *
                               _data_eq[2].ub[_FF_idx[SDSX_F + ivars] * NDOF_FEM + j];  //
                }                                                                       // end if compressible
            ...
            for (int jp = 0; jp < el_ndof[1]; jp++) {
              ...
              //  B^T diagonal term
              KeM(indx, jp + _nNSdim * el_ndof2) +=
                  dtxJxW_g * rho * (1 - _FSI_parameter._Compressible) *
                  (  // MPascal
                      -1. * _phi_g[1][jp] * _dphi_g[2][i + ivar * el_ndof2] +
                      _dphi_g[1][jp + ivar * el_ndof[1]] * (0. * phii_g + Phi_supg));
              //  B^T non-diagonal term
