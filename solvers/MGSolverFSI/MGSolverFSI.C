...
// coupling  basic system fields -------------------------------------------------------------------------------
    ...
    for (int iel=0; iel < (nel_e - nel_b); iel++) {
        ...
        if(ext_mesh->_mat_id[ iel+nel_b]  !=4) { // liquid region
            matrixrhs_liq_vol (KeM, FeM, el_ndof, u_old, u_nl, 0, mode, el_conn);
        }
        else {
            matrixrhs_sol_vol (KeM, FeM, el_ndof, u_old, u_nl, 0, mode, el_conn,flag_group);
        }
        // ---------------------- end volume (element) --------------------------------------
...
void FSI_param::read_param(
    ...
    if(_FileMap ["MaxNonLinearIterations"]!="") {
        ...
        std::cout <<" FSIproperties.in: default value for _FSI_parameter._MaxNonLinearIterations (in UserFSI.h)\n";
    }
    if(_FileMap ["DynamicUnderRelaxation"]!="") {
        _UnderRelaxation =stof(_FileMap["DynamicUnderRelaxation"]);
    }
    else {
        std::cout <<" FSIproperties.in: default value for _FSI_parameter._DynamicUnderRelaxation (in UserFSI.h)\n";
    }
