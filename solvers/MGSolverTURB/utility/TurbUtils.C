...
//
  ...
  if ( _FileMap["UseKays"] != "" ) {
      _4PwithKays = stoi ( _FileMap["UseKays"] );
      }
  ...
  if ( _FileMap ["SolveMuT"] != "" ) {
      _SolveMuT = stoi ( _FileMap["SolveMuT"] );
      }

  if ( _FileMap ["SolveAlphaT"] != "" ) {
      _SolveAlphaT = stoi ( _FileMap["SolveAlphaT"] );
      }
...
void TurbUtils::TherTurInitValues ( double & kappaT, double & omegaT, double WallDist, double RefDeltaT, double AvVel, double Diameter, bool FlatProfile ) {
  ...
  _ThermModel->CalcThermSourceTerms(_DynModel->_KappaAndOmega, TKappaAndOmega, source, diss, meccterm, st, sp, mut, alphaturb,dist);      
  ...
  _DynModel->CalcKappaAndOmega(KappaAndOmega);    
  _ThermModel->CalcThermSourceTerms(_DynModel->_KappaAndOmega, TKappaAndOmega, source, diss, meccterm, st, sp, Nut, alphaturb,dist);   
