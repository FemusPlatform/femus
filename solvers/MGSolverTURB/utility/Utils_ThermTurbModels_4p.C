...
void Therm4P::ThermTurInitKappaAndOmegaValues(
    double* ThermTurbVariables, double vel, double diameter, double RefDeltaT) {
...
void Therm4P::ThermTurNearKappaHAndOmegaHValues(
...
void Therm4P_logKHWH::CalcThermSourceTerms(
    double* KappaAndOmega, double* ThermTurbVariables, double* SourceTerms, double* DissTerms,
    double* MeccTerms, double TempGradMod, double VelGradMod, double muT, double alphaT, double ydist) {
  ...
  const double f_exp =
      (__CD2 * (1. - 0.3 * exp(-_Rt * _Rt / 42.25)) - 1.) * (1. - exp(-_Rd / 5.7)) * (1. - exp(-_Rd / 5.7));
