  ...
  for (int qp = 0; qp < el_ngauss; qp++) {
    ...
    for (int i = 0; i < el_ndof2; i++) {
      if (_bc_el[i] == 1) {
        ...
        if (_dir == 0) { source = _y_dist + _mgutils._geometry["Wall_dist"]; }

        if (_dir == 1) { source = mu_turb; }

        if (_dir == 2) { source = alpha_turb; }
      ...
      if (_dir == 0 && _bc_el[i] == 0) {
        ...
        for (int j = 0; j < el_ndof2; j++) { KeM(i, j) = 0.; }
...
void MGSolTURB::compute_y_plus(
    const int el_ndof2, const int el_ngauss, double xx_qnds[], const int unsteady, const int mode,
    int el_conn[],
    int iel)  // ==============================================================================================
{
...
void MGSolTURB::rhs_integral(
    DenseVectorM& FeM, const int el_ndof2, const int el_ngauss, double xx_qnds[], const int unsteady,
    const int mode,
    int el_conn
        [])  // ==============================================================================================
{
