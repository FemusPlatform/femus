...
void MGSolWilcoxLog::CalcAdvectiveAndDiffusiveTerms(int i, int j, int el_ndof2, double f_upwind) {
...
void MGSolWilcoxLog::CalcSourceAndDiss(int el_ndof2) {
  ...
  _explicit_diss[_dir] = _diss[_dir];
...
void MGSolWilcoxLog::VelocityForSUPG(
    ...
    double VEL[])  // NUMERICAL STABILIZATION - UPWIND AND SUPG ===============================
{
  for (int i = 0; i < DIMENSION; i++) {
    ...
    if (_ModifiedSupg) { VEL[i] -= _nueff * (_T_dxg[_dir][i]); }
