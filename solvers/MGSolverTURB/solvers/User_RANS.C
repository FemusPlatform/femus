...
void MGSolRANS::ic_read(int bc_gam, int bc_mat, double xp[], int iel, double u_value[]) {
  ...
  // KAPPA
  if (_dir == 0) { u_value[0] = initial_value[0]; }

  // OMEGA
  if (_dir == 1) { u_value[0] = initial_value[1]; }

  return;
}

// BOUNDARY CONDITIONS -------------------------------------------------------------------
void MGSolRANS::bc_read(int bc_gam, int bc_mat, double xp[], int bc_Neum[], int bc_flag[]) {
  bc_Neum[0] = Kinsulation;

  if (_dir == 1) { bc_Neum[0] = _RANS_parameter._map_DTWgroup[bc_gam]; }

  if (_dir == 0) { bc_Neum[0] = _RANS_parameter._map_DTKgroup[bc_gam]; }
  return;
}
