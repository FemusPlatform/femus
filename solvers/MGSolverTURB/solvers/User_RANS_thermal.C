...
// INITIAL CONDITION ---------------------------------------------------------------------
void MGSolRANS_thermal::ic_read (
    int bc_gam,
    int bc_mat,
    double xp[],
    int iel,
...
)
{
    ...
    _mgutils._TurbParameters->TherTurInitValues ( kh, wh, wall_dist, FluxCorr * NormFlux, vel, diameter, _FlatProfile );
    kh = 0.0001;
    wh_value = ( wh );
    kh_value = ( kh );
...
// BOUNDARY CONDITIONS -------------------------------------------------------------------
void MGSolRANS_thermal::bc_read (
    int bc_gam,
    int bc_mat,
    double xp[],
    int bc_Neum[],
...
)
{
    ...
    if ( _dir == 0 ) {
        bc_Neum[0] = _RANS_t_parameter._map_TTKgroup[bc_gam];
    }

    if ( _dir == 1 ) {
        bc_Neum[0] = _RANS_t_parameter._map_TTWgroup[bc_gam];
    }
