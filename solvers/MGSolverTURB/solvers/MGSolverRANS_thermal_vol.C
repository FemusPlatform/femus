        ...
        [])  // ==============================================================================================
{
  ...
  for (int dim = 0; dim < _nTKdim; dim++) { _AlphaTurbDxg[dim] = 0.; }
  ...
  for (int qp = 0; qp < el_ngauss; qp++) {  // INTERPOLATION OVER GAUSS NODE
    ...
    for (int idim = 0; idim < DIMENSION; idim++) { _sT += T_dxg[idim] * T_dxg[idim]; }
    ...
    if (_RANS_t_parameter._InterpolatedAlphaTurb == 1) {
      ...
      for (int l = 0; l < DIMENSION; l++) { _AlphaTurbDxg[l] *= _IRe * _InvSigma; }
    }

    if (_FF_idx[DIST] > -1) { _y_dist = _ub_g[2][_FF_idx[DIST]]; }

    // function must be called using _kappa_g and not values from non linear iterations
    _mgutils._TurbParameters->CalcThermTurSourceAndDiss(
        _kappa_g, _Tkappa_g, _y_dist, _sP, _sT, _alpha_turb, _source, _diss, _mecc_term);
    ...
    for (int i = 0; i < DIMENSION; i++) { ParVel[i] = 0.; }

    if (_SUPG || _UPWIND || _SCPG) { CalcSUPG(h_eff, f_upwind, mod2_vel, _Vel_g); }
    ...
    for (int nEq = 0; nEq < nEquations; nEq++) {  // LOOP OVER TEST FUNCTIONS PHI_I
      ...
      for (int j = 0; j < el_ndof2; j++) {  // INTERPOLATION OVER ELEMENT NODES
        ...
        if (_AxiSym == 1) { CylLap = _alpha_eff * (/* phij_g/xyz_g[0] -*/ _dphi_g[2][j]) / xyz_g[0]; }

        // LEFT HAND SIDE -----------------------------------------------------
        double TimeDer = (1 - _SolveSteady) * (phii_g + Phi_supg) * phij_g / _dt;  // time term

        double Eq_on_G = (phii_g + Phi_supg) * (+_Adv               // advection term
...
double MGSolRANS_thermal::CalcPhiSupg(
    ...
    double vel_g[])  // NUMERICAL STABILIZATION - UPWIND AND SUPG ===============================
{
  ...
  if (_ModifiedSupg) {  // MODIFIED SUPG - CROSS CONTRIBUTIONS AS ADDITIONAL ADVECTION ------------
    ...
    }
    {
      ...
      for (int i = 0; i < _nTKdim; i++) {
        LapK += _dir * (2. * _KH_2der[i * _nTKdim + i] + _WH_2der[i * _nTKdim + i]) +
                (1. - _dir) * _KH_2der[i * _nTKdim + i];
      }
  ...
  //
  //   f_upwind = 1/(sqrt(9.*(4.*_alpha_eff/(h_eff*h_eff))*(4.*_alpha_eff/(h_eff*h_eff))
