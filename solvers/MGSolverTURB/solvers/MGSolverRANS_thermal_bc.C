...
)
{
    ...
    if ( _RANS_t_parameter._WallFunctionApproach == 1 && bc_face != 10 ) {
        ...
        if ( bc_face == 1 ) {
            ...
            for ( int dir = 0; dir < _nTKdim; dir++ ) {
                _NormMidCell[dir] += normal[dir];
            }
        }
        ...
        for ( int lb_node = 0; lb_node < elb_ndof2; lb_node++ ) {
            if ( _bc_el[sur_toply[lb_node]] == 0 ) {
                ...
                if ( bc_wall ) { // DIRICHLET VALUES ON WALL BOUNDARIES
                    ...
                    if ( _RANS_t_parameter._WallFunctionApproach == 1 ) {
                ...
                if ( bc_inlet ) { // DIRICHLET VALUES ON INLET BOUNDARIES   ->    http://support.esi-cfd.com/esi-users/turb_parameters/
                    inlet_value[0] = _data_eq[2].ub[ ( _FF_idx[KTT_F] ) * NDOF_FEM + lv_node]; // k_value;//(1.-_khlog)*k_in + _khlog*log(k_in);
                    inlet_value[1] = _data_eq[2].ub[ ( _FF_idx[KTT_F] + 1 ) * NDOF_FEM + lv_node]; //w_value;//(1.-_whlog)*w_in + _whlog*log(w_in);
                }
            ...
            for ( int qp = 0; qp <  elb_ngauss; qp++ ) { // GAUSS LOOP
                ...
                for ( int lsi_node = 0; lsi_node < elb_ndof2; lsi_node++ ) { // local side loop (over the node face)
                    ...
                    if ( _bc_el[lei_node] != 0 ) {
                        ...
                        if ( FemBC == 1 ) {
                            _FeM ( lei_node ) += dtxJxW_g * phii_g * ( bc_alpha * FemBC * wall_der[_dir] ) ;
                        }

                        // Assemblying Matrix ---------------------------------
                        if ( KemBC == 1 ) {
                            for ( int lsj_node = 0; lsj_node < elb_ndof2;  lsj_node++ ) {
                                _KeM ( lei_node, sur_toply[lsj_node] ) += dtxJxW_g * bc_alpha * KemBC * wall_der[_dir] * phii_g * _phi_g[2][lsj_node];
