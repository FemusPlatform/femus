      ...
      _muf(_mgutils._mat_prop["mu0"])            // parameter  viscosity reference
{
  ...
  for (int k_index = 0; k_index < 40; k_index++) { _FF_idx[k_index] = -1; }
  ...
  if (_dir == 0) { _DiffusionCoefficient = 1.e-8; }

  for (int i = 0; i < 2 * NDOF_FEM; i++) { _KW[i] = _KHWH[i] = 1.; }
    ...
    )              // ===============================================
{
  ...
  for (int iel = 0; iel < (nel_e - nel_b); iel++) {
    ...
    if (_Solve == 1) {
      ...
      for (int i = 0; i < NDOF_FEM; i++) { _bc_el[i] = 1; }
      ...
      for (int iside = 0; iside < el_sides; iside++) {
        if (el_neigh[iside] == -1) {
          for (int idof = 0; idof < elb_ndof[2]; idof++) {
            ...
            if (_bc_bd[idofb] % 2 == 0) { _bc_el[idofb] = 0; }
            //                          for ( int idim=0; idim< _nTdim; idim++ )
            //                          _xxb_qnds[idim*NDOF_FEMB+idof]=_xx_qnds[idim*NDOF_FEM+idofb]; //
            //                          coordinates
          }
  ...
  if (_dir == 1) {
    ...
    //         MPI_Gather(_mgmesh._yplus + nel_b, nel_e-nel_b, MPI_DOUBLE, _mgmesh._yplus, nel_e-nel_b,
    //         MPI_DOUBLE, 0, MPI_COMM_WORLD);

    // Gathering yplus values on proc 0 for printing
    MPI_Gatherv(
        _mgmesh._yplus + nel_b, nel_e - nel_b, MPI_DOUBLE, _mgmesh._yplus, counts, shifts, MPI_DOUBLE, 0,
        MPI_COMM_WORLD);
    delete[] shifts;
    delete[] counts;
  }
