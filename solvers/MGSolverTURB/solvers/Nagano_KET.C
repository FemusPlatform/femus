    ...
    )   // parameter  conductivity reference
{
    ...
    double ehlim = _mgutils._TurbParameters->GetEHlim();

    _LowerLimit = ( _dir==0 ) ? khlim : ehlim;

    return;
}

void MGSolNaganoKET::CalcAdvectiveAndDiffusiveTerms ( int i, int j, int el_ndof2, double f_upwind )
{
...
void MGSolNaganoKET::VelocityForSUPG (
    double & mod2_vel,
    double vel_g[],
    double VEL[]
