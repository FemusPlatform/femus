...
void MGSolRANS::vol_integral(
    const int el_ndof2, const int el_ngauss, double xx_qnds[], int ElId,
    int el_conn
        [])  // ==============================================================================================
{
  ...
  for (int qp = 0; qp < el_ngauss; qp++) {  // INTERPOLATION OVER GAUSS NODE
    ...
    for (int l = 0; l < DIMENSION; l++) { _MuTurbDxg[l] *= _IRe * _InvSigma; }
    ...
    if ((_SUPG && _ModifiedSupg) || _SCPG) { CalcSUPG(h_eff, f_upwind, mod2_vel, _Vel_g); }
    ...
    for (int nEq = 0; nEq < _EquationNodes.size(); nEq++) {
      ...
      for (int j = 0; j < el_ndof2; j++) {  // INTERPOLATION OVER ELEMENT NODES
        ...
        if (_AxiSym == 1) { CylLap = _nueff * (/* phij_g/xyz_g[0] - */ _dphi_g[2][j]) / xyz_g[0]; }

        double MatTimeDer = (1 - _SolveSteady) * T_0ts * (phii_g + Phi_supg) * phij_g / _dt;  // time term

        kem_ij = (phii_g + Phi_supg) * (+_Adv                            // advection term
  ...
  for (int qp = 0; qp < el_ngauss; qp++) {
    ...
    for (int n = 0; n < el_ndof2; n++) {
      for (int dim = 0; dim < DIMENSION; dim++) {
        der_dist[dim] +=
            JxW_g2 * _data_eq[2].ub[_FF_idx[DIST] * el_ndof2 + n] * _dphi_g[2][n + dim * el_ndof2];
      }
    }

    for (int dim = 0; dim < DIMENSION; dim++) { grad[dim] += JxW_g2 * der_dist[dim]; }
  }

  double norm[DIMENSION];
  double mod = 0;

  for (int dim = 0; dim < DIMENSION; dim++) { mod += grad[dim] * grad[dim]; }

  mod = sqrt(mod) + 1.e-10;

  for (int dim = 0; dim < DIMENSION; dim++) { norm[dim] = -grad[dim] / mod; }
...
void MGSolRANS::CalcSCPG(
    double ParVel[], double& tauc, double DefSource, double vel_g[], double mod2_vel,
    double h_eff)  // Stabilized finite element method for heat transfer and turbulent flows inside industrial
                   // furnaces
{
...
void MGSolRANS::CalcSUPG(
    double& h_eff, double& f_upwind, double mod2_vel,
    double vel_g[])  // NUMERICAL STABILIZATION - UPWIND AND SUPG ===============================
{
  ...
  if (_ModifiedSupg) {  // MODIFIED SUPG - CROSS CONTRIBUTIONS AS ADDITIONAL ADVECTION ------------
    ...
    {
      // TAU FORMULATION FROM ELIE HACHEM - STABILIZED FINITE ELEMENT METHOD FOR HEAT TRANSFER AND TURBULENT
      // FLOWS INSIDE INDUSTRIAL FURNACES
    ...
    }
    {
      ...
      for (int i = 0; i < _nTdim; i++) {
        LapK += _dir * (2. * _T_2dxg[0][i * _nTdim + i] + _T_2dxg[1][i * _nTdim + i]) +
                (1. - _dir) * _T_2dxg[0][i * _nTdim + i];
      }
  ...
  //
  //   f_upwind = 1/(sqrt(9.*(4.*_nueff/(h_eff*h_eff))*(4.*_nueff/(h_eff*h_eff))
  //   + 4.*mod2_vel*mod2_vel/(h_eff*h_eff)));

  return;
}  //==========================================================================

double MGSolRANS::CalcPhiSupg(
  ...
  for (int idim = 0; idim < _nTdim; idim++) {
    ...
    if (_SCPG) { ShockCapturing += tauc * ParVel[idim] * dphiidxg; }
  ...
  if (_ModifiedSupg) {
    if (!_SolveSteady) { SReact += (1. / _dt) * phii_g; }
...
void MGSolRANS::CalcSourceAndDiss(int el_ndof2) {
  ...
  for (int k = 0; k < _nTdim; k++) { _MuTurbDxg[k] = 0.; }

  if (_InterpolatedMuTurb == 1) {
    interp_el_gdx(_NodeMuTurb, 0, 1, _dphi_g[2], el_ndof2, _MuTurbDxg);
    interp_el_sol(
        _NodeMuTurb, 0, 1, _phi_g[2], el_ndof2, MuTurb);  // muturb interpolated from the node values
  ...
  _mgutils._TurbParameters->CalcDynTurSourceAndDiss(
      _kappa_g, _y_dist, _sP, _mu_turb, _source, _diss, Div_g);  // point wise source and diss
  _mu_turb = max(0., _mu_turb);

  if (_InterpolatedMuTurb == 1) { _mu_turb = max(0., MuTurb[0]); }
...
void MGSolRANS::vol_stab(
    const int el_ndof2, const int el_ngauss, const int mode,
    int el_conn
        [])  // ==============================================================================================
{
  // --------------------------------------------------------------------------------------------------------------------
