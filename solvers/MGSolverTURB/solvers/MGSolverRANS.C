      ...
      _kappa0(_mgutils._mat_prop["kappa0"])  // parameter  conductivity reference
{
  ...
  for (int l = 0; l < _NoLevels; l++) { _solver[l]->set_solver_type(_RANS_parameter._SolverType); }
    ...
    )              // ===============================================
{
  ...
  for (int iel = 0; iel < (nel_e - nel_b); iel++) {  // LOOP OVER MESH ELEMENTS
    ...
    for (int dir = 0; dir < _nTdim; dir++) { _NormMidCell[dir] = 0.; }

    for (int iside = 0; iside < el_sides; iside++)  // CHECK IF THE ELEMENT HAS BOUNDARY SIDES =======
      if (el_neigh[iside] == -1) { _BoundElem = true; }
    ...
    if (_BoundElem) {  // INTEGRATION OVER BOUNDARY ==================================================
      ...
      for (int j = 0; j < el_ndof[2]; j++) { _NodeOnNwallSides[j] = 0; }
    ...
    for (int i = 0; i < el_ndof2; i++)
      if (_bc_el[i] != 0) { _EquationNodes.push_back(i); }
    ...
    for (int i = 0; i < el_mat_nrows; i++) {
      RowResidual = 0.;
      if (_bc_el[i] != 0) {
        for (int j = 0; j < el_mat_ncols; j++) { RowResidual += _KeM(i, j) * _x_1ts[j]; }
        RowResidual -= _FeM(i);
      }
...
void MGSolRANS::SetUpFlags() {
  ...
  for (int pr = 0; pr < _mgmesh._iproc; pr++) {
    int delta =
        _mgmesh._off_el[0][pr * _NoLevels + _NoLevels + 1] - _mgmesh._off_el[0][pr * _NoLevels + _NoLevels];
    ndof_lev += delta;
  }
...
void MGSolRANS::MGTimeStep(
  ...
  if (_SolveRANS) {
    ...
    if (_RANS_parameter._FractionalStep == 1) { FractionalTimeStep(time); }

    if (_RANS_parameter._FractionalStep == 0) { StandardTimeStep(time); }
...
void MGSolRANS::FractionalTimeStep(const double time) {
...
void MGSolRANS::StandardTimeStep(const double time) {
  /// B) Assemblying of the Matrix-Rhs
  _NonLinearIt = 0;
  ...
  if (_iproc == 0) {
    ...
    for (int Level = _NoLevels - 1; Level >= 0; Level--) std::cout << _LevelResidual[Level] << " ";
    std::cout << std::endl;
  }
...
void MGSolRANS::MGTimeStep_no_up(
  ...
  if (_SolveRANS) {
    std::cout << std::endl
              << "\033[038;5;" << 217 << ";1m "
              << "--------------------------------------------------- \n\t" << _eqname.c_str() << "   "
              << _var_names[0].c_str() << " solution of problem " << _mgutils.get_name()
              << "\n --------------------------------------------------- \n \033[0m";
...
void MGSolRANS::FillBoundaryMap() {
  ...
  for (int i = 0; i < nWalls; i++) { _BoundaryMap[WallGroups[i]] = WALL; }

  for (int i = 0; i < nBound; i++) {
    std::cout << "Group Num: " << _RANS_parameter._BoundaryGroupsIDs[i] << " Bound Type "
              << _BoundaryMap[_RANS_parameter._BoundaryGroupsIDs[i]] << std::endl;
  }
