...
void MGSolRANS::bc_set(int sur_toply[], int el_ndof2, int elb_ndof2, int elb_ngauss, int el_conn[]) {
  ...
  if (_RANS_parameter._WallFunctionApproach == 0 || _WallElement == 0) {
    ...
    for (int lb_node = 0; lb_node < elb_ndof2; lb_node++) {
      ...
      if (_bc_el[sur_toply[lb_node]] == 0 &&
          NodeBC == FaceBD) {              // ONLY IF NODE BC IS THE SAME OF BOUNDARY MID POINT BC
        ...
        if (bc_inlet == 1) {  // DIRICHLET VALUES ON INLET BOUNDARIES   ->
        ...
        _FeM(lv_node) =
            Ipenalty * (bc_wall * wall_value[_dir] + bc_inlet * inlet_value[_dir] + bc_init * init_val[_dir]);
