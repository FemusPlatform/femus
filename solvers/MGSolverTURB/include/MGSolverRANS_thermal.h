...
protected:
  ...
  double _FemDiss[2], _KemDiss[2], _FemSource[2], _KemSource[2], _diss[2],
    _source[2];
  double _LowerLimit;
  ...
  double _explicit_diss[2], _explicit_source[2], _implicit_diss[2],
    _implicit_source[2], _mecc_term[2];
  double _Cross[2], _Log_Cross[2];
  double _Adv, _Lap, _Vel_g[DIMENSION], _AlphaTurbDxg[DIMENSION], _LapSupg,
    _LapMuTurb;
...
//     ~MGSolTBK() {}                              ///< Destructor
  ...
  void bc_set (int sur_toply[] /**< Local matrix  */ ,
	       int el_ndof2 /**< el dofs*/ , int elb_ndof2 /**< el bd dofs*/ , int elb_ngauss
		  /**< #pt gauss  */
    );
  // ==========================================================================
  /// This function assembles the volume integral to obtain the algebraic sytem.
  /// It is called by MGSolTBK::GenMatRhs in MGSolverT.C
  void vol_integral (const int el_ndof2,
			/**< el dofs*/
		     const int el_ngauss,
			/**< #pt gauss  */
  ...
  void turb_utils ();
  void GetSolution (int iel, int Level,
