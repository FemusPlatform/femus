...
void EquationsMap::initNavierStokes ( EquationSystemsExtendedM & EqMap ) {
    ...
    _nvars[0]=0;
    _nvars[1]=NS_EQUATIONS%2; // linear(1) approx
    if ( NDOF_K>0 ) {
    ...
    EqMap.AddSolver<MGSolNS> ( "NS0Y", NS_F +1, _nvars[0],_nvars[1],_nvars[2],"v" );
#if DIMENSION==3
    EqMap.AddSolver<MGSolNS> ( "NS0Z", NS_F +2, _nvars[0],_nvars[1],_nvars[2],"w" );
#endif
#endif
