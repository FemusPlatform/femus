...
void EquationsMap::FillEquationMap(
  ...
  for (int iname = 0; iname < n_equations; iname++) {
    ...
    if (_myproblemP[iname] == NS_F || _myproblemP[iname] == NSX_F || _myproblemP[iname] == NSY_F ||
        _myproblemP[iname] == NSZ_F) {
      initNavierStokes(EqMap);
    }

    if (_myproblemP[iname] == FS_F) { initFluidStructure(EqMap); }

    if (_myproblemP[iname] == SDS_F) { initDisplacements(EqMap); }

    if (_myproblemP[iname] == B_F || _myproblemP[iname] == BX_F || _myproblemP[iname] == BY_F ||
        _myproblemP[iname] == BZ_F) {
      initBoundary(EqMap);
    }

    if (_myproblemP[iname] == T_F) { initTemperature(EqMap); }

    if (_myproblemP[iname] == K_F || _myproblemP[iname] == EW_F) { initDynamicTurbulence(EqMap); }

    if (_myproblemP[iname] == KTT_F || _myproblemP[iname] == EWTT_F) { initThermalTurbulence(EqMap); }

    if (_myproblemP[iname] == CO_F) { initColor(EqMap); }

    if (_myproblemP[iname] == NSA_F || _myproblemP[iname] == NSAX_F || _myproblemP[iname] == NSAY_F ||
        _myproblemP[iname] == NSAZ_F) {
      initAdjointNavierStokes(EqMap);
    }

    if (_myproblemP[iname] == FSA_F) { initAdjointFSI(EqMap); }

    if (_myproblemP[iname] == TA_F) { initAdjointTemperature(EqMap); }

    if (_myproblemP[iname] == KA_F || _myproblemP[iname] == EWA_F) { initAdjointDynamicTurbulence(EqMap); }

    if (_myproblemP[iname] == CTRL_F) { initControlTemperature(EqMap); }

    if (_myproblemP[iname] == IB_F) { initImmersedBoundary(EqMap); }

    if (_myproblemP[iname] == TCOUP_F) { initCoupledTemperature(EqMap); }

    if (_myproblemP[iname] == A_F) { initAreaMono(EqMap); }

    if (_myproblemP[iname] == Q_F) { initFlowrateMono(EqMap); }
    if (_myproblemP[iname] == TAU_F || _myproblemP[iname] == TAUXX_F || _myproblemP[iname] == TAUXY_F ||
        _myproblemP[iname] == TAUYY_F) {
      initReynoldsStressTensor(EqMap);
    }
    if (_myproblemP[iname] == THF_F || _myproblemP[iname] == THFX_F || _myproblemP[iname] == THFY_F ||
        _myproblemP[iname] == THFZ_F) {
...
void EquationsMap::setProblems(MGSolBase*& ProbObj) {
  ...
  for (std::map<std::string, int>::iterator eq = _EquationsToAdd.begin(); eq != _EquationsToAdd.end(); ++eq) {
    ...
    if (EqnLabel > 0) {
      ...
      // TEMPERATURE
      if (EqnName == "MG_Temperature") { ProbObj->ActivateScalar(2, T_F, "T", _QuadEq); }
      ...
      // ADJOINT TEMPERATURE
      if (EqnName == "MG_AdjointTemperature") { ProbObj->ActivateScalar(2, TA_F, "TA", _QuadEq); }
