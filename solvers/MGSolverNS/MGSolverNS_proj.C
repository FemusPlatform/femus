...
void MGSolNS_proj::get_el_field_data(
    int iel, int Level, int el_conn[], int offset, int el_ndof[], int ndof_lev) {
  ...
  for (int i = 0; i < NDOF_P; i++) {
    ...
    if (_NS_parameter._TimeDisc == 1) _Pressure[i] = (_p_1ts[i]);
  }
...
void MGSolNS_proj::MomentumEquation(double JxW_g2, int el_ndof[], int qp) {
  ...
  for (int i = 0; i < el_ndof[2];
    ...
    for (int idim = 0; idim < _nNSdim; idim++) dphiidx_g2[idim] = _dphi_g[2][i + idim * el_ndof[2]];
    ...
    if ((_bc_el[indx_eq] < 0 || (_bc_el[indx_eq] == 0 && (_bc_vol[i] == 11 || _bc_vol[i] == 31))) &&
        _ImmVal) {
      for (int ivarN = _dir; ivarN < _dir + _NComp;
        ...
        if (_NS_parameter._SolveSteady == 0) {
          if (_NS_parameter._TimeDisc == 1) TimeDer = (_u_1ts_g[ivar]) / (_dt);

          if (_NS_parameter._TimeDisc == 2) TimeDer = (2. * _u_1ts_g[ivar] - 0.5 * _u_2ts_g[ivar]) / (_dt);
        }

        _FeM(i) += dtxJxW_g *
                   (TimeDer + _IFr * _dirg[ivar]  // force
                    - _P_gdx[ivar]                // pressure
                    ) *
                   (phii_g + _Phi_supg);

        for (int ot = ivar; ot < ivar + _nNSdim;
             ot++) {  // out of diagonal tensor components -> better explicit
        ...
        for (int j = 0; j < el_ndof[2]; j++) {
          ...
          if (_AxiSym == 1) {  // axysimmetry only --------------------------------------------------
            Lap_g = 2. * (1 - (_dir)) *
                    (_IReEff + _NS_parameter._Upwind * _f_upwind * _u_1ts_g[0] * _u_1ts_g[0]) * phij_g *
                    (phii_g + _Phi_supg) / (_xyzg[0] * _xyzg[0]);
          }  // --------------------------------------------------------------------------------
          ...
          for (int kdim = 0; kdim < _nNSdim; kdim++) {
            ...
            if (_NS_parameter._TimeDisc == 1) Adv_g += (_u_1ts_g[kdim]) * dphijdx_g2[kdim];
          ...
          if (_NS_parameter._TimeDisc == 1) TimeDerivative = dtxJxW_g * (phii_g + _Phi_supg) * phij_g / (_dt);

          const double KeM_ij = dtxJxW_g * (Adv_g * (phii_g + _Phi_supg)  // advection
...
void MGSolNS_proj::set_bc_matrix(
  ...
  for (int dir = 0; dir < _nNSdim; dir++) { _ProjDir[2][dir] = normal[dir]; }
  ...
  for (int lbnode = 0; lbnode < NDOF_FEMB; lbnode++) {
    ...
    if (bd_face == bd_node) {
      ...
      if (DirichletBC && norm_face == norm_node /*!_AlreadyWrittenDirBC[indx_row]*/) {
        ...
        for (int i = 0; i < _nNSdim; i++) { prod += _u_1ts[node + i * el_ndof[2]] * normal[i]; }
        for (int i = 0; i < _nNSdim; i++) { u_norm[i] = prod * normal[i]; }
        ...
        if (bd_node == 88 || bd_node == 66) {
          ...
          if (bd_node == 98) {
            ...
            double scal_prod = 0.;
            double vel_proj[DIMENSION];
            for (int i = 0; i < _nNSdim; i++) {
              scal_prod += _u_1ts[node + i * el_ndof[2]] * _ProjDir[direction][i];
            }
            for (int i = 0; i < _nNSdim; i++) { vel_proj[i] = scal_prod * _ProjDir[direction][i]; }
            _KeM(node, node) = 1.;
  ...
  for (int qp = 0; qp < elb_ngauss;
    ...
    for (int i = 0; i < elb_ndof[2];
      ...
      const int bc_flag =
          (_bc_el[indx] == -3) ? bd_node / 10 : bd_node % 10;  // normal or tangential bc flags
      ...
      if (bd_face == bd_node) {
        ...
        if (bc_flag < 6 &&
          ...
          for (int ivarN = _dir; ivarN < _dir + NComp;
               ivarN++) {  // LOOP OVER VELOCITY COMPONENTS FOR EQUATION WRITTEN IN ROW indx
            ...
            if (_bc_el[indx] == -1 || _bc_el[indx] == -2) {
              if (bc_flag != 1) {
                ...
                if (bc_flag == 5) { alpha_beta = _ProjDir[0][ivar]; }
                double tau = (_bc_el[indx] == -1) ? (_NS_parameter._Tg1_stress)
                                                  : _NS_parameter._Tg2_stress;  // utau*utau/vel_bound;
