...
void MGSolNS::matrixrhsvol (
  ...
  for ( int qp = 0; qp <  el_ngauss; qp++ ) {
      ...
      if ( _FF_idx[MU_T] >= 0 ) {
          _IReEff   += _IRe * _ub_g[2][_FF_idx[MU_T]];
          }
...
void MGSolNS::CalcTimeDer ( double & TimeDer, TIME_DER_TYPE Type, int SpaceDir, int nPhi, int interpNodeID )
  ...
  if ( Type == OLDER_STEPS ) {
      ...
      if ( _NS_parameter._SolveSteady == 0 )
        TimeDer = ( TD_1ts * _u_1ts_g[SpaceDir] - TD_2ts * _u_2ts_g[SpaceDir] ) * ( _phi_g[2][nPhi] + _Phi_supg ) / ( _dt );
      }
...
void  MGSolNS::CalcAdv_Lap_LapSupg ( double & Adv, double & Lap, double & LapSupg, int nPhi, int j, int el_ndof[], int dir )
  {
  ...
  if ( _AxiSym == 1 && dir == 0 ) { // axysimmetry only --------------------------------------------------
      Lap = 2.* (
              _IReEff + _NS_parameter._Upwind * _f_upwind * _u_nl_g[0] * _u_nl_g[0]
            ) * _phi_g[2][j] * ( _phi_g[2][nPhi] + _Phi_supg ) / ( _xyzg[0] * _xyzg[0] );
      } // --------------------------------------------------------------------------------
  ...
  for ( int kdim = 0; kdim < _nNSdim; kdim++ ) {
      ...
      Adv     += ( ADV_1ts * _u_1ts_g[kdim] - ADV_2ts * _u_2ts_g[kdim] ) * dphijdx_g2[kdim];                                                            // Adv_g +=_u_1ts_g[kdim]*dphijdx_g2[kdim]*phii_g;
      Lap     += ( _IReEff + up ) * dphijdx_g2[kdim] * _dphi_g[2][nPhi + kdim * el_ndof[2]];
      if( _NS_parameter._Supg == 1 ) {LapSupg += ( _IReEff ) * _ddphi_g[2][j * _nNSdim * _nNSdim + kdim * _nNSdim + kdim];}
      }
...
void MGSolNS::CalcVelDivergence ( double & Div, int PresOrder, int nPhi, int InterpNodeID, int SpaceDir, int el_ndof[] )
  {
  Div = 0.;
...
void MGSolNS::RowSetUp ( int nPhi, int indx_eq, int qp, int el_ndof[] )
  ...
  if ( _BoundEquation && _bc_el[indx_eq] == -3 ) {
      ...
      for ( int dim = 0; dim < _nNSdim; dim++ ) {
          _ProjDir[2][dim] = _normal_pt[dim + nPhi * _nNSdim];
          }
      }
...
void MGSolNS::InterpolateSolutions ( int el_ndof[] )
  ...
  interp_el_sol ( _data_eq[2].ub, 0, _data_eq[2].indx_ub[_data_eq[2].n_eqs], _phi_g[2], el_ndof[2], _ub_g[2] ); // field _ub_g[2][DIM]
  ...
  for ( int idim = 0; idim <  _nNSdim; idim++ ) {
      _u_1ts_g[idim] = _ub_g[2][_FF_idx[NS_F] + idim];
      }
