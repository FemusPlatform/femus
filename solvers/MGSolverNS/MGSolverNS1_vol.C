...
void MGSolNS::matrixrhsvol (
    ...
    double dphijdx_g2[DIMENSION], dphiidx_g2[DIMENSION], vel_gddx[DIMENSION*DIMENSION*DIMENSION], vel_gdx[DIMENSION*DIMENSION], vel_g[DIMENSION],u_nlg[DIMENSION];
...
//     }
        ...
        for ( int i=0; i< el_ndof2; i++ ) { // LOOP OVER TEST FUNCTIONS --------------------------------------------------
            ...
            for ( int  row_shift=0; row_shift< _nvars[2]; row_shift++ )    { // LOOP OVER ROWS: i + row_shift*el_ndof2
                ...
                if ( _bc_el[indx] == -8 ) {
//                     _bc_el[indx]=0;
                    std::cout<<"ATTENTION!!! el node "<<i<<" glob node "<<el_conn[i]<<" row "<<indx<<" bc "<<_bc_el[indx]<<"  "<<_bc_vol[i]<<std::endl;
		    ...
		    if(_bc_vol[i]==88) {
		      ...
		      FeM(indx) = 0.;
                ...
                if ( BoundEquation && _bc_el[indx] == -3 ) {
                    NormalEquation = true;
                }

                if ( BoundEquation && !NormalEquation ) {
                    CalcTangDir ( i, el_ndof[2], _bc_el[indx], qp, tang1, tang2, 1 );
                }
                    ...
                    for ( int  ivarN=row_shift; ivarN< row_shift+NComp; ivarN++ )     { // Loop over velocity components for equation written in row indx
                        ...
                        if ( mode == 1 ) { // rsh  terms
                            if ( _NS_parameter._SolveSteady == 0 ) {
                                FeM ( indx )  +=  dtxJxW_g* rho*vel_g[ivar]* ( phii_g+Phi_supg ) /_dt;
                            }
                            FeM ( indx )  +=  dtxJxW_g* rho*_IFr*_dirg[ivar]* ( phii_g+Phi_supg );
                        }
                        // -------------------------------------------------------------------------------------------------------------
                        //-------------------------------------- Assemblying matrix ----------------------------------------------------
                        for ( int j=0; j<el_ndof2; j++ ) {
                            ...
                            for ( int kdim=0; kdim< _nNSdim; kdim++ ) {
                                ...
                                Adv_g     += vel_g[kdim]*dphijdx_g2[kdim];                                                                  // Adv_g +=vel_g[kdim]*dphijdx_g2[kdim]*phii_g;
