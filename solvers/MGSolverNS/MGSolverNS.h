...
enum TANG_CALC_TYPE { GEOM_BASED = 0, VEL_BASED = 1 };

enum TIME_DER_TYPE { OLDER_STEPS = 0, ACTUAL_STEP = 1 };
...
class MGSolNS : public MGSolDA {
 ...
 protected:
  ...
  double _u_1ts_g[DIMENSION], _u_2ts_g[DIMENSION], _u_nl_g[DIMENSION], _vel_gdx[DIMENSION * DIMENSION],
      _vel_gddx[DIMENSION * DIMENSION * DIMENSION];
 ...
 public:
  ...
  virtual void matrixrhsvol(int el_ndof[], const int mode, int el_conn[]);
  ...
  void CalcMuTurbAndWallDist(
      double MuTurb[], double WallDist[], int WallNodes[], int ElId, int NumOfNodes, int Level);
  ...
  virtual void CalcAdv_Lap_LapSupg(
  ...
  virtual void CalcTimeDer(
      double& TimeDerivative, TIME_DER_TYPE Type, int SpaceDir, int nPhi, int interpNodeID = 0);

  // CONTINUITY EQUATION DISCRETIZATION
  virtual void ContinuityEquation(double JxW_g2, int el_ndof[]){};
  virtual void CalcVelDivergence(
