      ...
      if (_wall_frac > 1.e-5) {
        ...
        for (int j = 0; j < el_ndof[1]; j++) {  // loop over linear nodes - pressure laplacian
          ...
          for (int idim = 0; idim < _nNSdim; idim++)
            Lap += _dphi_g[1][j + idim * NDOF_P] * _dphi_g[1][i + idim * NDOF_P];
...
void MGSolNS_coup::MomentumEquation(double JxW_g2, int el_ndof[], int qp) {
  ...
  for (int i = 0; i < el_ndof[2];
    ...
    for (int row_shift = 0; row_shift < _nvars[2]; row_shift++) {  // LOOP OVER ROWS: i + row_shift*el_ndof2
      ...
      if ((_bc_el[indx] < 0 || (_bc_el[indx] == 0 && (_bc_vol[i] == 11 || _bc_vol[i] == 31))) && _ImmVal) {
        for (int ivarN = row_shift; ivarN < row_shift + _NComp;
          ...
          for (int j = 0; j < el_ndof[2]; j++) {
            ...
            _KeM(indx, j + ivar * el_ndof[2]) += dtxJxW_g * (Adv_g * (phii_g + _Phi_supg)  // time + advection
          ...
          for (int jp = 0; jp < el_ndof[1]; jp++) {
            ...
            //  B^T diagonal term
            _KeM(indx, jp + _nNSdim * el_ndof[2]) +=
                dtxJxW_g * (  // MPascal
        ...
        if (DirichletBC /*!_AlreadyWrittenDirBC[indx_row]*/) {
          ...
          if (bd_node == 88 || bd_node == 66) {
            ...
            for (int jvar = 0; jvar < _nNSdim; jvar++) {  //  -beta_n*n*( u.n)
              ...
              if (!NormBC) { alpha_beta = (_bc_el[indx_row] == 1) ? _ProjDir[0][jvar] : _ProjDir[1][jvar]; }
              ...
              if (_bc_el[indx] == -1 || _bc_el[indx] == -2) {
                if (bc_flag != 1) {
                  ...
                  for (int j = 0; j < elb_ndof[2]; j++) {
                    _KeM(indx, sur_toply[j] + ivar * el_ndof[2]) +=
                        -1. * stress * _phi_g[2][j];  /// ( modulus[j] );
                  ...
                  if (bc_varn_flag == 4)
                    for (int j = 0; j < elb_ndof[2]; j++)
                      for (int jvar = 0; jvar < _nNSdim; jvar++) {
                        _KeM(indx, sur_toply[j] + jvar * el_ndof[2]) -= alpha_beta * normal[ivar] *
                ...
                if (bc_varn_flag > 1 && bc_varn_flag < 4) {
                  for (int j = 0; j < elb_dof0; j++) {
                    ...
                    //                       FeM(indx)  -=
                    //                       det*_phi_g[2][i]*_phi_g[1][j]*_data_eq[_pres_order].ub[(_FF_idx[NS_F])
