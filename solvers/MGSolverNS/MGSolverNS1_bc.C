...
void MGSolNS::set_bc_matrix(
  ...
  double modulus[NDOF_FEMB];
  
  for(int l=0; l<NDOF_FEMB; l++){    
     modulus[l] = 0.;
     for(int dim=0; dim<DIMENSION;dim++){
         const int indj=sur_toply[l]+dim*el_ndof[2];
         modulus[l] += _data_eq[2].ub[(_FF_idx[NS_F]) *NDOF_FEM + indj]*_data_eq[2].ub[(_FF_idx[NS_F]) *NDOF_FEM + indj];
    }      
    modulus[l] = sqrt(modulus[l]);
  }
...
//           _AlreadyWrittenDirBC[indx_row] = true;
          ...
          if(bd_node == 88 || bd_node == 66) {
              KeM(indx_row,indx_row) = 1000000000000000.;
              FeM(indx_row)          = 0.;
            }
           else if(OrDir) {
  ...
  if(bc_side_n_flag<4 && bc_side_n_flag>1) {
    ...
    for(int i=0; i< elb_ndof[1]; i++) {
      ...
      FeM(indx)  +=  rhs_p*0.*Ipenalty*_data_eq[_pres_order].ub[_pres_order*sur_toply[i]];
...
//                 FeM (indx) += /*bc_rhs**/ stress*VelOnGauss[ivar];
                  for(int j=0; j<elb_ndof[2]; j++) {
                    KeM(indx,sur_toply[j] + ivar*el_ndof[2]) += -1.*stress*_phi_g[2][j] ;/// ( modulus[j] );
    ...
    if(bd_face==bd_node) {
      ...
      if(_bc_el[NormRow] == _BdFlagId) {  // only _BdFlagId=-8 (first time)
        if(Norm_Dirichlet_Bound_Cond) { _bc_el[NormRow] = 3; }
          ...
          if(bc_n_flag==1 || bd_node == 31) { _bc_el[NormRow] = 0; }   else { _bc_el[NormRow] = -3; }
        }
      }//  norm  ----------------------------------------------------------
      ...
      for(int kdir= NormDir+1; kdir<NormDir+_nvars[2]; kdir ++) {
        ...
        if(_bc_el[tg_dirRow]==_BdFlagId) {   // only _BdFlagId=-8 (first time)
          if(Tang_Dirichlet_Bound_Cond) {  dir_tg0 ++; _bc_el[tg_dirRow] = dir_tg0;   }// Dirichlet bc
          else { // Neumann bc (surface integration)
            if(bd_node == 11 || bd_node == 31) {_bc_el[tg_dirRow] = 0;} // for outflow bc we don't project the equation along normal-tangential directions
            else { NeuTg++; _bc_el[tg_dirRow] = -NeuTg; }
          }
        }
      }//  Tan ----------------------------------------------------------
    }// if (bd_face==bd_node)
...
void MGSolNS::CorrectBCFlags(
  ...
  int tg1 = (MaxNormal+1) %DIMENSION;  int tg2 = (MaxNormal+2) %DIMENSION;
