...
MeshExtended::MeshExtended(const ParallelObjectM& comm, MGUtils& mgutils, MGGeomEl& geomel)
    : MGMesh(comm, mgutils, geomel)
...
void MeshExtended::B_dist(const int Level) {
  ...
  for (int iproc = 0; iproc < _n_subdom; iproc++) {
    ...
    for (int iel = 0; iel < nel_e - nel_b; iel++) { _yplus[iel + nel_b] = 0.; }
  }
    ...
    for (int iprocb = 0; iprocb < _n_subdom; iprocb++) {  // LOOP OVER PROCS ----------------------------
      ...
      for (int ielb = 0; ielb < (nel_eb - nel_bb); ielb++) {  // LOOP OVER BOUNDARY ELEMENTS -------------
        ...
        for (int i = 0; i < NumberOfWalls; i++) {
          if (fabs(_bc_id[el_connb[NDOF_FEMB - 1]]) == WallGroupsIDs[i]) {
            IsAWallGr = true;
            break;
          }
        }
...
// med->libmesh map  second order only
  ...
  //
  //   // init  -------------------------------------------
  std::cout << "n_nodes " << n_nodes << " n_elements " << n_elements << " nodes_el " << nodes_el << std::endl;
  int icount = 0;
  int ielemcount = 0;
  int ibccount = 0;
  int bd_n_elements = 0;
  ...
  for (int i = 0; i < n_nodes; i++) {
    for (int idim = 0; idim < _dim; idim++) { coord[i * _dim + idim] = _xyz[i + idim * n_nodes]; }
  }

  //
  int* conn_bd;
  conn_bd = new int[n_elements * el_sides * pt_sides_bd];
  int* conn;
      ...
      for (int iside = 0; iside < el_sides; iside++) {
        if (el_neigh[iside] == -1) {  // domain boundary
          ...
          for (int idof = 0; idof < pt_sides_bd; idof++) {  // idof -> boundary node
            sur_toply[idof] =
                _GeomEl._surf_top[nodesinvbd[idof] + pt_sides_bd * iside];  // use map to find global node
  ...
  for (int i = 0; i < n_elements; i++) { mesh1->insertNextCell(MED_EL_TYPE, nodes_el, conn + i * nodes_el); }
  ...
  delete[] conn;
  elem_bd_id2.clear();
  vol_group_elements.clear();
  bd_group_elements.clear();
  for (int idel = 0; idel < n_bd_group; idel++) { g_bd[idel]->decrRef(); }
  delete[] g_bd;
  gr_bd.clear();
  for (int idel = 0; idel < n_vol_group; idel++) { g_vol[idel]->decrRef(); }
  delete[] g_vol;
  gr_vol.clear();
  return;
}
...
void MeshExtended::print_mat_hf5(
  ...
  int* ucoord;
  ucoord = new int[n_elements * n_subdivision];
  ...
  for (int iproc = 0; iproc < _n_subdom; iproc++) {
    ...
    for (int iel = 0; iel < _off_el[0][_NoLevels - 1 + iproc * _NoLevels + 1] - nel_b; iel++) {
      for (int is = 0; is < n_subdivision; is++) { ucoord[cel * n_subdivision + is] = _mat_id[iel + nel_b]; }
      cel++;
    }
  }
  ...
  hid_t dataset = H5Dcreate(
      file_id, dir_name.c_str(), H5T_NATIVE_INT, dataspace, H5P_DEFAULT
#if HDF5_VERSIONM != 1808
      ,
