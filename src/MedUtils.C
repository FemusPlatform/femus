...
MEDCoupling::MEDCouplingFieldDouble* MedUtils::InterpolatedField(
    const MEDCoupling::MEDCouplingFieldDouble* SourceField, int order) {
...
void MedUtils::GaussLoop(
  ...
  const int QPoints =
      ((DimRelToMax == 0) ? _fe[XOrd]->_NoGauss1[WeightDim] : _fe[XOrd]->_NoGauss1[WeightDim]);
  ...
  for (int qp = 0; qp < QPoints; qp++) {
    double det = (DimRelToMax == 0) ? _fe[XOrd]->Jac(qp, Coordinates, InvJac[XOrd])
                                    : _fe[XOrd]->JacSur(qp, Coordinates, InvJac[XOrd]);  //
    double JxW_g = det * _fe[XOrd]->_weight1[WeightDim][qp];                             // weight
    for (int i = 0; i < 3; i++) {
      _fe[i]->get_phi_gl_g(dim - DimRelToMax, qp, phi_g[i]);
...
MEDCoupling::MEDCouplingFieldDouble* MedUtils::GetCellField(
    const MEDCoupling::MEDCouplingFieldDouble* SourceField, int ConversionMode) {
