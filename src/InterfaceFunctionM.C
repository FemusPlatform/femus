...
InterfaceFunctionM::~InterfaceFunctionM() {
  ...
  if (_support_med) _support_med->decrRef();
      ...
      for (int i = 0; i < n_nodes_mg; i++) {  // cycle on mg nodes
        if (fabs(mesh->_bc_id[i]) == interface_id) {
          ...
          for (int idim = 0; idim < dim_mg; idim++)
            xyz_mg.push_back(mesh->_xyz[i + idim * n_nodes_mg]);  // d->getIJ(i, idim);
...
void InterfaceFunctionM::set_mesh_interface_nodeID(
  ...
  switch (order_cmp) {
    ...
    case 1:
      ...
      // build xyz_mg and nodeid_mg finding only linear nodes on boundary interface (interface_id) from mg
      // mesh
      for (int i = 0; i < n_nodes_mg; i++) {  // cycle on mg nodes
...
void InterfaceFunctionM::set_mesh_interface_elemID(
  ...
  int ndof_lev = 0;
  for (int ilev = 0; ilev < Level + 1; ilev++) { ndof_lev += _mesh_mg->_NoElements[0][ilev]; }
  ...
  for (int ielem = 0; ielem < n_elements_med; ielem++) {
    ...
    for (int inode = 0; inode < n_nod_el; inode++) {
      for (int idim = 0; idim < dim_mg; idim++) x_m[idim] += d->getIJ(nodes1[inode], idim);
    }                // end inode
    nodes1.clear();  // clear element node vector
    for (int idim = 0; idim < dim_mg; idim++) xyz_med[dim_mg * ielem + idim] = x_m[idim] / n_nod_el;
  }  // *************************************************************************
    ...
    for (int iel = nel_b; iel < nel_e; iel++) {
      if (fabs(mesh->_mat_id[iel]) == interface_id) {
        ...
        for (int inode = 0; inode < n_nod_el; inode++) {
          ...
          for (int idim = 0; idim < dim_mg; idim++) x_m[idim] += _mesh_mg->_xyz[el_conn + idim * n_nodes_mg];
        }
        for (int idim = 0; idim < dim_mg; idim++) xyz_mg[n_mat_elem * dim_mg + idim] = x_m[idim] / n_nod_el;
  ...
  for (int i_mg = 0; i_mg < n_mat_elem; i_mg++) {
    ...
    if (itdist == dist_med.end()) {
      std::cout << "non ci sono punti con toll_dist" << toll_dist << '\n';
      std::cout << xyz_mg[i_mg * dim_mg] << " " << xyz_mg[i_mg * dim_mg + 1] << " "
                << xyz_mg[i_mg * dim_mg + 2] << '\n';
      abort();
    }
    ...
    for (; itdist < dist_med.end(); itdist++) {
      ...
      for (int idim = 0; idim < dim_mg; idim++)
        dd += fabs(
            xyz_mg[i_mg * dim_mg + idim] -
            xyz_med[itdist->second * dim_mg + idim]);  // d->getIJ(itdist->second, idim));
      if (dd < min) {
        min = dd;
        k_min = itdist->second;
      }
  ...
  //   <<"\n"; std::map<int, int>::const_iterator itN = _elemID.begin(); for(; itN != _elemID.end(); itN++) {
...
void InterfaceFunctionM::set_analytic_field(
  ...
  if (symbolic_eq == std::string("")) {
    std::cout << "InterfaceFunctionM::set_analytic_field: NULL function";
    abort();
  }
...
void InterfaceFunctionM::set_analytic_field_elem(
  ...
  if (dim > 2) vars[2] = "z";
  if (symbolic_eq == std::string("")) {
    std::cout << "InterfaceFunctionM::set_analytic_field_elem: NULL function";
    abort();
  }
...
// ========================================================================
void InterfaceFunctionM::eval(
    int node_med, int n_cmp,
    double val[]) {  // ====================================================================
  if (_field == NULL) {
    for (int i = 0; i < n_cmp; i++) val[i] = 0.0;
  } else {
    //     int mid = _faceID[std::pair<int, int>(id, side)];
...
void InterfaceFunctionM::printOn(
  ...
  out << std::endl
      << "================================================== \n"
      << std::endl
      << " InterfaceFunctionM =" << id << std::endl
      << std::endl;
  // ---------------------------------------------------------------------
  // ---------------- Mapping ---------------------------------------------
  ...
  for (int i_mg = 0; i_mg < _n; i_mg++) {
    out << "  " << std::setw(4) << _map_mg[i_mg] << " <-> " << std::setw(4) << _map_med[i_mg] << std::endl;
  }
  out << std::endl;
  ...
  switch (type) {
    case MEDCoupling::ON_NODES:
      ...
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < nc; j++)
          out << (j == 0 ? "f( " : ", ") << std::setw(10) << std::fixed << d->getIJ(i, j);
        out << ") = ";
        for (int j = 0; j < ncf; j++)
          out << (j == 0 ? "( " : ", ") << std::setw(10) << std::fixed << v->getIJ(i, j);
        out << ")  ";
        out << std::endl;
      }
    ...
    case MEDCoupling::ON_CELLS:
      ...
      out << "==================================================" << std::endl;
      break;
    default: std::cout << "Error type"; abort();
  }
