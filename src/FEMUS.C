    ...
    : _comm(MPI_COMM_WORLD)  // MPI_COMM_WORLD communicator
{
  ...
  FieldClass.FillEquationMap(
      get_MGExtSystem());  // Init the equation class objects and set systems for each class
...
double FEMUS::GetValue(const int& ff, int flag) { _mg_equations_map->GetValue(ff, flag); }
//=============================================================================
...
/*MGEquationsSystem &*/ void FEMUS::init_equation_system(
    ...
    int n_data_cell)  // ==========================================================================
{
  _mg_equations_map = new EquationSystemsExtendedM(
      *_mg_utils, *_mg_mesh, *_mg_femap, n_data_points, n_data_cell);  // MGEquationsMap class
...
void FEMUS::set_mesh()  // ==========================================================================
{
  ...
  if (NoLevels != _mg_mesh->_NoLevels) {
    std::cout << "Inconsistent Number of Levels between Mesh and SolBase" << std::endl;
    abort();
  }
...
void FEMUS::solve_and_update(
  ...
  _mg_time_loop->transient_solve_and_update(
      t_in, t_step, print_step, time, dt, eq_min, eq_max);  ///< step time
  return;
}
...
{
  _mg_time_loop->transient_control_onestep(
      nmax_step, it, t_step, print_step, time, dt, eq_min, eq_max, converged);  ///< step time
  return;
}
// This function solves one step  for transient problems
void FEMUS::solve_control_onestep(
...
{
  _mg_time_loop->transient_control_onestep(
      nmax_step, it, t_step, print_step, time, dt, eq_min, eq_max, controlled_eq, converged,
      toll);  ///< step time
  return;
}
// ========================================================================
...
{
  _mg_time_loop->transient_underrelaxed_onestep(
      it, t_step, print_step, time, dt, eq_min, eq_max, controlled_eq, converged, toll);  ///< step time
  return;
}
