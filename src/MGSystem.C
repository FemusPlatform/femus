...
void MGSystem::read_data(const std::string& file_name  // <-file name
  ...
  if (!inf) {  // autogenerate data file
    ...
    for (int kc = 0; kc < _n_data[0] + _n_data[1]; kc++) {
      ...
      Name << "DATA" << kc;
      status = H5Dread(
          H5Dopen(file_id, Name.str().c_str()), H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT,
          _sys_data[kc]);
      assert(status == 0);
    }
  }
...
void MGSystem::print_data_view(
  ...
  int n_elements = _mgmesh._NoElements[0][top_lev - 1];
  double* ucoord;
  ucoord = new double[n_elements * 4 * (DIMENSION - 1)];
  // storage DIST -> ucoord -----------------------------
  int cel = 0;
  ...
  hsize_t dimsf[2];
  dimsf[0] = n_elements * 4 * (DIMENSION - 1);
  dimsf[1] = 1;

  hid_t dataspace = H5Screate_simple(2, dimsf, NULL);
  hid_t dataset = H5Dcreate(file_id, dir_name.c_str(), H5T_NATIVE_DOUBLE, dataspace, H5P_DEFAULT);
  hid_t status = 0;
...
void MGSystem::print_data(const std::string& filename) const {
  ...
  for (int i = 0; i < _n_data[0] + _n_data[1]; i++) {
    ...
    hid_t dataspace = H5Screate_simple(2, dimsf, NULL);
    hid_t dataset = H5Dcreate(fileP, name.str().c_str(), H5T_NATIVE_DOUBLE, dataspace, H5P_DEFAULT);
  ...
  H5Fclose(fileP);
#ifdef PRINT_INFO
  std::cout << " Print data(MGSys): pt " << _n_data[0] << " cell " << _n_data[1] << " field(s) into "
            << name1.str() << " \n";
#endif
  return;
}
