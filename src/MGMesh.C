...
//       hid_t  file_id     = H5Fopen ( meshname.str().c_str(), H5F_ACC_RDWR, H5P_DEFAULT );
  ...
  hid_t  status = 0;
//       int  topdata[2];
  status = H5Dread(H5Dopen(file_id, setdata_name.c_str()      //
...
//           _dxdydz[inode+kc*n_nodes]=0.; //nondimensionalization
    ...
    status = H5Dread(H5Dopen(file_id,"/NODES/MAP/OFF_ND",H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, _off_nd[0]);
    _off_nd[1] = new int[_n_subdom * _NoLevels + 1];
    status = H5Dread(H5Dopen(file_id, "/NODES/MAP/OFF_ND1",H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,_off_nd[1]);

    // Reading  _NoElements --------------
    _off_el = new int * [_NoFamFEM];             //volume and boundary
    _off_el[0] = new int [_n_subdom * _NoLevels + 1]; //offset for VOLUME
    status = H5Dread(H5Dopen(file_id, "/ELEMS/FEM0/OFF_EL", H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,_off_el[0]);
    _off_el[1] = new int [_n_subdom * _NoLevels + 1]; //offset for boundary
    status = H5Dread(H5Dopen(file_id, "/ELEMS/FEM1/OFF_EL", H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,_off_el[1]);
    ...
    for(int  ifem = 0; ifem < _NoFamFEM; ifem++) {
      ...
      status = H5Dread(H5Dopen(file_id, Name.str().c_str(), H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, _NoElements[ifem]);
    }
    ...
    for(int  ilev = 0; ilev < _NoLevels; ilev++) {    // maps from o to 2*_NoLevels-1
      ...
      // inverse map
      for(int  inode = 0; inode < n_nodes_top; inode++)  if(temp[inode] != -1) _node_map[ilev][temp[inode]] = inode;
    }
    ...
    status = H5Dread(H5Dopen(file_id, Name.str().c_str(), H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, temp);  // read from file
    ...
    status = H5Dread(H5Dopen(file_id, "/ELEMS/FEM0/MSH", H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, _el_map[0]);

    _el_map[1] = new int  [_off_el[1][_NoLevels * _n_subdom]*NDOF_FEMB]; // Boundary
    status = H5Dread(H5Dopen(file_id, "/ELEMS/FEM1/MSH", H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, _el_map[1]);
...
//     const int n_sides=_GeomEl._n_sides[0];
    ...
    status = H5Dread(H5Dopen(file_id, "/ELEMS/FEM0/EL_NEIG", H5P_DEFAULT),H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, _el_neighbor[0]);
  ...
  for(int pr = 0; pr < _n_subdom; pr++) {
    ...
    for(int iel = 0; iel < (nel_e - nel_b); iel++) {
      ...
      for(int idim = 0; idim < _dim; idim++) {    // quad loop entities (vector)
        x_m[idim] = 0.;   for(int d = 0; d < NDOF_FEM; d++)   x_m[idim] += xx_qnds[idim *NDOF_FEM + d] /NDOF_FEM;
      }
...
//         set_bc_matrix(dir_maxnormal, sur_toply, el_ndof, elb_ndof, elb_ngauss, normal, el_conn);
          ...
          for(int  lbnode = 0; lbnode < NDOF_FEMB; lbnode++) {
            int in=    elb_conn[lbnode];  int ibc=_NodeBDgroup[in];
            int in_mid=    elb_conn[ NDOF_FEMB-1];  int ibc_mid=_NodeBDgroup[in_mid];
            // Note; ibc == ibc_mid geometric -> normals based on the Group regions
            for(int ivar = 0; ivar <_dim ; ivar++)   if(ibc == ibc_mid) _normalb[in*DIMENSION+ivar]= normal[ivar];
...
//  normal= normal_tg[0;_dim-1] tangent = normal_tg[_dim;2*_dim-1]
  ...
  double sp=0.; for(int kdim=0; kdim<_dim; kdim++) sp += normal_tg[kdim]* (x_c[kdim]-xx[0+kdim*NDOF_FEMB]) ;
...
//  tangent2 = normal_tg[_dim;2*_dim-1]
  int idx_tg1=_dim; int idx_tg2=2*_dim;
...
//   _mgutils.cross(tg01,tg03,normal_g);
  for(int i=0; i<_dim; i++)   normal_tg[i]=normal_tg[(i+1)%3+idx_tg1]*normal_tg[(i+2)%3+idx_tg2]-normal_tg[(i+2)%3+idx_tg1]*normal_tg[(i+1)%3+idx_tg2];
...
//     normal_tg[2]=normal_tg[0+idx_tg1]*normal_tg[1+idx_tg2]-normal_tg[1+idx_tg1]*normal_tg[0+idx_tg2];
  ...
  for(int i=0; i<_dim; i++)   normal_tg[i+idx_tg2]=normal_tg[(i+1)%3]*normal_tg[(i+2)%3+idx_tg1]-normal_tg[(i+2)%3]*normal_tg[(i+1)%3+idx_tg1];

#endif
// normalization -----------------------
  for(int in=0; in< _dim; in++) {
    double mm=0.;  for(int kdim=0; kdim< _dim; kdim++) mm +=normal_tg[kdim+ in*_dim]*normal_tg[kdim+in*_dim];
    mm=sqrt(mm+1.e-20); for(int idim=0; idim< _dim; idim++) normal_tg[idim+ in*_dim] /=mm;
  }
  return;
...
//         set_bc_matrix(dir_maxnormal, sur_toply, el_ndof, elb_ndof, elb_ngauss, normal, el_conn);
          ...
          for(int  lbnode = 0; lbnode < NDOF_FEMB; lbnode++) {
            int in=    elb_conn[lbnode];int idofu=node_dof[in]; int ibc=  bc[idofu]; int ibc0=_NodeBDgroup[in];
            int in_mid=    elb_conn[ NDOF_FEMB-1]; int idofu_mid=node_dof[in_mid]; int ibc_mid=  bc[idofu_mid]; int ibc_mid0=_NodeBDgroup[in_mid];
            
            // Note; ibc == ibc_mid geometric -> normals based on the Group regions
             if(ibc == ibc_mid && ibc0 != ibc_mid0) for(int ivar = 0; ivar <_dim ; ivar++)  {
                _normalb[in*DIMENSION+ivar]= normal[ivar];
                  printf(" Mesh normal corrected : %d %d  %f %f %d %d \n",_iproc,in,normal[0],normal[1], ibc,ibc_mid);
            }
...
//         set_bc_matrix(dir_maxnormal, sur_toply, el_ndof, elb_ndof, elb_ngauss, normal, el_conn);
          ...
          for(int  lbnode = 0; lbnode < NDOF_FEMB; lbnode++) {
                 int in=    elb_conn[lbnode];int idofu=node_dof[in];int idofv=node_dof[in+offset]; int ibc=  bc[idofu]; int ibc0=_NodeBDgroup[in];
            int in_mid=    elb_conn[ NDOF_FEMB-1]; int idofu_mid=node_dof[in_mid]; int ibc_mid=  bc[idofu_mid]; int ibc_mid0=_NodeBDgroup[in_mid];
            int xibc=ibc;
            
            // Note; ibc == ibc_mid geometric -> normals based on the Group regions
             if(ibc != ibc_mid &&  xibc!=11) {
                 int nor_f=ibc/10;  int tg_f=ibc%10;
                  int nor_midf=ibc_mid/10;  int tg_midf=ibc_mid%10;
           ...
           if(ibc == ibc_mid && (ibc0 != ibc_mid0))   for(int ivar = 0; ivar <_dim ; ivar++)  {
                _normalb[in*DIMENSION+ivar]= normal[ivar];
                  printf(" Mesh normal corrected : %d %d  %f %f %d %d from %d \n",_iproc,in,normal[0],normal[1], ibc,ibc_mid, xibc);
            }
...
//            sp[ivar]=0;
//            for(int kvar = 0; kvar < _nNSdim ; kvar++)    sp[ivar] += vel_field[kvar]*normal[kvar+ivar*_dim];
//        }
//
//
