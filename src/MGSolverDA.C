...
void MGSolDA::init(const int Level) {
  ...
  disp[Level]->init(n_glob, n_local, false, AUTOMATICM);
  ...
  if (!in) {  // ------- function initial condition -> user fuinction -> ic_read
    ...
    for (int iel = 0; iel < off_el[off_proc + _NoLevels] - off_el[off_proc + _NoLevels - 1]; iel++) {
      ...
      for (int i = 0; i < NDOF_FEM; i++) {
        ...
        ic_read(
            face_id_node, mat_id_elem, xp, iel + off_el[off_proc + _NoLevels - 1],
            u_value);  //-----------------------==
        // -------------------------------------------------==
        // ===================================================
...
void MGSolDA::GenBc_loop(
  ...
  for (int isub = 0; isub < _mgmesh._n_subdom; ++isub) {
    ...
    for (int iel = 0; iel < delta; iel++) {  // element loop
      ...
      for (int i = 0; i < ndof_femv; i++) {                              // node lement loop
        ...
        //                 std::cout<<"i  "<<i<<"  k "<<k<<"   _node_dof[_NoLevels-1][k]
        //                 "<<_node_dof[_NoLevels-1][k]<<std::endl;
        // coordinates
        if (_node_dof[_NoLevels - 1][k] > -1) {
          ...
          _bc[0][_node_dof[_NoLevels - 1][k]] = (_bc[0][_node_dof[_NoLevels - 1][k]] == -1000)
                                                    ? bc_Neu[0]
                                                    : (_bc[0][_node_dof[_NoLevels - 1][k]]);
      ...
      for (int iside = 0; iside < el_sides; iside++) {
        if (el_neigh[iside] == -1) {
          ...
          // //             int score=0;
          // //             int  bc_id=bc[0][ _node_dof[_NoLevels-1][_mgmesh._el_map[vb][(iel+iel0)
          // *ndof_femv+ndof_femv-1-ik]]];
          // //             score +=(bc[0][
          // _node_dof[_NoLevels-1][_mgmesh._el_map[vb][(iel+iel0)*ndof_femv+2-ik]]]    == bc_id)?1:0;
          // //             score +=(bc[0][ _node_dof[_NoLevels-1][_mgmesh._el_map[vb][(iel+iel0)
          // *ndof_femv+(3-ik)%3]]]== bc_id)?1:0;
          // //             if(score<score_old) {
  ...
  //
  //         for(int i=0; i< ndof_femv; i++)  // node lement loop
  //           for(int idim=0; idim< DIMENSION; idim++) { xxb_qnds[i+ndof_femv*idim]=
  //           xyzgl[_mgmesh._el_map[vb][(iel+iel0) *ndof_femv+i]+idim*offset]; }
  //
  //
  //         // normal
  //         _fe[2]->normal_g(xxb_qnds,normal);
  ...
  //             int score=0;
  //             int  bc_id=bc[0][ _node_dof[_NoLevels-1][_mgmesh._el_map[vb][(iel+iel0)
  //             *ndof_femv+ndof_femv-1-ik]]]; score +=(bc[0][
  //             _node_dof[_NoLevels-1][_mgmesh._el_map[vb][(iel+iel0)*ndof_femv+2-ik]]]    == bc_id)?1:0;
  //             score +=(bc[0][ _node_dof[_NoLevels-1][_mgmesh._el_map[vb][(iel+iel0)
  //             *ndof_femv+(3-ik)%3]]]== bc_id)?1:0; if(score<score_old) {
  //               score_old=score; k_el=_mgmesh._el_map[vb][(iel+iel0) *ndof_femv+ndof_femv-1-ik];
  //               bc_el=bc[0][ _node_dof[_NoLevels-1][k_el]];
  //             }
  //           }
  ...
  //             for(int ivar=0; ivar<_nvars[2]; ivar++) {// quad el --
  //               int kdof=_node_dof[_NoLevels-1][k+(ivar)* offset];
  //               if(ivar+_dir == (bc_id_pts-1)%DIMENSION) { bc[1][kdof] =(bc_id/10)+(ivar+_dir+1)*10; }  //
  //               normal= bc_id_pts-1 else {   bc[1][kdof] =bc_id%10; }  // tang
  //             }
  //
  //           }  // end ------(bc_id > 99)
  //           else { //   pts label 00 ----------------------------------------------------------
  ...
  // //                 int kdof= _node_dof[_NoLevels-1][k+ (ivarp) * offset]; int tdof=
  // _node_dof[_NoLevels-1][k];
...
void MGSolDA::interp_el_gdx(
  ...
  for (int ivar = 0; ivar < nvars; ivar++) {
    ...
    for (int eln = 0; eln < n_shape; eln++) {
      for (int jdim = 0; jdim < DIMENSION; jdim++) {
        uold_dx[ivar * DIMENSION + jdim] +=
            dphi[eln + jdim * n_shape] * uold_b[eln + (ivar + ivar0) * NDOF_FEM];
      }
    }
  }
  ...
  for (int ivar = 0; ivar < _nvars[0]; ivar++) {
    ...
    for (int iproc = 0; iproc < _mgmesh._n_subdom; iproc++) {
      ...
      for (int iel = 0; iel < delta; iel++) {
        ...
        double val =
            (*x_old[Level])(_node_dof[Level][indx * _el_dof[0] + (ivar + _nvars[2] + _nvars[1]) * offset]) *
            _refvalue[_nvars[0]];
      ...
      for (int iproc = 0; iproc < _mgmesh._n_subdom; iproc++)
        for (int iel = 0; iel < _mgmesh._off_el[0][iproc * _NoLevels + Level + 1] -
          ...
          for (int i = 0; i < el_nds; i++) {            // linear and quad
            ...
            switch (vec_from) {
              case 1: value = (*x_old[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 2: value = (*x_oold[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 3: value = (*x_ooold[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 4: value = (*x_nonl[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 5: value = (*disp[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 6: value = (*disp_old[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 7: value = (*disp_oold[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 8: value = (*disp_ooold[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              case 9: value = (*x_oooold[Level])(_node_dof[_NoLevels - 1][k + ivar * offset]); break;

              default: cout << "Incorrect vec_from number in set_uoold function" << endl; break;
            }
...
void MGSolDA::read_u(
  ...
  if (Level_restart != 0) {
    ...
    namefile_sol << resu_dir << "/sol.msh1." << setw(ndigits) << setfill('0') << restart
                 << ".h5";  //  namefile_sol
    ...
    for (int iel = 0; iel < _mgmesh._off_el[0][_iproc * _NoLevels + Level + 1] -
                                _mgmesh._off_el[0][_iproc * _NoLevels + Level];
         iel++) {
    ...
    for (int ivar = 0; ivar < _n_vars; ivar++) {
      ...
      for (int iel = 0; iel < _mgmesh._off_el[0][_iproc * _NoLevels + _NoLevels] -
                                  _mgmesh._off_el[0][_iproc * _NoLevels + _NoLevels - 1];
           iel++) {
        ...
        for (int i = 0; i < el_nds; i++) {            // linear and quad
          int k = _mgmesh._el_map[0][elem_gidx + i];  // the global node
          x[_NoLevels - 1]->set(
              _node_dof[_NoLevels - 1][k + ivar * offset],
              sol_f[i] * Irefval);  // sol[k]*Irefval);    // set the field
        }
      }
    }
  ...
  for (int iel = 0; iel < (nel_e - nel_b); iel++) {
    ...
    for (int eq = 0; eq < _data_eq[2].n_eqs; eq++) {
      _data_eq[2].mg_eqs[eq]->get_el_sol(
          0, _data_eq[2].indx_ub[eq + 1] - _data_eq[2].indx_ub[eq], _el_dof[2], el_conn, offset,
...
void MGSolDA::ActivateVectField(
  ...
  if (coupled == 0) {
    ...
    if (SystemFieldName.compare("TAU") == 0) {
      std::string FieldX = SystemFieldName + "XX";
      std::string FieldY = SystemFieldName + "XY";
      std::string FieldZ = SystemFieldName + "YY";
      ActivateEquation(Order, Field, FieldX, n_index);
      ActivateEquation(Order, Field + 1, FieldY, n_index);
      ActivateEquation(Order, Field + 2, FieldZ, n_index);
    }
...
void MGSolDA::ActivateControl(
  ...
  if (vector == 1) {
    ...
    if (dimension == 3) ActivateEquation(Order, Field + 2, FieldZ, n_index);
  }
