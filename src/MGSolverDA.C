...
//       _data_eq[deg].mg_eqs.insert (std::pair<int,MGSolBase *>(kl,NULL) );
...
// #ifdef PRINT_INFO // ====================================================
//   std::cout<< "\n ========================== \n"<<
//            " Equation system = "<< _eqname.c_str()  <<" \n";
// //     <<
// //            " Finite Element= 1Q " << "\n";
// //   std::cout<< " -------------------------- \n"<<
// //            " Nondimensional parameters: \n"<<
// //            " Reynolds number = " << 1./_IRe <<"\n"<<
// //            " Froud number    = " << 1./_IFr <<"\n";
//   std::cout<< " -------------------------- \n"<< " External fields: \n";
...
//   double normal[DIMENSION];
  ...
  double normal[DIMENSION];
  int   el_neigh[NDOF_FEM]; int   el_flag[NDOF_FEM];
...
//          for(int ivar=0; ivar<n_dofs; ivar++)  {
	...
	int old_val =  bc[0][_node_dof[_NoLevels-1][k]] ;
        ...
        for(int ivar=0; ivar<n_dofs; ivar++)  {
          ...
          if(abs(number)==1) { bc[1][kdof] =bc[1][kdof] - bc[1][kdof]*10000/abs(bc[1][kdof]); } // if the count is 1 set 0 if >1 leave
        }
	  ...
	  bc_face = bc_face%100;
          ...
          for(int i=0; i< NDOF_FEMB; i++) {  // node lement loop
            ...
            if((bc_id%1000)/100 > 3) {  //   pts label x00 ----------------------------------------------------------
              std::cout << "\n ... single pressure pts .. \n";
              if(i<NDOF_PB) { bc[1][_node_dof[_NoLevels-1][k+DIMENSION* offset]] =4; }
...
//                            const int nvars_l           // # linear variables
  ...
  int lev[3]; lev[0]=Level; lev[1]=lev_c; lev[2]=Level;
...
//       int  jlq=0;
  ...
  for(int  i=0; i<m_l; i++) {n_dz[i]=0; n_oz[i]=0;}  //initialize to zero
  ...
  for(int  i=0; i<m_l; i++) {n_dz[i] -= n_oz[i];}  //count correctly diagonal entries
...
void MGSolDA::ReadProl(
  ...
  int off_proc=_NoLevels*_iproc; int ml[3];   // row processor dofs
  ...
  int ml_init[3]; ml_init[0]=0; ml_init[1]=0; ml_init[2]=0; //proc initial dofs [0]=const [1]=linear [2] quad
  for(int isubd=0; isubd<_iproc; isubd++) {
  ...
  for(int iql=0; iql<3; iql++) {  //type of variable loop (0=k,1=l,2=q)
    ...
    for(int ivar=0; ivar<nvars_in[iql]; ivar++) {  //loop on variables of iql type
      ...
      for(int i=ml_init[iql]; i<ml_init[iql]+ml[iql]; i++) {  //row loop
        ...
        tmp[0]=irow; std::vector<int> ind(ncol);            // row-col indeces
...
void MGSolDA::ReadRest(
  ...
  for(int jql=0; jql<3; jql++) {
    ...
    if(status==0) {dim_qlk[jql][0]=ldim[0]; dim_qlk[jql][1]=ldim[1];}
    else {std::cout<< "  MGSolDA::ReadRest: error reading dimension "; abort();}
  }
  ...
  int off_proc=_NoLevels*_iproc; int ml[3];
...
//   virtual double MGSolDA::eval_var2(double in[]){return 0.;}
//   virtual double MGSolDA::eval_var3(double in[]){return 0.;}
// #endif
void MGSolDA::ActivateVectField(std::string FieldName, FIELDS Field, std::vector<FIELDS> pbName, std::string SystemFieldName, int &n_index, int n_equations) {
...
void MGSolDA::ActivateControl(
    std::string FieldName,      // field name from SimulationConfiguration.in
    FIELDS Field,               // Field to activate
    std::vector<FIELDS> pbName, // problem name fields
    std::string SystemFieldName, // system field name
    int &n_index,                // n_index (collecting index)
    int n_equations              // number of equations for this field
) {//===============================================================================================
