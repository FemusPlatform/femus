...
void FEMUS::init_interface(
  ...
  if (on_nodes) {
    ...
    tmp = MEDCoupling::ReadFieldNode(
        filename.c_str(), "Mesh_Lev_" + to_string(FinerLevel), 0,
    ...
    tmp = MEDCoupling::ReadFieldCell(
        filename.c_str(), "Mesh_Lev_" + to_string(FinerLevel), 0, "MG_cell_id_Lev_" + to_string(FinerLevel),
        -1, -1);
  }

  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingFieldDouble> FieldContainingMap(
      MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingField, MEDCoupling::MEDCouplingFieldDouble>(tmp));
  ...
  if (IDSvec[0] < 10) { id_level = 0; }

  support =
      MEDCoupling::ReadUMeshFromGroups(localFile.c_str(), "Mesh_Lev_" + to_string(FinerLevel), id_level, vG);
...
void FEMUS::init_interface(
  ...
  int id_level = 0;
  MEDCoupling::MEDCouplingUMesh* support =
      MEDCoupling::ReadUMeshFromFile(localFile.c_str(), "Mesh_Lev_" + to_string(FinerLevel), id_level);
...
void FEMUS::getNodeMapAndProcMeshAtLevel(
    int level, MEDCoupling::MEDCouplingUMesh*& FemusPar, MEDCoupling::MEDCouplingFieldDouble*& NodeMap) {
  ...
  MEDCoupling::MEDCouplingUMesh* FemusPart = MEDCoupling::ReadUMeshFromFile(
      filename.c_str(), "Mesh_Lev_" + to_string(level), 0);  // ORIGINAL FEMUS FROM MED FILE
  ...
  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingFieldDouble> ProcField(
      MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingField, MEDCoupling::MEDCouplingFieldDouble>(tmp));
  ...
  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingFieldDouble> FinerLevelIDS(
      MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingField, MEDCoupling::MEDCouplingFieldDouble>(tmp1));
  ...
  FemusPar = FemusPart->buildPartOfMySelf(
      CellId, CellId + CellIdProc.size());  // EXTRACTION OF PROC MESH FROM GLOBAL MESH
  FemusPar
      ->zipCoords();  // It changes nodes numbering -> new numbering is from 0 to FemusPar->getNumberOfNodes
...
void FEMUS::init_interface(
  ...
  for (int i = 0; i < celle; i++) { InterfaceMesh->getNodeIdsOfCell(i, MedNodesIds); }
  ...
  if (interface_id == 0)
    printf(
        "\n \033[1;31m Interface %d support set to mesh without volume group and order %d and nodes "
        "%d\033[0m\n",
        interface_name, order_cmp, celle);
  else
    std::cout << "\n \033[1;31m Interface " << interface_name << " support set to mesh with interface "
              << interface_id << " and order " << order_cmp << " and nodes " << NODI << "\033[0m\n";
...
void FEMUS::setMedMesh() {
  ...
  for (int lev = 0; lev < FinerLevel + 1; lev++) {
    MEDCoupling::MEDCouplingUMesh* FemusPart = MEDCoupling::ReadUMeshFromFile(
        dataFile, "Mesh_Lev_" + to_string(lev), 0);  // ORIGINAL FEMUS FROM MED FILE
    MEDCoupling::MCAuto<MEDCoupling::MEDCouplingField> tmp =
        MEDCoupling::ReadFieldCell(/*MEDCoupling::ON_CELLS,*/
                                   dataFile, "Mesh_Lev_" + to_string(lev), 0, "Proc_Lev_" + to_string(lev),
                                   -1, -1);  // PIECEWISE FIELD CONTAINING PROC IDS
    ...
    MEDCoupling::MCAuto<MEDCoupling::MEDCouplingField> tmp2 =
        MEDCoupling::ReadFieldCell(/*MEDCoupling::ON_CELLS,*/
                                   dataFile, "Mesh_Lev_" + to_string(lev), 0,
                                   "MG_cell_id_Lev_" + to_string(lev), -1, -1);
    MEDCoupling::MCAuto<MEDCoupling::MEDCouplingFieldDouble> CellField(
        MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingField, MEDCoupling::MEDCouplingFieldDouble>(tmp2));

    double* MgCellId = const_cast<double*>(CellField->getArray()->getPointer());

    MEDCoupling::MCAuto<MEDCoupling::MEDCouplingField> tmp1 =
        MEDCoupling::ReadFieldNode(/*MEDCoupling::ON_NODES,*/
                                   dataFile, "Mesh_Lev_" + to_string(lev), 0,
                                   "FinerLevelNodeIDS_Lev_" + to_string(lev), -1, -1);
    MEDCoupling::MCAuto<MEDCoupling::MEDCouplingFieldDouble> FinerLevelIDS(
        MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingField, MEDCoupling::MEDCouplingFieldDouble>(tmp1));
    ...
    MEDCoupling::MEDCouplingUMesh* FemusPar = FemusPart->buildPartOfMySelf(
        CellId, CellId + CellIdProc.size());  // EXTRACTION OF PROC MESH FROM GLOBAL MESH
    FemusPar
        ->zipCoords();  // It changes nodes numbering -> new numbering is from 0 to FemusPar->getNumberOfNodes
    ...
    MEDCoupling::WriteFieldUsingAlreadyWrittenMesh(
        "RESU_MED/FEMUS_PAR_" + to_string(proc) + ".med", _NodeMap[lev]);
...
// =============================================================================
void FEMUS::setExtField(const std::string& systemName, MEDCoupling::MEDCouplingFieldDouble* bcField) {
...
void FEMUS::setFieldSource(
    int interface_name, int n_cmp, const MEDCoupling::MEDCouplingFieldDouble* srcField) {
...
MEDCoupling::MEDCouplingFieldDouble* FEMUS::getProcSolution(  // field (out)
  ...
  MyAssert(
      _NodeMap.size() > Level, "FEMUS::getProcSolution _NodeMap not initialized for level " +
                                   to_string(Level) + " maximum levels " + to_string(_NodeMap.size()));
...
const MEDCoupling::MEDCouplingUMesh* FEMUS::getUMesh(int name) {
  return _mg_equations_map->getUMeshCoupling(name);
}
// // // ===========================================================================
/// This function gets the original mesh of FEMUS problem
const MEDCoupling::MEDCouplingUMesh* FEMUS::getUMesh_orig(int name) {
...
void FEMUS::update_interface(
    const int interface_name, int n_cmp,
    const std::vector<MEDCoupling::MEDCouplingFieldDouble*>&
        srcField) {  // =========================================================================
  MEDCoupling::DataArrayDouble* mg_disp =
      MEDCoupling::DataArrayDouble::Meld(srcField[0]->getArray(), srcField[1]->getArray());

  if (n_cmp == 3) { mg_disp = MEDCoupling::DataArrayDouble::Meld(mg_disp, srcField[2]->getArray()); }

  std::cout << "FEMUS::UPDATE: Tuples  src_field  " << mg_disp->getNumberOfTuples() << "  Comp    "
            << mg_disp->getNumberOfComponents() << " and x value is " << mg_disp->getIJ(0, 0)
            << " and y value is " << mg_disp->getIJ(0, 1) << std::endl;
  std::cout << "FEMUS::UPDATE: support  set to boundary with name " << interface_name << "\n";
...
void FEMUS::SetPieceFieldOnYdist(MEDCoupling::MEDCouplingFieldDouble* Field) {
  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingField> tmp = MEDCoupling::ReadFieldCell(
      /*MEDCoupling::ON_CELLS,*/ "MESH/" + _mg_utils->_interface_mesh,
      "Mesh_Lev_" + std::to_string(_mg_mesh->_NoLevels - 1), 0,
      "MG_cell_id_Lev_" + std::to_string(_mg_mesh->_NoLevels - 1), -1, -1);

  MEDCoupling::MCAuto<MEDCoupling::MEDCouplingFieldDouble> CellMap(
      MEDCoupling::DynamicCast<MEDCoupling::MEDCouplingField, MEDCoupling::MEDCouplingFieldDouble>(tmp));
