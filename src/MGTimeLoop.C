...
// =============================================================================
/// Constructor
MGTimeLoop::MGTimeLoop(MGUtils& mgutils_in, EquationSystemsExtendedM& mgeqmap_in)
    ...
    )                    // ========================================================================
{
  ...
  if (restart) {
    ...
    t_in = restart;
    time += restartime;  // set initial time
    std::cout << "\n *+*+* MGTimeLoop::transient_setup: RESTART  " << restart << " time= " << restartime
              << std::endl;
  }
  return;
}
...
{
  _mgeqmap.set_uooold(
      vec_from, vec_to, toll, time, eq_min, eq_max);  // copy values from 'vec_from' to 'vec_to'
...
//   time += dt;
  ...
  std::cout << " Time solver ----->= " << double(end_time - start_time) / CLOCKS_PER_SEC
            << " Time printing ----->= " << double(end_time2 - end_time) / CLOCKS_PER_SEC << std::endl;
#endif  // ----------------------------------------------------------------------
...
// A Soving the system ****************************************************************
  ...
  _mgeqmap.eqnmap_timestep_loop_control(
      nmax_step, it, dt, eq_min, eq_max, converged);  // solve one step control
...
// A Soving the system ****************************************************************
  ...
  _mgeqmap.eqnmap_timestep_loop_control(
      nmax_step, it, dt, eq_min, eq_max, controlled_eq, converged, toll);  // solve one step control
...
// A Soving the system ****************************************************************
  ...
  _mgeqmap.eqnmap_timestep_loop_underrelaxed(
      it, dt, eq_min, eq_max, controlled_eq, converged, toll);  // solve one step control
    ...
    )           // ========================================================================
{
  ...
  Name << _mgutils._inout_dir << basetime << "." << setw(ndigits) << setfill('0') << t_init << ".xmf";
  ...
  for (int kp = 0; kp < nprt; kp++) {
    ...
    for (int it = t_init; it <= t_init + nsteps; it = it + print_step) { /*if(it%print_step ==0)*/
      out << "<xi:include href=\"" << basesol << "." << setw(ndigits) << setfill('0') << it << ".xmf"
          << "\""
  ...
  //         out << "<Domain> \n";
  //         for (int kp=0;kp< nprt; kp++)    {
  //           out << "<Grid Name=\""<< gname[kp].c_str() <<"\"  GridType=\"Collection\"
...
// A Soving the system ****************************************************
  ...
  std::clock_t start_time = std::clock();
#endif  // -------------------------------------------------------------------
  std::cout << "\n  ** Dumming time step " << t_step << ", time = " << time << " ***" << std::endl;
