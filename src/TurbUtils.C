...
TurbUtils::~TurbUtils()
{
     ...
     if ( sizeN != 0 ) for ( int Level=0; Level<sizeN; Level++ ) {
               _NodeWallDist[Level]->decrRef();
          }
     if ( sizeM != 0 ) for ( int Level=0; Level<sizeM; Level++ ) {
               _MuTurbField[Level]->decrRef();
          }
     if ( sizeA != 0 ) for ( int Level=0; Level<sizeA; Level++ ) {
     ...
     if ( _numod ) {
          ...
          double kappa, omega, epsilon;
          {
               ...
               if ( _emod ) {
                    ...
                    omega = ( 1.-_wlog ) * KappaAndOmega[1] + _wlog*exp ( _wlog*KappaAndOmega[1] );
               }
...
{     // REAL KAPPA, OMEGA AND EPSILON
     ...
     omega = ( omega>1.e-20 ) ? omega:1.e-20;
     if ( _emod ) {
          epsilon = ( epsilon>1.e-20 ) ? epsilon:1.e-20;
     }
...
void TurbUtils::DynTurInitValues ( double & kappa, double & omega, double WallDist, bool FlatProfile )
{
     ...
     if ( FlatProfile ) {
          ...
          const double utau = 0.5*_nu/_BoundWallDist;
