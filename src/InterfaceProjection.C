...
InterfaceProjection::InterfaceProjection(
    const MEDCoupling::MEDCouplingUMesh* SourceMesh, const MEDCoupling::MEDCouplingUMesh* TargetMesh,
    DomainType vol_sur)
    : MedUtils() {
  __Filled = 0;
  _AlreadyInitialized = 0;
...
InterfaceProjection::InterfaceProjection(
    const MEDCoupling::MEDCouplingUMesh* SourceMesh, std::vector<double> coord, int procId)
    : MedUtils() {
  __Filled = 0;
  _AlreadyInitialized = 0;
  setProcId(procId);
...
void InterfaceProjection::FillParameters(
    const MEDCoupling::MEDCouplingUMesh* SourceMesh, const MEDCoupling::MEDCouplingUMesh* TargetMesh,
    int DomainType, double XiEtaToll) {
  // Important!
  ...
  if (Type == 6 || Type == 7 || Type == 14 || Type == 20)
    _FamilyType = 0;
  else
    _FamilyType = 1;
  ...
  switch (DomainType) {
    case (Boundary):
      ...
      for (int iCell = 0; iCell < _TrgCells; iCell++) {
        ...
        for (int i_node = 0; i_node < TrgCellNodes; i_node++) {  // Loop over the element nodes
          ...
          for (int j = 0; j < _SpaceDim; j++) { PointsCoords[i_node * _SpaceDim + j] = coord[j]; }
          coord.clear();
        }
        ...
        int SrcIntCells =
            vettore
                ->getNbOfElems();  // Number of source mesh cells intersecting target mesh cell bounding box
        MEDCoupling::MemArray<int> SrcIntCellsIds =
            vettore->accessToMemArray();  // Array containing source mesh cell ids

        double** ExtrCoord = new double*[SrcIntCells];  // Matrix containing Min and Max coordinates of source
                                                        // mesh cells intersecting target mesh cell bounding
                                                        // box
        for (int i = 0; i < SrcIntCells; ++i) { ExtrCoord[i] = new double[2 * _SpaceDim]; }
        ...
        for (int IntCellNum = 0; IntCellNum < SrcIntCells; IntCellNum++) {
          ...
          for (int i = 0; i < 2 * _SpaceDim; i++) { ExtrCoord[IntCellNum][i] = minmax[i]; }
          SrcConn.clear();
          XYZ->decrRef();
        }
        ...
        for (int j = 0; j < TrgCellNodes; j++) {
          if (targetArray->getIJ(TrgConn[j], 0) > 0.5) {
            std::vector<int>
                CellIds;  // Vector where we store the Ids of cells that can contain the target mesh node j

            for (int IntCellNum = 0; IntCellNum < SrcIntCells; IntCellNum++) {
              if (_MeshDim == 2) {
                /// verifica dell'appartenenza del nodo j alla cella valori[conf]
                if (  //
                    PointsCoords[j * _SpaceDim] >= ExtrCoord[IntCellNum][0] - 1.e-9 &&
                    PointsCoords[j * _SpaceDim] <= ExtrCoord[IntCellNum][1] + 1.e-9 &&
                    PointsCoords[j * _SpaceDim + 1] >= ExtrCoord[IntCellNum][2] - 1.e-9 &&
                    PointsCoords[j * _SpaceDim + 1] <= ExtrCoord[IntCellNum][3] + 1.e-9 &&
                    PointsCoords[j * _SpaceDim + 2] >= ExtrCoord[IntCellNum][4] - 1.e-9 &&
                    PointsCoords[j * _SpaceDim + 2] <= ExtrCoord[IntCellNum][5] + 1.e-9) {
            ...
            // If there are two possible cells containing the target mesh node we search the one having
            // fabs(xi)<=1 and fabs(eta)<=1
            ...
            if (numberOfCell >
                ...
                if (_MeshDim ==
                  ...
                  if (fabs(XiEtaBound[0]) <= 1. + 1.e-5 && fabs(XiEtaBound[1]) <= 1. + 1.e-5) {
                    ...
                    std::cout << " cell found for node *********** " << j << " of cell " << iCell
                              << " calculated xi: " << XiEtaBound[0] << " eta: " << XiEtaBound[1]
                              << std::endl;
              ...
              for (int nMed = 0; nMed < SrcNodesConn.size(); nMed++)
                if (_MeshDim == 2) { Ordering[nMed] = nMed; }
              ...
              if (_MeshDim == 2) { XiEtaCalc_2D(NodeCoord, XiEtaBound, Quad4); }
              if (_MeshDim == 1) { XiEtaCalc_1D(NodeCoord, XiEtaBound); }
              //             std::cout<<" bob calculated xi: "<<XiEtaBound[0]<<" eta:
              //             "<<XiEtaBound[1]<<std::endl;
              for (int node = 0; node < _SrcCellNodes; node++) {
                BoundingNodes1->setIJ(TrgConn[j], node, SrcNodesConn[node]);
              }
              SrcNodesConn.clear();
            ...
            for (int dim = 0; dim < _MeshDim; dim++) { XiEta1->setIJ(TrgConn[j], dim, XiEtaBound[dim]); }
    ...
    case (Volume):
      ...
      //******************************************************************************
      // controllare metodo di ricerca ed utilizzo degli array elts e eltsIndex -> guarda classe modificata da
      // daniele
      ...
      //       for(int SNode = 0; SNode<_SrcCellNodes; SNode++) BoundingNodes1->setIJ(TNode, SNode,
      //       SourceConn[SNode]); for(int SNode1 = 0; SNode1<_SrcCoordInterpNodes; SNode1 ++) {
        ...
        if (NumPossibleCells > 0) {
          ...
          while (!found && iCount < NumPossibleCells) {
            ...
            for (int SNode = 0; SNode < _SrcCellNodes; SNode++) {
              //             BoundingNodes1->setIJ(TNode, SNode, SourceConn[MedLibmesh[SNode]]);
              //             //SourceConn[MedLibmesh[SNode]]=med index if(SNode <_SrcCoordInterpNodes)
              //             __InMesh -> getCoordinatesOfNode(SourceConn[MedLibmesh[SNode]],_CoordMatrix); //
              //             only linear nodes
              BoundingNodes1->setIJ(
                  TNode, SNode, SourceConn[SNode]);  // SourceConn[MedLibmesh[SNode]]=med index
          ...
          if (!found) {
            for (int SNode = 0; SNode < _SrcCellNodes; SNode++)
              BoundingNodes1->setIJ(TNode, SNode, -1);  // SourceConn[MedLibmesh[SNode]]=med index
          } else {
            for (int Mcomp = 0; Mcomp < _MeshDim; Mcomp++) { XiEta1->setIJ(TNode, Mcomp, _XiEtaChi[Mcomp]); }
          }
          _CoordMatrix.clear();
          ...
          count++;
          //         std::cout<< " Cell not found for target point "<< TNode << " count "<<count << " x:
  ...
  MEDCoupling::MEDCouplingFieldDouble* CanonicalPosition =
  ...
  MEDCoupling::MEDCouplingFieldDouble* CellS =
      MEDCoupling::MEDCouplingFieldDouble::New(MEDCoupling::ON_NODES);
...
void InterfaceProjection::FillParametersProbe(
    const MEDCoupling::MEDCouplingUMesh* SourceMesh, std::vector<double> coord, double XiEtaToll) {
  // Important!
  // Update the map when interpolation between different element types is available  //
  std::map<int, int> InterpCoordNodes;
  InterpCoordNodes[27] = 8;
  ...
  if (__Filled == 1) { __InMesh->decrRef(); }
  __InMesh = SourceMesh->deepCopy();
  __Filled = 1;
  ...
  int dimm = coord.size();
  double pos[dimm];
  for (int dim = 0; dim < dimm; dim++) { pos[dim] = coord[dim]; }
  ...
  if (NumPossibleCells > 0) {
    ...
    if (!found) {
      ...
      for (int Mcomp = 0; Mcomp < _MeshDim; Mcomp++) { XiEta1->setIJ(0, Mcomp, _XiEtaChi[Mcomp]); }
    }
    _CoordMatrix.clear();
    _XiEtaChi.clear();
  } else {
...
void InterfaceProjection::GetXiEta(int node, std::vector<double>& XiEtabound) {
  for (int dim = 0; dim < _MeshDim; dim++) { XiEtabound.push_back(_XiEta->getIJ(node, dim)); }
  return;
}
// ===================================================================================
void InterfaceProjection::GetXiEtaChi(int node, std::vector<double>& XiEtaChi) {
  for (int dim = 0; dim < _MeshDim; dim++) { XiEtaChi.push_back(_XiEta->getIJ(node, dim)); }
  return;
}
...
void InterfaceProjection::GetInterNodes(int node, std::vector<int>& VectorContainingNodes) {
  ...
  for (int i = 0; i < SourceNodes; i++) { VectorContainingNodes.push_back(_BoundingNodes->getIJ(node, i)); }
  return;
}

// ===================================================================================
int InterfaceProjection::GetBoundNodesPerCell() { return __BoundNodesPerCell; }

// ===================================================================================
int InterfaceProjection::GetDomainType() { return __Domain; }
...
void InterfaceProjection::XiEtaCalc_2D(
  ...
  for (int kdim = 0; kdim < _SpaceDim; kdim++) {
    ...
    for (int node = 0; node < npt_el; node++) {
      Co[kdim * _SpaceDim] +=
          0.25 * _CoordMatrix[_SpaceDim * node + kdim] * Quad4[node] * Quad4[node + npt_el];
  ...
  double beta_0 = -XYZs[id2] * Co[0 + 3 * id1] + XYZs[id1] * Co[0 + 3 * id2] +
                  Co[1 + 3 * id1] * Co[2 + 3 * id2] - Co[1 + 3 * id2] * Co[2 + 3 * id1];
  ...
  if (fabs(alpha_0) > 1.e-10) {  //  alpha_0 != 0 two roots 1.1e-10 ==========================================
    ...
    if (delta_0 < 1.e-10) { std::cout << " !!!!!!!!!!!!!!! Error delta " << std::endl; }
    ...
    if (fabs(xieta[0]) > 1.001 ||
        fabs(xieta[1]) >
            1.001) {  //  std::cout<<" first not good x1 "<< xieta[0] << " " << xieta[1] <<std::endl;
      xieta[1] = 0.5 * (-beta_0 - sqrt(delta_0)) / alpha_0;
      xieta[0] = -(-XYZs[id1] + Co[2 + 3 * id1] * xieta[1]) / (Co[1 + 3 * id1] + Co[0 + 3 * id1] * xieta[1]);
      if (fabs(Co[1 + 3 * id1] + Co[0 + 3 * id1] * xieta[1]) < 1e-10) {
        xieta[0] =
            -(-XYZs[id2] + Co[2 + 3 * id2] * xieta[1]) / (Co[1 + 3 * id2] + Co[0 + 3 * id2] * xieta[1]);
      }
    }
...
int InterfaceProjection::IsNodeContained() {
  ...
  double toll = 1.e-3;
  if (_FamilyType == 1) {
    if (fabs(_XiEtaChi[0]) > 1 + toll || fabs(_XiEtaChi[1]) > 1 + toll ||
        fabs(_XiEtaChi[DIMENSION - 1]) > 1 + toll)
      Verify = 0;
  }
  if (_FamilyType == 0) {
    if (_XiEtaChi[0] < -toll || _XiEtaChi[0] > 1 + toll || _XiEtaChi[1] < -toll || _XiEtaChi[1] > 1 + toll ||
        _XiEtaChi[DIMENSION - 1] < -toll || _XiEtaChi[DIMENSION - 1] > 1 + toll)
      Verify = 0;
  }
...
void InterfaceProjection::XiEtaChiCalc(double NodePos[]) {
  ...
  for (int dim = 0; dim < _MeshDim; dim++) { XiEtaChi[dim] = 0.; }
  ...
  while (!FoundXiEtaChi) {
    ...
    if (_MeshDim == 3) {
      DetHex = _d2F[0] * _d2F[4] * _d2F[8] + 2. * _d2F[1] * _d2F[5] * _d2F[2] - _d2F[4] * _d2F[2] * _d2F[2] -
               _d2F[8] * _d2F[1] * _d2F[1] - _d2F[0] * _d2F[5] * _d2F[5];
      for (int row = 0; row < _MeshDim; row++) {
        for (int col = 0; col < _MeshDim; col++) {
          if (row <= col) {
            ...
            InvHex[row * _MeshDim + col] = (1 - 2 * sign) *
                                           (_d2F[a * _MeshDim + c] * _d2F[b * _MeshDim + d] -
                                            _d2F[a * _MeshDim + d] * _d2F[b * _MeshDim + c]) /
                                           DetHex;
          } else {
    ...
    if (_MeshDim == 1) { InvHex[0] = 1 / _d2F[0]; }

    // Calculating the variation of xi, eta, chi
    for (int dir = 0; dir < _MeshDim; dir++) {
      double SUM = 0.;
      for (int i = 0; i < _MeshDim; i++) { SUM += InvHex[_MeshDim * dir + i] * _dF[i]; }
      deltaXEC[dir] = -0.5 * SUM;
      //     if(fabs(deltaXEC[dir])>3){
      //       std::cout<<"\033[1;31m Computed delta xi eta chi is greater than 2, now abort \033[0m"
      //       <<std::endl; abort();
      //     }
    }

    for (int i = 0; i < _MeshDim; i++) { XiEtaChi[i] += deltaXEC[i]; }
    ...
    if (count > 1000) {
      ...
      //       std::cout<<" Process stopped, error function: " << SquaredError << " xi: "<< XiEtaChi[0] << "
      //       eta " <<XiEtaChi[1]; if(_MeshDim==3) { std::cout<<" chi " <<XiEtaChi[2]; }
      //        std::cout<< std::endl;
    }
    _dF.clear();
    _d2F.clear();
  }
  for (int Mcomp = 0; Mcomp < _MeshDim; Mcomp++) { _XiEtaChi.push_back(XiEtaChi[Mcomp]); }
...
// Function for the calculation of F, first derivatives of F and second derivatives of F where F is the
// quantity
//                 F = (xp - xi)*(xp - xi) + (yp - yi)*(yp - yi) + (zp - zi)*(zp - zi)
// (xp,yp,zp) are the coordinates of the point of interest P and (xi,yi,zi) are the one calculated with
// (xi,eta,chi) Refer to "Exact and efficient interpolation using finite elements shape functions" (here the
// formula is correct)
// ************************ F ******************************
...
MEDCoupling::MEDCouplingFieldDouble* InterfaceProjection::InterpolatedField(
    const MEDCoupling::MEDCouplingFieldDouble* SourceField, const int order) {
  ...
  switch (__Domain) {
    case (Boundary):
      ...
      for (int iNode = 0; iNode < _TrgNodes; iNode++) {
        // Vector where we store the ids of the source mesh nodes that are used for the interpolation of the
        // solution on target mesh node iNode
        ...
        for (int dim = 0; dim < _MeshDim; dim++) { CanPos[dim] = XiEtaBound[dim]; }
        // qui
        std::map<int, int> MedLibmesh;
        BuildCanonicalElementNodesMap(_SrcCellNodes, MedLibmesh);
        double TrgValue = 0.;
        for (int iComp = 0; iComp < NComp; iComp++) {
          ...
          //           TrgValue +=  val*phi;
          //         }
          if (((EqName == "NS0" || EqName == "FSI0" || EqName == "FSIA0" || EqName == "NSA0") &&
               iComp == _SpaceDim) ||
              order == 1) {
            //             TrgValue = 0.;
    ...
    case (Volume):
      ...
      for (int iNode = 0; iNode < _TrgNodes; iNode++) {
        ...
        if (BoundingNodes[0] == -1) {
          ...
          for (int dim = 0; dim < _MeshDim; dim++) { CanPos[dim] = XiEtaChi[dim]; }
          double val, phi;
          for (int iComp = 0; iComp < NComp; iComp++) {
            if (((EqName == "NS0" || EqName == "FSI0" || EqName == "FSIA0" || EqName == "NSA0") &&
                 iComp == _SpaceDim) ||
...
MEDCoupling::MEDCouplingFieldDouble* InterfaceProjection::InterpolatedField(
    const MEDCoupling::MEDCouplingFieldDouble* SourceField,
    const MEDCoupling::MEDCouplingFieldDouble* TargetField, const int order) {
  const int NComp = SourceField->getNumberOfComponents();
...
MEDCoupling::MEDCouplingFieldDouble* InterfaceProjection::InterpolatedField(
    const MEDCoupling::MEDCouplingFieldDouble* SourceField, double DefaultValue, const int order) {
  const int NComp = SourceField->getNumberOfComponents();
  ...
  switch (__Domain) {
    case (Boundary):
      ...
      for (int iNode = 0; iNode < _TrgNodes; iNode++) {
        ...
        if (BoundingNodes[0] == -1) {
          for (int icomp = 0; icomp < NComp; icomp++) { targetArray->setIJ(iNode, icomp, DefaultValue); }
          BoundingNodes.clear();  //       break;
        } else {
          double CanPos[_MeshDim];
          for (int dim = 0; dim < _MeshDim; dim++) { CanPos[dim] = XiEtaBound[dim]; }
...
double InterfaceProjection::InterpolatedFieldProbe(
    const MEDCoupling::MEDCouplingFieldDouble* SourceField, const int order) {
  if (_OutOfDomain == 1) {
...
double InterfaceProjection::QuadPhi(int N, double GPoint[]) {
  ...
  if (_FamilyType == 1) {
    switch (_MeshDim) {
      case (1):
        value =
            (1. - 0.5 * fabs(_CoordEdge3[N])) * ((2. * fabs(_CoordEdge3[N]) - 1.) * GPoint[0] * GPoint[0] +
                                                 _CoordEdge3[N] * GPoint[0] + (1. - fabs(_CoordEdge3[N])));
        break;
      case (2):
        value = 1.;
        for (int dim = 0; dim < _MeshDim; dim++)
          value *= (1. - 0.5 * fabs(_CoordQuad9[N + dim * _Quad9Off])) *
                   ((2. * fabs(_CoordQuad9[N + dim * _Quad9Off]) - 1.) * GPoint[dim] * GPoint[dim] +
                    _CoordQuad9[N + dim * _Quad9Off] * GPoint[dim] +
                    (1. - fabs(_CoordQuad9[N + dim * _Quad9Off])));
        break;
      case (3):
        value = 1.;
        for (int dim = 0; dim < _MeshDim; dim++)
          value *= (1. - 0.5 * fabs(_CoordHex27[N + dim * _Hex27Off])) *
                   ((2. * fabs(_CoordHex27[N + dim * _Hex27Off]) - 1.) * GPoint[dim] * GPoint[dim] +
                    _CoordHex27[N + dim * _Hex27Off] * GPoint[dim] +
                    (1. - fabs(_CoordHex27[N + dim * _Hex27Off])));
        break;
    }
  }
  if (_FamilyType == 0) {
